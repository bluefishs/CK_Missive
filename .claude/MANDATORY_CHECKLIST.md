...
### ⚠️ 驗證總則（2026-09-03，L124／L138／L139 三個家族合併）

「驗證通過」只在下面三句都成立時才算數：
- [ ] **打端點，不打服務層**——服務層與端點之間隔著 Pydantic 驗證、參數轉換、回應包裝；容器內 `httpx.ASGITransport(app=app)` 一次就讓 100→100／1000→226／1001→422 現形（L124）。
- [ ] **印原始回應形狀**——不只印自己解析出來的數字；`data.items` 與頂層 `items` 差一層就是永遠 0 筆。
- [ ] **做負向對照**——判準改完先確認它抓得到已知的壞例（首版 weekly 101 對 inline tuple 是盲的、印 GREEN；L138）。
- [ ] **rollback 不是 dry-run**——鏈路上任何一支內部 `commit()` 都會讓外層 rollback 失效；試算前先 grep（L139，同一教訓第二次）。
- [ ] **收斂成單一來源的驗收，是用會走那條路的操作去打**——靜態 grep 找得到想到的寫法，找不到沒想到的。
