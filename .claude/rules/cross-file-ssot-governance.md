...
| **SSO session 存活期 TTL**（I10/L80 新增 7/21，跨 repo）| **策略值單一：IdP `ck_employee` cookie TTL >= 消費端 SSO session TTL**；三處＝IdP `CK_Website/functions/auth/callback.ts`（signJWT+Max-Age）／消費端 `SSO_ACCESS_TOKEN_EXPIRE_MINUTES`（各子系統 config）／前端 idle timeout（活動制，獨立不比較） | **消費端 session TTL 若 > IdP cookie TTL → session 過期時 IdP cookie 已先失效、P1 refresh 無痛續命失憑證 → 使用者被迫重登。現況（2026-08-07 實測）：IdP 8h ＝ Missive 8h，GREEN 對齊**（原記「IdP 4h」為過期記載；稽核 `sso_ttl_ssot_audit.py` 寫於 7/21 卻**從沒接進任何 runner**，故 IdP 改值無人知曉 → 2026-08-07 接為 weekly **step 31**）。⚠️ **對齊只保證「不會提早死」，不代表使用者無感**：TTL 相等時兩者同時到期，8h 一到必然重登；且 Missive 在 cookie 只剩 10 分鐘時仍會發出 60 分鐘的 session（owner 08-07 實測：16:39 refresh 成功、憑證 16:49 死、17:39 才爆）。**故重點不在拉長 TTL，而在到期時必須是明確的「請重新登入」而非靜默失敗** —— 見 `auth/session.py` 的 `X-Reauth-Required`。**2026-08-08 起另有 sliding renewal**：IdP `POST www.cksurvey.tw/auth/renew`（`CK_Website/functions/auth/renew.ts`）讓「有在用就續」，含撤銷檢查與 **7 天絕對上限**；消費端接線指南＝`CK_Website/docs/SSO-SLIDING-RENEWAL.md`（Missive 已接，lvrland／pile／DT **待接**）。⚠️ 續期**只能由 IdP 做**：消費端雖持有 HS256 secret，但讓四個系統都能簽發＝四個發證機關，任一被攻破等同 IdP 被攻破（L41 家族） |
