...
| `scripts/checks/rwd_mobile_quality_gate.py`＋`rwd_mobile_quality_probe.cjs` | **RWD 手機品質閘門**（owner 09-05「加強視覺檢核機制，確保 RWD 設計正確性」）——weekly 109 只守整頁撐寬、視覺走查拍圖要人看；這支在 host 以 adapter 簽臨時憑證、Playwright 390px 登入量五件「人看圖會抱怨、斷言量得出來」的事：截字（非 ellipsis 的硬截）／字級<11px／點擊目標<28px／fixed 浮動元件壓住可點元素 ≥40%／統計卡獨佔一列（§2.6 ①）。RED＝遮蔽、獨列、截字超基線；字級與點擊目標走基線 `.rwd_quality_baseline.json`（首跑 327／399；同晚字級地板與點擊目標地板後 0／59，容忍 +5 浮動）。**390 與 1440 兩寬度都跑**，另量 `narrowSelect`（下拉 <80px；09-05 FilterBar 的桌面回歸只有視覺走查看得到，負向對照 3＋3 → 0）。⚠️ 首版把外層容器 col 也算成統計卡，行事曆兩張一列被誤報獨列——只看最內層 col 才對；遮蔽判準是「捲到視窗中央仍重疊」，否則任何浮動鈕在任何位置都壓到某個東西。不動 vendored 引擎 | weekly 111 |
| `scripts/checks/role_enum_drift_audit.py` | **角色列舉 vs `role_permissions`** —— 角色的權威在資料表，而 `schemas/auth.py` 的 `UserRole` 是手抄的一份。`exec`／`ops`／`finance` 2026-08-27 進了資料表與權限頁，列舉沒跟上 ⇒ 把任何人改成高階主管／營運管理／財務**一律 422 且沒有任何訊息**，畫面上看不出原因，卡了 11 天（owner 09-07 回報）。⇒ **角色做出來了、列得出來、就是指派不上去**。DB 有而列舉沒有＝RED；反向＝YELLOW。刻意不比對前端 `constants/permissions.ts`（那是建立角色時的預設權限範本，不是「現在有哪些角色」） | weekly 118 |
| `scripts/checks/rwd_crushed_column_control.cjs` | **weekly 111 `crushedCol` 判準的正負向控制** —— 判準新增當天全站跑出 0，而 0 有兩種意思（沒有這個問題／永遠不會紅）。用判準**自己的實作**（require 探針的 `measure`，不抄第二份）跑造出來的表：名稱欄被擠到 ~30px 要紅、60px 的「案數」與 220px 的名稱欄都不能紅。它紅代表判準壞了不是頁面壞了 | weekly 111 開跑前自檢 |
| `scripts/checks/async_sync_io_audit.py` | **async 路徑上的同步 I/O**（效能評估 #5）——09-05 一支同步 HTTP（對已廢止主機 DNS 失敗 4 秒）在事件迴圈上跑，同時進來的每支請求一起變 4.5 秒，探針全綠、p50 正常。AST 掃 `async def` 本體內未經 `to_thread`／`run_in_executor` 的 `requests.*`／`httpx.Client(`／`subprocess.run(`／`urlopen(`／`time.sleep(`＝RED；`os.walk`／`rglob`／`shutil.copy*` YELLOW。首跑 11 處 RED（含 Google 登入的同步 token 驗證、備份的 pg_dump、五處 git 快照）全部當天改成 `to_thread` ⇒ 基線 0，新增即紅。不算巢狀的同步 `def`（那是丟給執行緒的本體） | weekly 112 |
| `scripts/checks/testing_map_report.py` | **自主測試機制圖（僅報告）**——owner 09-06「把散在各處的自主測試層盤成一張機制圖」。層定義在腳本（靜態），新鮮度從各層留痕檔讀（動態）；產出 `docs/health/TESTING_MAP.md`。不判紅：判紅是各層自己的事，這支只照出「哪一層沒人在跑」（前端 vitest 沒留痕就是這樣被照出來的） | weekly 113 |
| `scripts/checks/frontend_test_suite_health.py` | **前端 vitest 基線比對**——228 個測試檔此前**沒有任何排程或閘門在跑**，首次完整跑 254 失敗；三個共同根因（barrel 部分 mock 沒跟上新 export／`shared-modules/sso-js` 自帶第二份 React 讓 zustand 的 `useRef` 讀到 null／`useResponsive` mock 漏 `responsiveValue`）修掉後剩 85，逐檔的「畫面改了測試沒跟」進基線待清。同 weekly 24 的三道守衛：跑不起來不寫基線、0 失敗要 `--force`、解析要對得上 JSON 摘要 | weekly 114 |
| `scripts/checks/prepush_related_tests.py` | **只跑相關測試的 pre-push**（A46 快速版）——原 `.git/hooks/pre-push` 3 階段全套 467 秒從未被 git 執行（hooksPath 指向 husky）。改問「推送範圍改了哪些檔」：後端 `test_<stem>*.py`／前端 `<Stem>*.test.tsx` 對映，對兩份基線只擋新失敗；找不到相關測試放行並印出（放行≠綠，是沒蓋到）。緊急 `--no-verify` | `frontend/.husky/pre-push`（每次 git push） |
| `scripts/checks/route_cost_trend_report.py`＋`visual_walk_weekly.sh` | **效能與視覺兩層從「手動」改成「每週留痕」**（機制圖缺口 4）——前者把每路由的 API 支數與 wall 入庫、與上次比對（>50% 或多 3 支才提），後者每週拍五個代表頁存 `docs/health/visual/<日期>/`。**兩支都不判紅**：效能隨資料量變動、視覺要人看，做成閘門只會得到沒有人看的紅燈 | weekly 115／116 |
| `scripts/checks/infra_facts_drift_audit.py` | **基礎設施事實漂移** —— owner 09-08 從舊附件讀到「資料庫三套（sqlite／5434／GeminiCli）、後端兩套（8001／8003）、行事曆兩條認證」，逐項實查都不存在，出處是 5 份未作廢的活文件（DEVELOPMENT_GUIDE／SYSTEM_CONFIG_CHECKLIST／UNIFIED_CONFIG_GUIDE 的 Adminer 8080、Google_Calendar_Integration_Setup 的 `./credentials.json`＋把 SSO 鍵寫成服務帳號欄位、frontend/README 的 8003）。活文件出現這些字樣＝RED；`archived/`、CHANGELOG、歷史 `~~` 行與「作廢／已不存在」的更正句豁免。⚠️ 首版掃描迴圈縮排錯＝永遠綠，負向控制抓到才修 | weekly 122 |
| `scripts/checks/roc_date_ssot_audit.py` | **民國年解析唯一定義**（A119）——收斂前 `_roc_to_date` ×4、`_parse_roc_date` ×4 加 19 處散裝 `+ 1911`，每一份只吃自己那個來源的格式；09-08 收到 `app/core/roc_date.py`（`parse_roc_date`／`roc_to_iso`／`roc_year_to_ad`），原名字留作薄委派。判準走 AST 找 `+ 1911`、該檔不 import 唯一定義即 RED；豁免 `case_code.py`（§2.5 輸入容錯）。⚠️ 同日另一發現：**weekly 117–122 六支從未登記進 `run_fitness_weekly.sh`**，寫了檢核沒有 runner 在叫＝L111 同型，一併登記 | weekly 123 |
| `scripts/checks/duplicate_definition_audit.py` | **同一個名字在幾個模組裡各有一份**（owner 09-08：「重複版次多次圖譜或檢核都沒發現」）——既有檢核只比對有人先命名過的那一對宣告，沒有一支問「同一概念有幾份」。代理判準＝同名**私有**函式（底線開頭、非 dunder）定義於 ≥3 模組；存量走基線，新名字／模組數增加 RED | weekly 124 |
| `scripts/checks/doc_lifecycle_audit.py` | **文件生命週期**（A118）——「過期」此前不是可量的屬性（172 份活文件 32 份有狀態）。檔頭 `> \`lifecycle: status=current reviewed=YYYY-MM-DD owner=CK_Missive\``（CK_AaaP 09-08 拍板，跨 repo 同格式），**有核對才寫日期、不得批次蓋章**；活區出現 superseded RED、新增無檔頭 RED、>90 天 YELLOW；閘門 `declaration_gate` 同日加第三處（runner 登記） | weekly 125 |
| `backend/app/services/erp/party_resolver.py`＋`scripts/checks/entry_time_guard_audit.py` | **把事後稽核搬到填報當下**（owner 09-07）——名稱→鍵四層解析（統編／精確／正規化／唯一前綴），不明確回候選不猜；併寫偵測（`銢欣有限公司乃耳企業社`／`楊長燁加李雅倫`／`祐鴻+昱緯+建倫`）在建立時就擋，因為**事後要拆時系統裡已經沒有憑證能決定比例**。發票：稅額須 0 或 5%、金額不得超過所屬請款、未指定時自動綁同額且唯一未綁的那一期（兩期同額不猜）。收款：不得超過請款額。守衛本身由 weekly 117 以執行時呼叫驗證（含負向控制） | weekly 117 |
