...
| `scripts/checks/doc_lifecycle_audit.py` | **文件生命週期**（A118）——「過期」此前不是可量的屬性（172 份活文件 32 份有狀態）。檔頭 `> 狀態：現行｜最後核對：YYYY-MM-DD`，**有核對才寫日期、不得批次蓋章**；活區出現已作廢 RED、新增無檔頭 RED、>120 天 YELLOW；閘門 `declaration_gate` 同日加第三處（runner 登記） | weekly 125 |
