...
| `scripts/checks/rwd_mobile_quality_gate.py`＋`rwd_mobile_quality_probe.cjs` | **RWD 手機品質閘門**（owner 09-05「加強視覺檢核機制，確保 RWD 設計正確性」）——weekly 109 只守整頁撐寬、視覺走查拍圖要人看；這支在 host 以 adapter 簽臨時憑證、Playwright 390px 登入量五件「人看圖會抱怨、斷言量得出來」的事：截字（非 ellipsis 的硬截）／字級<11px／點擊目標<28px／fixed 浮動元件壓住可點元素 ≥40%／統計卡獨佔一列（§2.6 ①）。RED＝遮蔽、獨列、截字超基線；字級與點擊目標走基線 `.rwd_quality_baseline.json`（首跑 327／399，多為 AntD small 控制項，存量不判紅）。⚠️ 首版把外層容器 col 也算成統計卡，行事曆兩張一列被誤報獨列——只看最內層 col 才對。不動 vendored 引擎 | weekly 111 |
