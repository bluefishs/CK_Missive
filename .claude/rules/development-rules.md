...
## 2.6 列表頁三要素：統計卡／互動篩選／當年度基準（owner 2026-08-29 裁示）

> **「具備統計圖卡與列表互動篩選機制，且皆以當年度為統計基準，以利檢索查詢」**

> ⭐ **owner 2026-09-05 定為首要核心**：「統計圖卡對應動態篩選、資訊卡片呈現風格，以及數據正確性與一致性」。
> 意思是：①每張統計卡點下去**必須真的篩列表**（只換底色不算——09-04 一天抓到 `/contract-cases`、`/erp/quotations` 兩頁的卡片是假的）；
> ②卡片（統計卡、手機列表卡 `MobileCard`）樣式統一走共用元件，不各頁自畫；③卡片與列表的數字必須同口徑（同年度、同含稅基準、同一個算法），
> 見 `FIELD_SEMANTICS.md`「經費名詞字典」。守門＝weekly 82（分母）＋weekly 108（卡片是否接到篩選）。

**所有業務列表頁**（`/pm/cases`、`/erp/*` 各列表、`/contract-cases` 等）必須具備：

| 要素 | 規定 | 反例 |
|---|---|---|
| ① **統計圖卡** | 卡片的數字必須是**分頁前的全量**，由後端計算下發；**網格統一 `<Col xs={12} sm={6}>`**（三張卡 `sm={8}`）——手機兩張一列，同 `/erp/quotations`（owner 09-05：「部分頁面採獨立列，應統一」；weekly 108 判 `xs={24}`） | ❌ 前端 `items.reduce(...)` 只加總當頁 —— 資料超過 `limit` 就靜默少算而不報錯 |
| ② **互動篩選** | 卡片可點擊，點下去篩選列表；再點一次取消 | ❌ 純 `Statistic` 唯讀卡 —— 使用者看到數字卻無法追進去 |
| ③ **當年度基準** | 預設當年度（**西元**，見 §2.5），可切「全部年度」 | ❌ 不篩年度 ⇒ 所有年度混在一起算成一個總數 |

| ④ **篩選與排序走表格本身**（owner 2026-09-04） | 表頭漏斗（`filters`＋`filterMultiple:false`）、欄位搜尋框、點欄名排序（`sorter:true`）；**後端分頁時勾選值經 `Table onChange` 進查詢參數、由後端篩全庫**，欄位**不帶 `onFilter`**（帶了會被 `stripClientOnlyColumnFeatures` 連漏斗一起剝掉）。工具列的下拉**可以留**（owner 09-04 晚：「不是移除原篩選機制，是要完善表頭」），但必須與表頭漏斗**共用同一份狀態**——在哪邊改另一邊同步顯示 | ❌ 工具列下拉與表頭漏斗各持一份狀態（兩套機制）；❌ 漏斗帶 `onFilter` 在後端分頁下只篩本頁；❌ 有漏斗就把工具列拆掉（第一版就是這樣做錯的） |
...
- **①** 2026-08-29 實測：發票彙總卡只算當頁 20/48 筆，顯示 1,892,988 而正確值是
  7,258,898（**少 74%**），而它不會報錯。同型另有統一帳本（誠實標成「本頁收入」——
  標籤說了真話，但那不是使用者要的數字）與委託單位帳款。
- **②** 卡片是入口不是裝飾：看到「待審核 5 筆」的下一個動作必然是「哪 5 筆」。
...
（獨立 `/totals` 端點，且刻意把 `entry_type` 排除，讓卡片分母不隨點選歸零）
是兩種正確形狀。**卡片是分母，不該隨自己的篩選變動。**

⚠️ 不適用者：詳情頁、儀表板、圖譜頁（非列表）；資產類存量頁的「年度」語意
需另議（購置年度 vs 在帳年度不同）。
