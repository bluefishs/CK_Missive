...
（09-06 的 12 筆名稱漂移與應付 #51 的併寫，全是這樣來的）。

| 入口 | 填報當下做什麼 | 對應的事後稽核 |
|---|---|---|
| 協力廠商指派／應付 | `party_resolver` 四層解析（統編→精確名→正規化名→唯一前綴），不明確回候選**不猜**；解析不到**出聲**而不是靜靜留一個沒有鍵的名字；名稱存成主檔現行寫法 | weekly 107（名稱對得到主檔卻沒填鍵） |
| ├ 併寫偵測 | `銢欣有限公司乃耳企業社`／`楊長燁加李雅倫`／`祐鴻+昱緯+建倫` 一律擋下請人一家一筆 —— **事後要拆時系統裡已經沒有憑證能決定比例**（09-06 實證） | 09-06 拆帳卡關 |
| 發票 | ①稅額必須 0 或 5%（誤把未稅當含稅會被擋）②金額不得超過所屬請款 ③未指定請款時**自動綁**同額且唯一未綁的那一期（兩期同額不猜） | weekly 104 ②⑤／weekly 99 |
| 收款 | 收款額不得超過請款額（超過會讓「應收未收」變負而報表看不出是哪一筆） | 09-06 逐案查證 |
| 守衛本身 | **weekly 117**：執行時真的呼叫服務層驗證每一條會不會擋（grep 只能證明有那段字），含 3 條負向控制 | — |

端到端實測（公網映像 e5061831）：併寫 400、未知廠商 400、正常廠商 200；發票超額 400、稅額不相稱 400、同額自動綁 200 且 `billing_id` 正確。
全量正向控制：26 家協力廠商解得出 24 家 —— **解不出的 2 家是主檔自己就併寫**（`楊長燁加李雅倫`／`祐鴻+昱緯+建倫`），列入待辦。
...
### 持續優化（09-05 晚，/loop「請持續優化」五輪）

| 輪 | 內容 | 量化 |
|---|---|---|
| ① | 手機字級地板（<768px 把 inline 10px 文字升 12px）；`/pm/cases` 表頭補年度／類別／狀態漏斗（§2.6 ④） | weekly 111 tinyFont 327 → 0 |
| ② | 費用列表手機卡片（分組可展開）、分組表每頁 20 組；對不到主檔的舊案號費用標「未對應案件＋案號」 | smallTap 392 → 262 |
| ③ | 視覺走查桌面圖抓到 FilterBar 桌面回歸（下拉塌成 40px，109／110 都不會紅）⇒ 改 flex；weekly 111 加 `narrowSelect` 並在 390／1440 兩寬度都跑 | 塌陷 3＋3 → 0 |
| ④ | 請求日誌降噪（CK_AaaP 跨 session 回報）：START 只在 DEBUG 記、health／metrics／根路徑等 2xx 不記 END 與 access、連線池訊息降 DEBUG | 10 分鐘 1,243 → 119 行（−90%），Loki 複驗一致 |
| ⑤ | 手機點擊目標地板（小尺寸分頁鈕／按鈕／小輸入框墊到 32px） | smallTap 262 → 59，閘門 GREEN |

教訓入冊：L142「檢查通過≠到達生效路徑」（互引 `CK_AaaP#L51`／`#L37`）、L143「工具存在但沒有成為預設路徑」。
...
## 附錄：自 `rules/skills-inventory.md` 檔頭移入的版本變更史

> 2026-08-27 /doctor 移入。它原本在一份**每個 session 都載入**的規範檔開頭，
> 4,512 字元的歷史敘事每次都要進 context 一遍。搬移不是刪除。
