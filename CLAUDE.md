...
> **版本**: v6.73（2026-09-05）/ ⭐⭐⭐**「工具存在但沒有成為預設路徑」**：owner 問「不是可以模擬手機登入檢測嗎」——可以，`run.sh --visual` 早就會簽臨時憑證以 390px 登入拍圖，我卻繞了 Chrome（沒登入態）與免認證 Vite（沒資料）一小時；現加 `--click=` 先點再拍。⭐⭐**年度＝案號年**：委託單位帳款此前年度只掛在報價單金額子查詢（案件數沒篩、桃園任何年度都 7 案），且 `erp_quotations.year` 是補建那年（14/277 與案號年不同）⇒ 案件類統計一律案號年（`repositories/erp/case_year.py`）；承攬金額同口徑後 2026 委託單位帳款 83,099,209.5 ＝ `/erp/quotations`。⭐⭐**名稱／鍵成對欄位全庫普查**：公文機關 186 筆、帳本 5 筆可精確對上卻沒鍵 ⇒ 回填；帳款腿 2 改主檔鍵分組（桃園「用地科」不再另列）；weekly 107 擴到公文（漂移只認毫無交集）；委託單位雙主檔＝A105。⭐**RWD 檢核加厚**：weekly 111 手機品質閘門（截字／字級／點擊目標／遮蔽／統計卡獨列），首跑抓到浮動鈕壓住分頁鈕、4 頁統計卡獨列，已修；財務儀表板加依計畫類別各機關應收／協力廠商應付。待 owner：A99 快照漂移 12 筆／A105。⭐**晚間 /loop 五輪**：手機字級與點擊目標地板（tinyFont 327→0、smallTap 392→59）、FilterBar 桌面回歸（下拉塌成 40px，只有視覺走查看得到 ⇒ weekly 111 加 `narrowSelect` 雙寬度）、請求日誌降噪（10 分鐘 1,243→119 行，CK_AaaP 以 Loki 複驗）、L142／L143 入冊。
