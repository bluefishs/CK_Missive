...
> **版本**: v6.62（2026-08-27）/ ⭐⭐⭐走查的「一般同仁」身分**做好了三天而沒有任何排程在用它**（能力有了會讓人以為有人在跑；補完整條鏈時我自己當場示範了 L81 —— 兩支任務讀同一個檔）＋⭐⭐**同一個排程判準四小時內修三次，每一次都是 CK_Hermes session 先看到我的盲點**：①「註冊晚於 StartBoundary」是通則性誤報（我把 Windows 的 `1999-11-30` 哨兵值當成時間讀出 9767 天，而本檔另一段自己就認得那個狀態碼）②回頭掃才發現**高頻排程根本沒有判準** —— 容器健康告警停了要 **8 天**才會有人知道 ③我的修復**隔夜重啟就會誤報**（`LastRunTime` 跨重啟保存，而關機期間不可能 fire）＋⭐⭐**紅燈說「這件事不對」不代表「把它變綠」是對的**：我啟用了一支「被停用正是對的」排程（它跑的腳本 08-02 起就設計成拒絕執行），已還原並列 A28＋逾期告警的年齡判不出來時回 0.0，於是那份提案**永遠達不到門檻**（註解承諾的 mtime fallback 從未實作；現況實測影響 0，是潛伏不是活的）
