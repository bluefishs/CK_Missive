...
> **版本**: v6.72（2026-09-04）/ ⭐⭐⭐**「每張表單獨看都正常，鏈才是斷的」第二天**：承攬案的委託單位三個欄位沒有一個是 `partner_vendors` 的鍵（與 PM 的 `client_vendor_id` 不是同一個 id 空間），帳款／篩選／主檔全靠字串對 ⇒ 一天三個「無法關聯」（竹崎地政無法點、張啟良三筆主檔、大有國際在主檔是協力廠商）。migration 20260904a003 加 `contract_projects.client_vendor_id` 回填 271/285，規則入 `FIELD_SEMANTICS`「主檔鍵與名稱快照」（id 是鍵、名稱是快照），weekly 107 守門。⭐⭐**指派即應付**（協力廠商指派↔應付沒有橋，16 案 13 無應付；weekly 106）／**帳款頁只列 50 家**（後端預設 limit）／**統編 vs 代碼**統一 `tax_id`／應收分頁**發票號碼欄合一**＋開立防呆（格式、全庫重複）＋**未關聯發票可直接掛到請款**＋weekly 104 ⑪ 已收無票／**篩選排序走表格本身**（owner：表格能篩就不重複做下拉；§2.6 ④，`/contract-cases` 為參考實作，其餘頁＝A98）／統計卡此前只換底色從未套用。7 次部署全通（探針 15/15）。待 owner：A96／A97／A99。
