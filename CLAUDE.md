...
>
> 🗺️ **09-03 全景覆盤**＝[`docs/architecture/SYSTEM_ARCHITECTURE_REVIEW_20260903.md`](docs/architecture/SYSTEM_ARCHITECTURE_REVIEW_20260903.md)：
> **系統的能力已經超過使用它的人**——近 30 天有操作紀錄的只有 5 人，承辦最多的兩位沒在系統上操作，資料靠總表匯入。
> 七個跨流程結構問題（入口錯位／同一事實多份／語意沒寫死／沒有起點的鏈／檢核沒收件人／分層是方向／驗證的替身）；
> 順序：**先 F3 承辦登入與 G1 紅燈有名字（零開發）**，再 F1 入口搬進系統。
>
> ⭐⭐**09-03 第五輪（ERP 正確性優先）**：欄位語意宣告 `FIELD_SEMANTICS.md`＋weekly 104 三方對帳（首跑 RED 0）／有請款後鎖金額／匯入審計／
> `erp_invoices.source`／通知分級＋缺口到承辦／部署第五層 probe 13 斷言不通即停。⭐⭐**匯出→匯入往返做了才看得見的三個副作用**：
> 補建 21 個 PM 案殼（對舊制／GN 案號也補）、103 筆指派用舊案號當 `case_code`（推不出就照抄——附件掛錯鍵同族）、指派查重只認一條綁法（**同族第十一處**）。
> ⇒ **凡是「匯入」都要用它自己的匯出檔跑一次往返，前後五張表計數不變才算冪等。**
>
> **最後更新**: 2026-09-03
>
