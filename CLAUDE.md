...
> **版本**: v6.74（2026-09-06）/ ⭐⭐⭐**這一輪沒有新功能，全部投在「壞了會有人知道」**：把散在各處的自主測試層（提交閘門／每日 runner／頁面走查／流程走查／視覺／部署態／效能／跨 repo 互驗）盤成一張機制圖（`docs/architecture/AUTONOMOUS_TESTING_MAP.md`，動態版 weekly 113），照出兩個**沒有任何排程或閘門在跑**的套件：前端 vitest 228 檔（首跑 254 失敗）與後端 pytest 基線（實際 65 失敗而基線只登 2）。⭐⭐**大面積紅的測試不是一支一支壞，是幾個共用契約一起壞**：前端 254 裡三個共同根因佔三分之二（barrel 部分 mock 蓋掉後來新增的 export／`shared-modules/sso-js` 自帶第二份 React 讓 zustand 的 `useRef` 讀到 null／`useResponsive` mock 漏 `responsiveValue`），逐檔對齊契約後**兩份基線都歸零**（前端 2,960 過、後端 4,455 過），weekly 24／114 從此任何一筆失敗都是新問題。⭐⭐**A46 pre-push 以「只跑相關測試」的形式結案**（全套 467 秒不接；推送範圍改到的檔對映測試、只擋基線外新失敗、前端先關並行複跑排除干擾）。⭐**效能**：`pg_stat_statements` 常駐，首剖析即抓到招標搜尋的相似度查詢佔 DB 時間 89%（已移除）；weekly 112 守 async 路徑上的同步 I/O（首跑 11 處當天改完 ⇒ 基線 0）。⭐**唯一的產品修正**：派工作業紀錄端點對不存在的派工單原本讓外鍵違反冒成 500，改為先查、回 400 並說原因。⭐**A110 零流量 API**：69 個候選裡 65 個前端有呼叫、3 個 Hermes 打的、真正沒人引用 1 個（已刪）——「零流量」不等於「死的」，判準改成零流量且無任何呼叫者才交給人看。
