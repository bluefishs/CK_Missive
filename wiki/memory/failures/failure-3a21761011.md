...
type: agent_memory
memory_type: failure
signature: 3a21761011
tool_sequence: ["find_correspondence", "search_dispatch_orders", "search_entities"]
hit_count: 18
failure_count: 15
failure_rate: 0.833
active: false
...
# Failure Mode 3a21761011

## Tool sequence（問題組合）

`find_correspondence`, `search_dispatch_orders`, `search_entities`
...
**觸發**：規劃包含 `find_correspondence` + `search_dispatch_orders` + `search_entities` 的組合
