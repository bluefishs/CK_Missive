...
import { App, Spin, Alert, Tabs } from 'antd';
...
const CodeGraphManagementPage: React.FC = () => {
  ...
  // Export functions — migrated from xlsx (SheetJS) to exceljs 2026-05-21 (CVE GHSA-4r6h-8v6p-xvw6 / GHSA-5pgg-2g8v-p4x9)
  const exportArchitectureLayers = useCallback(async () => {
    ...
    try {
      ...
      const ws = wb.addWorksheet('架構層統計');
      const headers = Object.keys(layerRows[0] || { 架構層: '' });
      ws.addRow(headers);
      layerRows.forEach(r => ws.addRow(headers.map(h => (r as Record<string, unknown>)[h])));
      const buf = await wb.xlsx.writeBuffer();
      saveAs(
        new Blob([buf], {
          type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        }),
        `架構層統計_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.xlsx`
      );
    } catch {
      /* noop */
    }
  ...
  const exportDbErd = useCallback(async () => {
    ...
    try {
      ...
      const wb = new ExcelJS.Workbook();
      const ws = wb.addWorksheet('DB ERD');
      const headers = Object.keys(rows[0] || { 資料表: '' });
      ws.addRow(headers);
      rows.forEach(r => ws.addRow(headers.map(h => (r as Record<string, unknown>)[h])));
      const buf = await wb.xlsx.writeBuffer();
      saveAs(
        new Blob([buf], {
          type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        }),
        `DB_ERD_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.xlsx`
      );
    } catch {
      /* noop */
    }
  ...
  return (
    <div style={{ display: 'flex', gap: 0, height: 'calc(100vh - 120px)', overflow: 'hidden' }}>
      <CodeGraphSidebar
        ...
        loadStats={loadStats}
        codeIngestLoading={handlers.codeIngestLoading}
        cycleLoading={handlers.cycleLoading}
        archLoading={handlers.archLoading}
        jsonImportLoading={handlers.jsonImportLoading}
        ingestIncremental={handlers.ingestIncremental}
        setIngestIncremental={handlers.setIngestIncremental}
        ingestClean={handlers.ingestClean}
        setIngestClean={handlers.setIngestClean}
        jsonClean={handlers.jsonClean}
        setJsonClean={handlers.setJsonClean}
        handleCodeGraphIngest={handlers.handleCodeGraphIngest}
        handleJsonImport={handlers.handleJsonImport}
        handleCycleDetection={handlers.handleCycleDetection}
        handleArchAnalysis={handlers.handleArchAnalysis}
        codeWiki={codeWiki}
      />
                    ...
                    <KnowledgeGraph
                      ...
                      onNodeClickExternal={node => {
                        if (node.type === 'menu_module') {
                          const mod = moduleMappings.find(
                            m => m.key === node.id.replace('mod_', '')
                          );
            ...
            },
            {
              ...
              children: (
                <ArchitectureOverviewTab
                  ...
                  moduleOverviewLoading={moduleOverviewLoading}
                  loadModuleOverview={loadModuleOverview}
                  layerTableData={layerTableData}
                  erdSearchText={erdSearchText}
                  setErdSearchText={setErdSearchText}
                  filteredErdData={filteredErdData}
                  exportArchitectureLayers={exportArchitectureLayers}
