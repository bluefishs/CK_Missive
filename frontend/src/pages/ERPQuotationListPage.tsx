...
export const ERPQuotationListPage: React.FC = () => {
  ...
  // 2026-09-09 owner：選承辦後列表 85 張、卡片仍是全公司 2,149 萬——統計卡是列表的分母。
  // 晚：改送**整組列表篩選**（weekly 133），只去掉分頁／排序／卡片自己（card 是分母的例外）。
  const { data: profitSummary } = useERPProfitSummary({
    year: params.year, search: params.search, category: params.category, client_name: params.client_name,
    staff_user_id: params.staff_user_id, status: params.status, case_status: params.case_status,
    case_code: params.case_code, include_unawarded: params.include_unawarded, anomaly: params.anomaly,
  });
