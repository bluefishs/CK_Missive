...
與「開機就把它們載進記憶體」。**這是兩件事**，而混在一起會在每次重啟做一次沒有效益的預熱。

⚠️ 2026-09-09 06:1x 訂正：**不要用「省記憶體」當理由**。CK_Website 撤回了那個推論 ——
`docker stats` 的 10.36 GiB 裡不可回收的匿名記憶體只有約 1.4 GB，其餘是可回收的 mmap，
而 `qwen2.5:7b` 是 100% GPU。理由是**效益未證實**，不是成本高。
