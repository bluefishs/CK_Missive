...
測試範圍:
- 通知建立: create_notification
- 未讀數量: get_unread_count
- 標記已讀: mark_as_read, mark_all_as_read
- 通知查詢: get_notifications (含分頁)
- 業務通知: notify_critical_change, notify_document_deleted, notify_import_result
- 實例模式: list_notifications, mark_notifications_read, mark_all_read_for_user
- 異常處理: 資料庫錯誤回退
...
# ============================================================
# get_unread_count 測試 (靜態方法)
# ============================================================

class TestGetUnreadCount:
    """get_unread_count 方法測試"""
    ...
    async def test_get_unread_count(self, mock_db):
        ...
        result = await NotificationService.get_unread_count(mock_db, user_id=1)

        assert result == 5
        mock_db.execute.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_get_unread_count_zero(self, mock_db):
        """測試無未讀通知"""
        mock_result = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_get_unread_count_none_returns_zero(self, mock_db):
    ...
    @pytest.mark.asyncio
    async def test_get_unread_count_db_error(self, mock_db):
        """測試資料庫錯誤時返回 0"""
...
class TestMarkAsRead:
    ...
    @pytest.mark.asyncio
    async def test_mark_as_read_success(self, mock_db):
        """測試成功標記通知為已讀"""
        mock_result = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_mark_as_read_empty_ids(self, mock_db):
        """測試空 ID 列表返回 0"""
        result = await NotificationService.mark_as_read(mock_db, [], user_id=1)

        assert result == 0
        mock_db.execute.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_mark_as_read_db_error(self, mock_db):
        """測試資料庫錯誤時返回 0 並回滾"""
        mock_db.execute.side_effect = Exception("DB Error")
...
# ============================================================
# mark_all_as_read 測試 (靜態方法)
# ============================================================

class TestMarkAllAsRead:
    """mark_all_as_read 方法測試"""

    @pytest.mark.asyncio
    async def test_mark_all_as_read_success(self, mock_db):
        """測試成功標記所有通知為已讀"""
        mock_result = MagicMock()
        mock_result.rowcount = 10
        mock_db.execute.return_value = mock_result

        result = await NotificationService.mark_all_as_read(
            mock_db, user_id=1
        )
    ...
    @pytest.mark.asyncio
    async def test_mark_all_as_read_no_unread(self, mock_db):
        """測試無未讀通知時返回 0"""
        mock_result = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_mark_all_as_read_db_error(self, mock_db):
        """測試資料庫錯誤時返回 0 並回滾"""
        mock_db.execute.side_effect = Exception("DB Error")
...
# ============================================================
# get_notifications 測試 (靜態方法)
# ============================================================

class TestGetNotifications:
    """get_notifications 方法測試"""

    @pytest.mark.asyncio
    async def test_get_notifications_empty(self, mock_db):
        """測試查詢通知 - 空結果"""
        # Mock total count
    ...
    async def test_get_notifications_with_data(
        self, mock_db, mock_notification
    ):
        """測試查詢通知 - 有資料"""
        mock_total_result = MagicMock()
        ...
        assert result["items"][0]["title"] == "測試通知"
        assert result["items"][0]["type"] == NotificationType.SYSTEM

    @pytest.mark.asyncio
    async def test_get_notifications_db_error(self, mock_db):
        """測試查詢通知 - 資料庫錯誤時返回空結構"""
        mock_db.execute.side_effect = Exception("DB Error")
