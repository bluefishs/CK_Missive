"""
VendorServiceV2 - 廠商服務（工廠模式）

使用工廠模式的新版服務，db session 在建構函數注入。
這是 VendorService 的新版本，為 v2.0 遷移目標。
...
使用方式:
    ...
    async def list_vendors(
        service: VendorServiceV2 = Depends(get_service_with_db(VendorServiceV2))
    ):
        return await service.get_list()

    # 手動建立
    async def some_function(db: AsyncSession):
        service = VendorServiceV2(db)
