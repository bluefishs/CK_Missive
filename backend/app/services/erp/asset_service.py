...
class AssetService(AuditableServiceMixin):
    ...
    async def get_stats(self, keyword: Optional[str] = None, category: Optional[str] = None,
                        case_code: Optional[str] = None) -> Dict[str, Any]:
        """取得資產統計（與列表同一份篩選；status 是卡片自己，不收）"""
        return await self.repo.get_asset_stats(keyword=keyword, category=category, case_code=case_code)
