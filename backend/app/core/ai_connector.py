...
#   · ⚠️ **不要用「省下 5 GB 記憶體」當理由** —— CK_Website 06:1x 自己撤回了那個推論：
#     `docker stats` 的 MemUsage 10.36 GiB 裡不可回收的匿名記憶體只有約 1.4 GB，其餘是模型權重的
#     mmap（可回收），而 `qwen2.5:7b` 是 100% GPU、對 VM 的 RAM 幾乎零成本。
#     **理由是「沒有效益」，不是「省記憶體」** —— 拿掉一個沒有效益的動作不需要先證明它有害。
#
...
class AIConnector(AIConnectorManagementMixin):
    ...
    # ensure_models, warmup_models, check_health — 繼承自 AIConnectorManagementMixin
