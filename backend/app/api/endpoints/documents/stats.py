...
@version 5.1.0
@date 2026-02-07
@updated 2026-03-23 — 趨勢/效率端點委派至 Service 層 (A2)
"""
...
async def get_document_trends(
    ...
    委派至 DocumentStatisticsService.get_trends()，
    底層查詢由 DocumentStatsRepository.get_monthly_trends() 執行。
    """
    try:
        trends = await service.get_trends(months=12)
...
async def get_document_efficiency(
    ...
    委派至 DocumentStatisticsService.get_efficiency()，
    底層查詢由 DocumentStatsRepository 執行。
    """
    try:
        result = await service.get_efficiency()
