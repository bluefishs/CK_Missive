...
from app.schemas.erp.case_filters import CaseListFilters  # 篩選條件的單一定義（09-09）
...
class VendorAccountListRequest(CaseListFilters):
    """廠商帳款列表查詢 —— 篩選條件繼承 `CaseListFilters`，本頁只多一個 vendor_type。"""
    vendor_type: str = Field(default="subcontractor", pattern=r"^(subcontractor|client)$")


class ClientAccountListRequest(CaseListFilters):
    """委託單位帳款列表查詢 —— 篩選條件全部來自 `CaseListFilters`。"""
