...
class NaturalSearchResponse(BaseModel):
    ...
    synonym_expanded: bool = Field(default=False, description="是否經過同義詞擴展")
    entity_expanded: bool = Field(default=False, description="是否經過知識圖譜實體擴展")
    expanded_keywords: Optional[List[str]] = Field(
        None, description="實體擴展後的關鍵字列表（僅在 entity_expanded=True 時有值）"
    )
    history_id: Optional[int] = Field(None, description="搜尋歷史 ID（用於回饋）")
    matched_entities: List[MatchedEntity] = Field(
        default=[], description="搜尋中匹配到的正規化實體（橋接圖譜）"
    )
    error: Optional[str] = Field(None, description="錯誤訊息")
