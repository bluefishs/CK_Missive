...
class TaoyuanProject(Base):
    ...
    road_length = Column(Numeric(15, 2), comment="道路長度(公尺)")
    current_width = Column(Numeric(15, 2), comment="現況路寬")
    planned_width = Column(Numeric(15, 2), comment="計畫路寬")
    ...
    construction_cost = Column(Numeric(15, 2), comment="工程費")
    land_cost = Column(Numeric(15, 2), comment="用地費")
    compensation_cost = Column(Numeric(15, 2), comment="補償費")
    total_cost = Column(Numeric(15, 2), comment="總經費")
...
class TaoyuanDispatchDocumentLink(Base):
    ...
    confidence = Column(String(20), nullable=True, comment="Match confidence: confirmed/high/medium/low")
    created_at = Column(DateTime, server_default=func.now())
...
class TaoyuanContractPayment(Base):
    ...
    work_01_amount = Column(Numeric(15, 2), comment="01.地上物查估-派工金額")
    work_02_date = Column(Date, comment="02.土地協議市價查估-派工日期")
    work_02_amount = Column(Numeric(15, 2), comment="02.土地協議市價查估-派工金額")
    work_03_date = Column(Date, comment="03.土地徵收市價查估-派工日期")
    work_03_amount = Column(Numeric(15, 2), comment="03.土地徵收市價查估-派工金額")
    work_04_date = Column(Date, comment="04.相關計畫書製作-派工日期")
    work_04_amount = Column(Numeric(15, 2), comment="04.相關計畫書製作-派工金額")
    work_05_date = Column(Date, comment="05.測量作業-派工日期")
    work_05_amount = Column(Numeric(15, 2), comment="05.測量作業-派工金額")
    work_06_date = Column(Date, comment="06.樁位測釘作業-派工日期")
    work_06_amount = Column(Numeric(15, 2), comment="06.樁位測釘作業-派工金額")
    work_07_date = Column(Date, comment="07.辦理教育訓練-派工日期")
    work_07_amount = Column(Numeric(15, 2), comment="07.辦理教育訓練-派工金額")

    # 彙總欄位
    current_amount = Column(Numeric(15, 2), comment="本次派工金額")
    cumulative_amount = Column(Numeric(15, 2), comment="累進派工金額")
    remaining_amount = Column(Numeric(15, 2), comment="剩餘金額")
