...
#:   同他們說的「把豁免當待驗斷言而不是開關」。
#:
#: ⚠️ **刻意不用「清單雜湊」，雖然 CK_AaaP 建議過。**
#:
#: 他們的 md5 白名單能運作，是因為他們斷言的是
#: 「**這一份 hook 是我讀過的那個舊副本**」—— 那是**內容相關**的斷言，
#: 內容一變，斷言當然要重問。
#:
#: 而我斷言的是「**他們有自己的機制**」—— 那是**數量相關**的。
#: 43 支全部改名成另外 43 支，我的斷言**仍然成立**（他們還是有機制）。
#: ⇒ 清單雜湊會在斷言仍為真時失效。
#:
#: 實測支持：`CK_lvrland_Webmap` 的 `scripts/checks/` **過去 30 天有 107 個
#: commit**（對照他們的 hook 檔 6 個）⇒ 清單雜湊平均**每 7 小時失效一次**，
#: 而每一次都要人重新分類。**那不是守門，是噪音產生器。**
#:
#: ⇒ **錨點要對應「被斷言的是什麼」**，不是照抄別人的錨點。
