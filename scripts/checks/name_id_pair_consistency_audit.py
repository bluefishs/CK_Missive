...
# (表, 名稱欄, 鍵欄, 主檔表, 主檔名稱欄, 額外條件)
PAIRS = [
    ("pm_cases", "client_name", "client_vendor_id", "partner_vendors", "vendor_name", "TRUE"),
    ("contract_projects", "client_agency", "client_vendor_id", "partner_vendors", "vendor_name", "TRUE"),
    ("erp_vendor_payables", "vendor_name", "vendor_id", "partner_vendors", "vendor_name", "TRUE"),
]

SQL = "SELECT json_build_object(" + ",".join(
    f"""'{t}', json_build_object(
      ...
      'drift', (SELECT json_agg(json_build_array(x.id, x.{n}, v.{mn})) FROM {t} x JOIN {mt} v ON v.id = x.{k}
          WHERE btrim(x.{n}) <> btrim(v.{mn}) AND {cond}),
