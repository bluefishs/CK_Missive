...
# 1. Services 頂層散戶比例
# ----------------------------------------------------------------------------
echo -e "${CYAN}[1/78] Services directory entropy${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[2/78] Dead config reader scan (multi-target v3)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[3/78] SOUL.md mirror drift check${NC}"
if $STRICT; then
...
# 4. Wiki ↔ KG 雙向引用率
# ----------------------------------------------------------------------------
echo -e "${CYAN}[4/78] Wiki ↔ KG link audit${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[5/78] KG pgvector embedding coverage${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[6/78] Architecture standard docs presence${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[7/78] Agent evolution health${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[8/78] Dispatch cache contract${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[9/78] Stub import lint (DDD boundary)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[10/78] Memory Wiki metrics alive${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[11/78] SOUL evolution alive${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[12/78] Memory signal consumer lint${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[13/78] Cron health check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[14/78] Wiki Unicode dup check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[15/78] Integration liveness check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[16/78] LINE notify 7d heartbeat${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[17/78] Alias RLS bidirectional check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[18/78] Memory Wiki freshness check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[19/78] Generic filter audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[20/78] Role permissions consistency${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[21/78] Alias RLS coverage audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[22/78] Domain score freshness${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[23/78] Capability usage audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[24/78] ADR lifecycle check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[25/78] Dead UI detector${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[26/78] Lessons drift check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[27/78] Service line count check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[28/78] Paths sloppy calc guard${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[29/78] Contracts only import guard${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[30/78] Module portability audit (contracts/)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[31/78] Naming convention audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[32/78] Facade only check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[33/78] Toolkit sync audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[34/78] Transitive deps audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[35/78] queryKey drift audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[36/78] autobiography freshness check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[37/78] cross-repo docker network audit (ADR-0043)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[38/78] docker compose volume consistency (L43)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[39/78] facade consumer audit (v6.10 P1)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[40/78] compose vs Dockerfile HEALTHCHECK SSOT (L45)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[41/78] cross-repo secret SSOT audit (L41)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[42/78] cross-repo SSO auth state audit (L44)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[43/78] DB schema drift audit (next_session_resume #1)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[44/78] container lifecycle audit (next_session_resume #4)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[45/78] subdomain registry audit (next_session_resume #3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[46/78] SSO autoload completeness audit (next_session_resume #7)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[47/78] startup dependency race audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[48/78] DB pool exhaustion audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[49/78] Synthetic baseline freshness audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[50/78] Frontend bundle size drift audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[51/78] Tender source freshness audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[52/78] container host dependency audit (L49)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[53/78] tender subscription scheduler watchdog (L48)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[54/78] powershell utf-8 bom audit (L49 #11)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[55/78] tender enrichment freshness (ADR-0046)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[56/78] tender review queue health (ADR-0046 task E)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[57/78] container env alignment audit (L51 LINE incident)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[58/78] agent query starvation check (L51.7 kunge review)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[59/78] diary density audit (L51.7 Sprint 2.P2.11)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[60/78] container image freshness check (L51 incident #8)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[61/78] facade adoption audit (P1.7 v6.12 路線)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[62/78] integration e2e validation (v6.13 整合連通 持續驗證)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[63/78] transaction pollution audit (L64 交易污染防復發)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[64/78] auth-state ssot audit (登入狀態單一權威, 防 SSO race 復發)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[65/78] uncommitted work audit (session 收尾完整性, 防寫好未接通)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[66/78] heterogeneous work audit (異質同工防增量, 防反覆回歸根源復發)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
# 導覽列 live 完整性（2026-07-31，site-management 模組自我檢測）
# 既有 route-sync-check 是「原始碼對原始碼」、navigation_validator 只擋新寫入，
# 兩者都不看 live DB —— 而導覽列是使用者唯一入口，DB 裡一筆死連結
# 靜態檢查全綠、使用者點了卻是 404。
# ----------------------------------------------------------------------------
