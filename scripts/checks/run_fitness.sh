...
# 1. Services 頂層散戶比例
# ----------------------------------------------------------------------------
echo -e "${CYAN}[1/77] Services directory entropy${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[2/77] Dead config reader scan (multi-target v3)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[3/77] SOUL.md mirror drift check${NC}"
if $STRICT; then
...
# 4. Wiki ↔ KG 雙向引用率
# ----------------------------------------------------------------------------
echo -e "${CYAN}[4/77] Wiki ↔ KG link audit${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[5/77] KG pgvector embedding coverage${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[6/77] Architecture standard docs presence${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[7/77] Agent evolution health${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[8/77] Dispatch cache contract${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[9/77] Stub import lint (DDD boundary)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[10/77] Memory Wiki metrics alive${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[11/77] SOUL evolution alive${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[12/77] Memory signal consumer lint${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[13/77] Cron health check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[14/77] Wiki Unicode dup check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[15/77] Integration liveness check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[16/77] LINE notify 7d heartbeat${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[17/77] Alias RLS bidirectional check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[18/77] Memory Wiki freshness check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[19/77] Generic filter audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[20/77] Role permissions consistency${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[21/77] Alias RLS coverage audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[22/77] Domain score freshness${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[23/77] Capability usage audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[24/77] ADR lifecycle check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[25/77] Dead UI detector${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[26/77] Lessons drift check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[27/77] Service line count check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[28/77] Paths sloppy calc guard${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[29/77] Contracts only import guard${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[30/77] Module portability audit (contracts/)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[31/77] Naming convention audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[32/77] Facade only check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[33/77] Toolkit sync audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[34/77] Transitive deps audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[35/77] queryKey drift audit${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[36/77] autobiography freshness check${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[37/77] cross-repo docker network audit (ADR-0043)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[38/77] docker compose volume consistency (L43)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[39/77] facade consumer audit (v6.10 P1)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[40/77] compose vs Dockerfile HEALTHCHECK SSOT (L45)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[41/77] cross-repo secret SSOT audit (L41)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[42/77] cross-repo SSO auth state audit (L44)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[43/77] DB schema drift audit (next_session_resume #1)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[44/77] container lifecycle audit (next_session_resume #4)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[45/77] subdomain registry audit (next_session_resume #3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[46/77] SSO autoload completeness audit (next_session_resume #7)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[47/77] startup dependency race audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[48/77] DB pool exhaustion audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[49/77] Synthetic baseline freshness audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[50/77] Frontend bundle size drift audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[51/77] Tender source freshness audit (v6.12 P3)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[52/77] container host dependency audit (L49)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[53/77] tender subscription scheduler watchdog (L48)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[54/77] powershell utf-8 bom audit (L49 #11)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[55/77] tender enrichment freshness (ADR-0046)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[56/77] tender review queue health (ADR-0046 task E)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[57/77] container env alignment audit (L51 LINE incident)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[58/77] agent query starvation check (L51.7 kunge review)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[59/77] diary density audit (L51.7 Sprint 2.P2.11)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[60/77] container image freshness check (L51 incident #8)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[61/77] facade adoption audit (P1.7 v6.12 路線)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[62/77] integration e2e validation (v6.13 整合連通 持續驗證)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[63/77] transaction pollution audit (L64 交易污染防復發)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[64/77] auth-state ssot audit (登入狀態單一權威, 防 SSO race 復發)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[65/77] uncommitted work audit (session 收尾完整性, 防寫好未接通)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[66/77] heterogeneous work audit (異質同工防增量, 防反覆回歸根源復發)${NC}"
if $STRICT; then
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[67/77] code semantic duplication (程式圖譜自動發現異質同工候選)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[68/77] code graph orphan audit (圖譜 stale orphan 偵測, DRY-RUN 只報不刪)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[69/77] producer output watchdog (沉默成功偵測, 行為層自我檢核)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[70/77] tier1 shared package version skew (ck-auth 跨 consumer 版本對齊 / L80)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[71/77] shared vendored drift (@ck-shared/* vendored copies vs canonical / L80)${NC}"
...
# ----------------------------------------------------------------------------
echo -e "${CYAN}[72/77] sso_bridge conformance (跨 repo 安全契約：用 ck_auth verify / 守衛順序 / L80)${NC}"
...
# ----------------------------------------------------------------------------
# CSRF Service Drift（2026-07-25 / L80 / Tier2）— pile↔lvrland csrf_service 單一源守門
# HH-1 真乾淨重複（189 行僅差 redis import）；drift audit 保證不偏移，避免 wheel 併入
# ck_auth 的主產品 blast radius（step 70 版本偏移）。graceful：sibling 不在則 skip
# ----------------------------------------------------------------------------
echo -e "${CYAN}[73/77] csrf_service drift (pile↔lvrland 單一源守門 / L80)${NC}"
...
# ----------------------------------------------------------------------------
# 承攬案件 case_code 橋接覆蓋率（2026-07-29）— owner 回報 /contract-cases/187
# 無法進行財務紀錄作業。case_code 只在「邀標→建案→成案」流程寫入，直接建立/歷史匯入
# 的案件永遠沒有，其財務分頁恆空且無告警 → 只看「進行中」案件避免歷史案噪音。
# ----------------------------------------------------------------------------
echo -e "${CYAN}[74/77] contract case_code coverage (進行中案件是否可做財務作業)${NC}"
...
# ----------------------------------------------------------------------------
# 登入後回跳 returnUrl 深連結護欄（2026-07-29）— 手機掃核銷 QR 進不去該案的根因：
# 認證入口頁寫死 navigate(ROUTES.DASHBOARD) 丟棄 returnUrl → 所有深連結（QR/Email/LINE）
# 在公網失效。負向測試已驗（修法前會標紅），非永久綠。
# ----------------------------------------------------------------------------
echo -e "${CYAN}[75/77] auth deeplink returnUrl (認證入口頁不得寫死登入後導向)${NC}"
node scripts/checks/auth_deeplink_returnurl_audit.cjs $($STRICT && echo --strict) || { $STRICT && FAIL_COUNT=$((FAIL_COUNT+1)); true; }
echo ""

# ----------------------------------------------------------------------------
# cron 外部執行檔依賴沉默跳過護欄（2026-07-30）— cf_tunnel_verify 因容器無 pwsh
# 而 logger.warning+return，cron 卻記 success → 公網監控實質失效數月（沉默成功家族）。
# 負向測試已驗（舊版模式會標紅），非永久綠。
# ----------------------------------------------------------------------------
echo -e "${CYAN}[76/77] cron external binary guard (依賴缺失須 raise 不得沉默跳過)${NC}"
...
# ----------------------------------------------------------------------------
# UI 檢核產出新鮮度（2026-07-31）— 前後端與頁面 UI 的管控缺口
# 既有 fitness 只驗原始碼與資料，端點探針只驗狀態碼，兩者都不會告訴你
# 「這一頁打開來是壞的」。UI 檢核（flow/sweep）由 Windows 排程每日跑，
# 此步驟只驗**它有沒有在跑、上次結果如何** —— 檢核器自己停跑也要被抓到。
# ----------------------------------------------------------------------------
echo -e "${CYAN}[77/77] UI 自我檢核產出新鮮度 (flow + page sweep)${NC}"
