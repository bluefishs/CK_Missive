...
<!--baseline:check_scripts-->合計 **234** 支（頂層 `*.py` + `*.sh`；子目錄 `.shared-selfaudit/` 由上游同步，不在表態閘門管轄內）。
...
| `finance_metrics_ssot_audit.py` ＋ `.finance_metrics_baseline.json` | **經費指標只能有一份實作** —— 09-09 owner：「統計應建構統一服務端，不應依各別頁面各自建構，避免反覆查核每個頁面是否來源或統計基準」。盤點實測：承攬金額 4 份、已請款／已收款／應付各 3 份，散在 3 個檔。**而且已經分歧**：「已收款」三份的狀態條件是 `IN ('paid','partial')`／`= 'paid'`／完全不看狀態三種不同的東西，**卻算出同一個數字**（23,072,409）——因為 59 筆有金額的請款狀態剛好全是 `paid`。那是最危險的狀態：看起來一致，出現第一筆 `partial` 就三個畫面三個數字而沒有一方報錯。唯一實作＝`app/services/erp/finance_metrics.py`（回 SQL 片段，讓既有查詢低成本遷移），口徑由 `test_finance_metrics.py` 15 條判準鎖守。存量走基線、**新增即紅**（一天全改不可能，而『第一天 20 個紅點』與沒有檢核是同一個下場）。⚠️ 判準首跑兩個誤報：①命中**我自己寫的註解**裡的 `case_code LIKE 'CK2026_%'`（今天第四次踩這個坑）⇒ 掃描前移除整行註解；②把 `case_profile.py` 正確的「year 欄優先、案號年後備」報成違規 ⇒ 加窗口判斷。正向控制：注入一份新實作→rc=2 並指名檔案行號、還原→rc=0 | weekly 132 |
