...
| **驗檢核腳本的測試，斷言有沒有錨定在「永遠會出現的字串」上** | **0 支**（4 支相關測試全部正確）。<br>形狀由 CK_AaaP 2026-08-30 提出：他們的測試斷言 `'不存在' in out`／`'不歸我們管' in out`，而那兩句命中的是**摘要標籤**（`Missing (repo 不存在或安裝失敗)`／`Custom (repo 自有 hook，不歸我們管)`）—— **與判定完全無關，且永遠出現** ⇒ 兩支測試第一版就是綠的假象。<br>⇒ 他們的修正比 L71 原版準確：**斷言要錨定到「只有判定成立時才會出現」的字串**。「用 AST」對 Python 有效，但對「比對一支 CLI 的輸出」沒有 AST 可用。<br>本 repo 沒有這個陷阱，而原因是**結構性**的：那 4 支測試斷言的是**函式回傳值**（`issues`／`reds` 清單）而不是整份 stdout（例：`any("State=Disabled" in r for r in reds)`）。<br>⇒ 判準：**能斷言回傳值就別斷言輸出** —— 輸出裡混著標題、摘要、說明，而回傳值只有判定本身。 |
