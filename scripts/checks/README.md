...
> **否定結論比肯定結論更需要證據** —— 它會替所有後來的人決定「不必再查」。
>
> ⚠️ **收窄判準之後得到的零，要再查一次。**（CK_AaaP 2026-08-30）
> 過寬給你一堆假陽性 —— **吵，但看得見**；
> 過窄給你一個乾淨的零 —— **安靜，且看起來像好消息**。
> 兩者的代價完全不對稱。
>
> 本表四筆「0」的自我複查（2026-08-30）：
> · 三筆的收窄是**逐項可核的排除**（環境變數不是 repo 名／那個步驟號指向別的腳本），
>   排除依據明確 ⇒ 不需要二度查證。
> · 一筆（斷言錨定）**做了二度查證**並更正了理由 —— 見該列的 ⚠️ 段。
>   二度查證時**又錯一次**（把 `scripts/checks/test_*.py` 這種
>   「檔名以 test_ 開頭的檢核腳本」收進測試檔清單）。
>   ⇒ **收集判準用檔名而不是位置**，是本 repo 反覆出現的形狀。

| 查了什麼 | 結果（2026-08-30） |
|---|---|
| **新唯一索引 `uq_ledger_source_nonmanual` 對既有正式資料會不會失敗**（2026-08-31）| **不會，而且它早就套用了。** 我連續多輪把它描述成「需要拿正式資料副本實跑 `alembic upgrade` 才能確認」——實際上一個唯讀 SELECT 就回答：`pg_indexes` 查得到該索引、`alembic_version` = `20260830a001`（五支全套用）、部分索引範圍內 81 筆、重複組 **0**。⇒ **我用「它需要什麼才能被驗證」代替了「它現在是什麼狀態」。** |
| **`set -e` 下 `VAR=$(可能失敗的命令)` 會不會讓腳本中止**（2026-08-31，跨 session CK_AaaP 提出）| **0 個真問題**（39 支 `.sh`、7 個候選、2 個需細看）。⚠️ **這個零依賴一個隨時會被改掉的前提**：該檔只有 `set -e` **沒有 `pipefail`** ⇒ 管線退出碼取自最後一個命令（`cut`／`head`，恆成功）；有人日後加上 `pipefail`（通常被視為更嚴謹的改動）那兩行就會變成真的 bug。⚠️ 另：此類**不宜做成靜態守門**——CK_AaaP 實測 2/2 假陽性，其中一個命中的是 `docker exec sh -c '…'` **字串裡**的 shell 程式碼。規則取決於語法位置，而語法位置需要解析器不是正則。 |
| **git hook 是不是別處正典的過期複本** | `pre-push` **是**（84 行 vs 正典 157，已自行同步，見待辦 A36）。其餘四支**不是**：`pre-commit`／`post-commit` 的「候選」是**組件**不是整支正典；`post-checkout`／`post-merge` 指向 monorepo 層的 `.claude/sync-skills.sh` —— **路徑正確、實跑成功**（我一度以為 `../..` 推導錯，驗了才知道那是刻意的）。 |
| **當日新增的 6 支稽核有沒有「宣告的執行者不存在」（L99 型）** | **0 支**。六支都真的被 `run_fitness_weekly.sh` 呼叫，步驟號 84–89 皆相符。 |
| **檔頭宣告的 weekly 步驟號與實際是否相符** | **12 支有寫、0 支不符**。⚠️ 首版判準掃「檔案裡任何一個 `weekly N`」，命中 7 支 —— **7 個全是誤報**：那些數字指的是**別的腳本**的步驟號（`spec_executor_audit` 的白名單、`pm2_process_liveness_audit` 的「誰在跑」表格另一列）。收窄成「只看檔頭『誰跑它』段落」後歸零。⇒ 同本日反覆出現的**判準命中的是散文不是程式碼**。 |
| **驗檢核腳本的測試，斷言有沒有錨定在「永遠會出現的字串」上** | **0 支**（4 支相關測試全部正確）。<br>形狀由 CK_AaaP 2026-08-30 提出：他們的測試斷言 `'不存在' in out`／`'不歸我們管' in out`，而那兩句命中的是**摘要標籤**（`Missing (repo 不存在或安裝失敗)`／`Custom (repo 自有 hook，不歸我們管)`）—— **與判定完全無關，且永遠出現** ⇒ 兩支測試第一版就是綠的假象。<br>⇒ 他們的修正比 L71 原版準確：**斷言要錨定到「只有判定成立時才會出現」的字串**。「用 AST」對 Python 有效，但對「比對一支 CLI 的輸出」沒有 AST 可用。<br>本 repo 沒有這個陷阱，而原因是**結構性**的。⚠️ **2026-08-30 二度查證**（因 CK_AaaP 指出「過窄會給出一個乾淨的零，而那安靜且看起來像好消息」）：用 AST 重掃 `backend/tests` 的 **309 個測試檔**，**跑外部 CLI 去斷言 stdout 的：0 支**。那 4 支相關測試是**直接 import 檢核模組、呼叫函式、斷言回傳值**（例：`from windows_task_liveness_audit import audit` 後 `any("State=Disabled" in r for r in reds)`）—— **連 stdout 都不碰**。<br>⇒ 判準：**能斷言回傳值就別斷言輸出** —— 輸出裡混著標題、摘要、說明，而回傳值只有判定本身。CK_AaaP 三次中招不是疏忽，是他們的被測物是 shell 腳本、**沒有回傳值可斷言**。<br>⚠️ 二度查證時我又錯了一次：首版把 `scripts/checks/test_*.py`（那是**檢核腳本本身**，只是檔名以 test_ 開頭）也收進測試檔清單，算出「CLI 類 2 支」。**收集判準用檔名而不是位置** —— 同一天的老毛病。 |
| **跨 repo 清單裡有沒有「幽靈條目」** | **0 個**（掃 276 個腳本）。唯一命中 `CK_Project` 是誤報 —— 那是 NAS 上的 `\\CKNAS\CK_Project\`，不是 monorepo 裡的 repo。<br>⚠️ 這個形狀由 CK_AaaP 2026-08-30 在自己身上查出：他們的 `REPOS` 清單有一筆 `hermes-agent`（**目錄不存在**，實際叫 `CK_Hermes`），於是安裝器每次把它算成 `Missing 1`，而**真正的 `CK_Hermes` 從來沒有被檢查過**。<br>⇒ **登記在錯名字下的東西，不會顯示為漂移，會顯示為「不存在」** ——而後者看起來像一個合理、不需要處置的狀態。<br>⇒ 判準：**清單類設定要驗「每一個條目都指向真實存在的東西」**，而不只是驗「存在的東西都在清單裡」。兩個方向要分開查。 |
| **會寫 baseline 的檢核有沒有第二個「寫出半份基準」的風險** | **0 個**。八支逐一核實：三支有 `--update`／一支有 `--write-baseline`／兩支寫的是報告與快照／兩支當日已處理。收穫在另一個方向：`http_method_convention_audit` 早有比我更強的守衛形式 —— **用領域知識定下限**（「740 個端點不可能一條 GET 都沒有」），它連「掃描壞了而摘要也一起壞了」都擋得住。 |
...
<!--baseline:check_scripts-->合計 **240** 支（頂層 `*.py` + `*.sh`；子目錄 `.shared-selfaudit/` 由上游同步，不在表態閘門管轄內）。
...
| `quotation_year_semantics_audit.py` | **`erp_quotations.year` 必須是「案件年度」不是「建單那年」**——年度篩選（`case_year.quotation_case_year_condition`）09-08 起以 year 欄為準，而它的正確性依賴 year 欄乾淨。判準＝案名的民國年（含「112至113」「114~115」「112年及113年」區間，兩端都抓）vs year 欄；`finance_anchor` 排除。09-08 回填 13 筆髒 year 後 GREEN，負向控制實測（改髒 → RED exit 2，還原 → GREEN）。**09-08 追加跨表比對**：同一案在 erp_quotations／contract_projects／pm_cases 各有一個 year，三者不一致 ⇒ 年度篩選在不同頁面得到不同總額（owner 回報 /erp/quotations 107,484,210 vs /erp/client-accounts 90,549,209.5，差 16,935,000 完全來自一筆 contract_projects.year 落後一年）。⚠️ 首版跨表分支只印 [RED] 卻沒改退出碼，負向控制抓到「印紅字回綠燈」後才修 | weekly 126 |
| `finance_anomaly_parity_audit.py` | **金流異常判準的方向一致性** —— 09-08 owner 從那 5 筆「已收 17,850 已付清、發票多開 18,690」要求「異常案件標註機制並增列篩選查詢，以利解除或處理」。做出來的 `app/services/erp/finance_anomaly.py` 與 weekly 104 問同一族問題但**粒度不同**（104 逐筆：這張發票 vs 它綁的請款；異常機制逐案：本案發票合計 vs 請款合計），兩者**刻意不相等** —— 一案分兩次開票互補時逐筆會紅而逐案不紅，發票沒綁 `billing_id` 時逐筆看不到而逐案看得到。⇒ 本支不比數量，只問**方向**：同一族一邊抓到 ≥1 而另一邊 0 ＝ 只改了一邊。兩邊都 0 或都 >0 都不判紅。負向控制 4/4（`--self-test`） | weekly 127 |
| `table_filter_key_wiring_audit.py` | **表頭篩選鍵有沒有人讀回去** —— 09-09 owner 問題 2：「若表格調整欄位呈現項目，那表標頭篩選排序機制是否又會失效或再次設定」。答案在此之前是**會**：後端分頁的一條篩選鏈有三處各自寫著它的名字（欄位 `key`／`filteredValue` 讀的參數名／`onChange` 裡手寫的字串）。`/erp/quotations` 的「年度」欄實例：key 是 `case_code`、值存 `params.year`、onChange 寫 `first('case_code')` ——**同一件事三個名字**，把年度獨立成 `dataIndex: 'year'` 這種正常調整就會讓漏斗點了沒反應，而 tsc 全綠、主控台安靜。正解＝`frontend/src/utils/tableFilters.ts` 的 `buildServerFilters`（一份宣告產生欄位片段與 onChange 解讀，欄位鍵與參數名打錯字都由 tsc 擋），用了它的欄位直接放行。⚠️ **自帶解析度下限**：首版用大括號配對切欄位，被 JSX 模板字串裡的 `${}` 打亂，三個檔案 2/3/5 個宣告只認出 1/2/1 個 —— **而它報 0 違規**；改用「兩個 `title:` 之間」當欄位範圍，認出率低於 80% 即判不可信（rc=2）而不是給綠燈。三個控制：注入斷鏈→紅、現況→綠、解析度不足→紅 | weekly 128 |
| `staff_alias_scope_parity_audit.py` | **同一個人的兩個帳號要問到同一個答案** —— 09-09 owner 從 `/erp/vendor-accounts` 回報「以王駿穠登入、篩選李昭德（3），卻顯示共 0 家、經費 0」。兩個數字**各自都對，來自兩份定義**：李昭德有 `id=11 staff_李昭德`（已停用，`canonical_user_id=19`，CK2025_01_03_001 的 380 萬掛在這裡）與 `id=19`（在用）；下拉 `assignable_staff` **有**展開 alias 算出 3，列表 `case_codes_of_user` **沒有**展開只拿到 2，選 2026 後一個不剩。**同一個檔案裡兩支函式對「這個人是誰」用了兩份定義**，而畫面只說「沒有資料」，讀起來像那位承辦今年沒案子。ADR-0025 身分合併家族。修後 4 家／380 萬，canonical 與 alias 兩個 id 答案一致。⚠️ **本支的判準自我更正過兩次**：①首版動態兩邊都用自己的展開 SQL ⇒ 永遠一致 ⇒ 永遠綠（改為靜態讀原始碼）；②靜態首版判「有沒有出現 `_ALIAS_GROUP`」，而**那支函式的 docstring 自己就寫著這個字**⇒ 注入回歸仍 rc=0（改判實際代入 SQL 的 `IN ({_ALIAS_GROUP})` 且須有 2 處）。正向控制：注入回歸→rc=2、現況→rc=0、還原→rc=0 | weekly 129 |
| `request_field_guess_audit.py` | **schema 宣告過的欄位不該用 `getattr` 去猜** —— 09-09 owner：「為何還有『猜』機制，是否對應架構不完善」。`getattr(params, 'staff_user_id', None)` 對 Pydantic 請求物件的意思是「有就給我，沒有就算了」，而請求物件的欄位是固定的 ⇒ **真正會發生的只有一種情況：有人改了 schema 的欄位名**，那一刻不報錯、靜默變 None、篩選悄悄失效，畫面顯示全部而使用者以為是篩過的。與同日兩個事故同型（表頭篩選鍵三處分家 weekly 128、承辦身分合併沒展開 weekly 129）。首跑抓到 13 處（報價單 6／帳款 2／PM 1／發票彙總 3／案件狀態 1），全部改成直接屬性存取——改名會當場 AttributeError。⚠️ 型別靠函式簽章回推，**推不出來就不判**（寧可漏報也不誤報：誤報會讓人把多型物件上合法的防禦寫法拆掉）。正向控制：注入一處→rc=2 並指名、還原→rc=0 | weekly 130 |
| `list_stat_scope_parity_audit.py` | **同一個畫面上，列表篩了誰的案，統計卡就要算誰的案** —— 09-09 owner 兩次回報同一形狀：早上「`/contract-cases` 已配合角色與登入帳號篩選清單，但統計圖卡未同步」（承辦 6 案的業務同仁看到卡片 123 件）、中午「PM 案件列表與統計兩者無對應，是不合理的統計數據」。⚠️ **首版用靜態掃描，當場誤報**：端點函式本體看不到 `staff_user_id`（它宣告在 schema，端點只把 params 整包傳下去）⇒ 掃字樣得到「PM 列表沒有身分訊號」，而實際上它有。**改成行為判準**：拿同一個承辦身分、同一組參數分別打列表與統計，比數字——數字才是使用者看到的東西。找不到「非管理員且有案」的樣本時**不判綠燈**（零樣本的綠燈是假綠）。須在容器內跑（要 import 應用程式碼）。正向控制：把 PM 統計的身分參數改成 None → 86 vs 118 並 rc=2、還原 → rc=0 | weekly 131 |
| `list_stats_filter_parity_audit.py` | **統計卡是列表的分母，分母的參數集合必須涵蓋列表的篩選參數集合** —— 09-09 owner：選承辦「邱元宏」＋02 類，列表 85 張、卡片 21,493,663（全公司 112 張）；`/erp/client-accounts` 同條件 5,057,835。真因＝列表 schema 有 `staff_user_id`、損益摘要 schema 沒有。既有兩支看不到這個形狀：weekly 108 看前端有沒有接、131 比身分範圍；都對，是**使用者自選的篩選**沒跟上。判準＝從 runtime `app.routes` 取每組（`/list`, 同前綴 `statistics|stats|summary|totals|overview|profit-summary|grouped-summary`）的請求 model，`缺 = 列表欄位 − 分頁排序 − 列表 schema 的 STATS_EXEMPT − 統計欄位`，缺即 RED。豁免寫在列表 schema 上（與欄位同處），唯一正當理由＝「那個欄位是卡片自己」。⚠️ 首版假綠：FastAPI 0.135 的 ModelField 沒有 `type_`，13 組全讀成 None→None 而印 GREEN ⇒ 改讀 `field_info.annotation`，並加「全部讀不到就不下結論」。首跑 12/13 RED（`/erp/assets`／`/erp/operational`／`/erp/expenses` 的統計端點連一個篩選都不收），存量走 `.list_stats_filter_baseline.json` 逐一清 | weekly 133 |
| `billing_placeholder_consistency_audit.py` | **請款佔位與下游紀錄的一致性** —— 09-09「已請款＝有請款日期」口徑改了之後，任何「下游已發生、佔位還沒成立」的紀錄都是兩張表各說各話。owner 先看到 `/erp/quotations/793`「發票 78,960｜請款 0」（總表匯入的發票掛在佔位上），再要求「統一複查，不要重複人工檢核」。七條規則全在 DB 量、逐筆列 id：R1 有發票無有日期請款／R2 無日期卻有收款／R3 帳本入帳對到佔位／R5 多筆佔位 ⇒ RED；R4 佔位金額≠承攬額／R7 成案無任何請款 ⇒ YELLOW；R6 只列數字。承攬金額片段從 `stats/finance` 拿。機制修法＝`billing_service.settle_placeholder_for_invoice`（發票落地即補佔位日期，發票建立與總表匯入共用）；首跑 R1=3（788／793／794）為存量，待 owner 補日期 | weekly 134 |
| `case_filter_bar_adoption_audit.py` ＋ `.case_filter_bar_baseline.json` | **列表頁不得自畫案件維度下拉** —— 09-09 晚 `components/erp/CaseFilterBar` 成為年度／類別／承辦／委託單位／異常的唯一畫法（此前三個財務分頁各畫一份，年度選項三份、其中一份沒有「全部年度」）。靜態掃 `pages/**/*.tsx` 的 placeholder 字樣與自養年度常數（先去註解）；存量走基線、新增即紅；另印採用頁數看趨勢。⚠️ 字樣判準，改 placeholder 就躲得掉——所以同時印採用數 | weekly 135 |
| `finance_metrics_ssot_audit.py` ＋ `.finance_metrics_baseline.json` | **經費指標只能有一份實作** —— 09-09 owner：「統計應建構統一服務端，不應依各別頁面各自建構，避免反覆查核每個頁面是否來源或統計基準」。盤點實測：承攬金額 4 份、已請款／已收款／應付各 3 份，散在 3 個檔。**而且已經分歧**：「已收款」三份的狀態條件是 `IN ('paid','partial')`／`= 'paid'`／完全不看狀態三種不同的東西，**卻算出同一個數字**（23,072,409）——因為 59 筆有金額的請款狀態剛好全是 `paid`。那是最危險的狀態：看起來一致，出現第一筆 `partial` 就三個畫面三個數字而沒有一方報錯。唯一實作＝`app/services/erp/finance_metrics.py`（回 SQL 片段，讓既有查詢低成本遷移），口徑由 `test_finance_metrics.py` 15 條判準鎖守。存量走基線、**新增即紅**（一天全改不可能，而『第一天 20 個紅點』與沒有檢核是同一個下場）。⚠️ 判準首跑兩個誤報：①命中**我自己寫的註解**裡的 `case_code LIKE 'CK2026_%'`（今天第四次踩這個坑）⇒ 掃描前移除整行註解；②把 `case_profile.py` 正確的「year 欄優先、案號年後備」報成違規 ⇒ 加窗口判斷。正向控制：注入一份新實作→rc=2 並指名檔案行號、還原→rc=0。**09-09 擴大兩次**：①身分判定（`core/case_scope.has_company_wide_scope` 之外的 `is_superuser or is_admin`／`COMPANY_WIDE_ROLES`）；②案件類篩選 schema（`*ListRequest／*ListQuery` 自行宣告 `staff_user_id` 而未繼承 `schemas/erp/case_filters.CaseListFilters`）——首跑抓到承攬案／PM／報價單三份存量，簽基線待方案 §7.4 步驟②遷移。家因此有三個：金額指標、身分判定、篩選 schema | weekly 132 |
| `weekly_summary_rollup.py` ＋ `weekly_step_taxonomy.json` | **weekly 的統整與歸納** —— 09-09 owner：「weekly 要統整與歸納」。131 步的輸出是逐步流水帳：09-09 那次 127 步裡 **44 支非綠**（rc=1 25、rc=2 19）攤在 127 行裡，看不出哪些重要。更關鍵的是趨勢：08-30 的 94 步非綠 15 → 09-06 的 111 步非綠 38 → 09-09 的 127 步非綠 44，**三週加了 33 步、非綠增加 29 ⇒ 新檢核幾乎一上線就紅，而紅燈沒有人在清**（本 repo 記過「永遠是紅的訊號與沒有訊號是同一個下場」）。⇒ 輸出三段：①按領域（10 個領域的綠／黃／紅與健康度）②按壽命（**本週新紅**＝真的變壞了；**從來沒綠過**＝永久紅燈存量，09-09 有 10 支）③分類覆蓋。**判紅的只有「本週新紅」與「未分類步號」** —— 存量紅燈刻意不判，每週報同一份清單只會讓人不再看它。⚠️ 兩個自我更正：①首版對只有 2 支紀錄的領域印出「健康度 0%」，**那是樣本不足不是很糟**，會讓人去修沒壞的東西 ⇒ <3 支不計健康度；②首次正向控制時「未分類」與「本週新紅」同時成立、退出碼都是 2，**分不出哪個判準在作用** ⇒ 抽出純函式加 `--self-test`（四情境：全綠／新紅／存量紅不算新紅／未登記步號），4/4 通過。接在 runner 尾段（歷史寫入之後）| weekly 收尾 |
