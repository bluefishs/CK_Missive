...
VITALS = [
    ...
    },
    {
        ...
        "level": "YELLOW",
        "min": 1,
        # 2026-08-27 已把這兩種可能分辨開（免得下一個人再查一次）：
        #   · `erp_quotation_items` **全表 0 筆**（不是今日 0，SQL 是 COUNT(*)）
        #   · 而端點是通的 —— 實測 `/erp/quotation-items/detail` 回 **200**、
        #     `/replace` 對空 body 回 **422**（是驗證失敗，不是 404）
        #   · 前端也確實把編輯器嵌在 PM 案件的報價單分頁
        #   ⇒ 目前是「**還沒開始用**」，不是入口不通。
        #
        # ⚠️ 入口本身是 2026-08-27 才收完整的（輸出移入 PM 案件頁、
        #    建立完回到該案的報價單分頁 —— 先前存檔後會被丟回 ERP 列表）。
        #    所以在那之前的零使用，有一部分是入口的責任。
        #    **若入口完成後仍長期為 0，那才要當成入口問題重查。**
        "why_exempt": "報價明細是使用者逐案填的，不是系統產生 ⇒ 零使用代表"
                      "入口不通或還沒開始用（2026-08-27 實測端點可用 ⇒ 屬後者），"
                      "兩者都要有人看見，但都不是故障",
    },
    {
        "module": "專案資金",
