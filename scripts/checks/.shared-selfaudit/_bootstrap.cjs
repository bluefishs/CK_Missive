  ...
  if (cookieHeader) {
    const cookies = cookieHeader.split(';').map((kv) => {
      ...
      return {
        ...
        path: '/',
        // 2026-08-08：刻意**不設** httpOnly。
        // 一度全部設成 httpOnly:true（誤以為是 pile 走查失敗的原因），
        // 結果 Missive 走查由 17 PASS 掉成 1 PASS/16 SKIP ——
        // csrf_token 必須能被 JS 讀取才送得出 X-CSRF-Token，強制 httpOnly 直接讓認證失效。
        // pile 的真因其實是呼叫端把環境變數傳丟了，與 cookie 屬性無關。
        // 教訓：對「所有 cookie」一律套用同一組屬性是錯的 —— 有些是刻意可讀的。
      };
    }).filter((c) => c.name);
