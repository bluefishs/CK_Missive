...
# 2026-09-01：容器重啟迴圈。
#
# owner 遇到間歇 502，而**公網探針、容器 healthcheck、blackbox 全都是綠的** ——
# 因為它是「反覆重啟後恢復」，不是持續中斷。綠燈之間的空窗才是使用者踩到的。
#
# ⚠️ 發現它的是**別的 repo 的 session**（CK_AaaP 從告警 sink 看到我們有 10 筆
# 未恢復告警、最早 13.5 小時前）。也就是說：這件事已經叫了 13.5 小時，
# 而本 repo 自己的檢核一支都沒有在看。
# 他們的結論值得抄過來：**偵測 → 投遞 → 消費是三層，而大家只守了前兩層。**
# 這支補的是本 repo 自己的第三層 —— 不依賴別人來告訴我們自己的容器在重啟。
#
# ⚠️ 判準會分辨「容器被重建（部署，計數歸零）」與「沒有重啟」——
# 不分辨的話，每次部署後都會報一次假的好消息，而真的迴圈開始時基準已被洗掉。
