#!/bin/bash
# ============================================================
# Fitness Tier 2 Weekly — 步數由 TOTAL_STEPS 自動計算（2026-09-09 實際 131 步）
#
# ⚠️ 這一行原本寫死「12 trend tracking step (~3 min)」，而實際已經 131 步 ——
#    **過期十倍**，且沒有任何人會發現，因為沒有人拿它跟現實比對。
#    現在不寫死數字：步數看 TOTAL_STEPS（`grep -c run_step`），
#    每次跑完的統整看最後一段 `weekly_summary_rollup.py`。
#
...
run_step "134" "請款佔位與下游一致性（發票／收款／帳本對到無日期佔位即紅；成案漏建佔位黃）" "scripts/checks/billing_placeholder_consistency_audit.py"
run_step "135" "列表頁不得自畫案件維度下拉（CaseFilterBar 是唯一畫法；存量走基線）" "scripts/checks/case_filter_bar_adoption_audit.py"
...
# ------------------------------------------------------------------
# 統整與歸納（owner 2026-09-09：「weekly 要統整與歸納」）
# ------------------------------------------------------------------
# 131 步的逐步流水帳，人看不出哪些重要。這一段把結果按領域、按壽命歸納，
# 並且**只把「本週新紅」當成本週的事故** —— 存量紅燈每週報一次同樣的清單，
# 只會讓人不再看它（本 repo 記過：永遠是紅的訊號與沒有訊號是同一個下場）。
#
# ⚠️ 它讀的是**這次執行剛寫進去的**逐步歷史，所以必須放在歷史寫入之後。
echo ""
python scripts/checks/weekly_summary_rollup.py || true
