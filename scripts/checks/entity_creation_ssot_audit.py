...
MANAGED = {
    "PMCase": {
        "allow": [
            ...
            # ③ **歷史批次匯入**（2026-08-27 加）—— 既有 XLS 彙整表補建。
            #    這是不同業務場景，不是繞過規則：
            #      · 案號**來自彙整表**（`B114-B003-0`），不能用 generate_case_code()
            #        產新號 —— 08-20 第一版就是那樣做的，結果 180 筆全拿到同一個號
            #      · 5 道查重是為「使用者手動建重複案」設計的；這裡是把**已經
            #        存在於現實世界**的紀錄搬進系統，查重的語意不同
            #      · 委託單位刻意**不自動建**（會把簡稱與筆誤變成新廠商）
            #
            #    ⚠️ **但不要把這條 allow 讀成「這支匯入沒問題」** ——
            #    它只比對完整 case_code，2026-08-20 因此靜靜建出 26 件分身
            #    （既有 `B114-B003`、彙整表帶 `B114-B003-0`）。
            #    修法是讓它出聲（commit `6a53c3af`：匯入前預覽列出
            #    「同 base 且案名完全相同、只差版次」的候選），不是改建立路徑。
            #    存量偵測見 `case_award_pipeline_audit.py`（weekly 77）。
            "services/erp/quotation_legacy_import.py",
        ],
    ...
    "ERPQuotation": {
        "allow": [
            # ① 成案時（`total_price` 取自 PM 案件的合約金額）
            "services/contract/case_code.py",
            # ② **建案時**（2026-08-17 owner：「邀標報價 新增報價單＋一鍵成案
            #    統一案件編號」）—— 報價是投標**前**的動作，報價單不該等到
            #    得標後才出生，否則邀標階段沒有東西可以填。
            #
            #    ⚠️ 這條 allow 是 2026-08-18 加的，因為**原本的規則文字過時了**，
            #    不是因為程式碼違規。原規則寫「邀標階段不得建立」，
            #    而 owner 的流程是邀標階段就要有報價單。
            #
            #    原本那條規則要防的是「案件一出生就是成本 0、毛利率 100%」。
            #    那個顧慮**用另一種方式解決了**：建立時 `total_price=None`
            #    而不是 0 —— None 讓 `cost_declared` 為 false，
            #    詳情頁顯示「尚未填寫成本，無法計算毛利」而不是一個假的 100%。
            #    （0 與 None 在這裡是兩件事：0 是「真的免費」，None 是「還沒報價」。）
            "services/tender/case_creation.py",
            # ③ 歷史批次匯入 —— 理由同 PMCase 那一條（案號來自彙整表、
            #    查重語意不同）。⚠️ 同樣不代表那支匯入沒問題，見上方註解。
            "services/erp/quotation_legacy_import.py",
        ],
        "why": "成案時（金額由 PM 帶入）或建案時（draft、total_price=None）；"
               "其餘路徑不得自行建構",
    },
}
