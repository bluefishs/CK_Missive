# CK_Missive Wiki

## 📚 快速開始
- [首頁](Home)
- [開發環境設定](../DEVELOPMENT_GUIDE.md)

## 🔧 後端 API
- [API 總覽](Backend-API-Overview)
- [公文管理 API](Backend-API-Documents)
- [專案管理 API](Backend-API-Projects)
- [廠商管理 API](Backend-API-Vendors)
- [機關單位 API](Backend-API-Agencies)
- [行事曆 API](Backend-API-Calendar)

## 🖥️ 前端開發
- [組件文檔](Frontend-Components)
- [頁面路由](Frontend-Routes)
- [狀態管理](Frontend-State)
- [API 整合](Frontend-API)

## 💾 資料庫
- [資料模型](Database-Models)
- [資料庫結構](../DATABASE_SCHEMA.md)
- [遷移指南](Database-Migrations)

## 🏗️ 系統架構
- [架構概述](Architecture-Overview)
- [部署指南](Deployment-Guide)
- [Docker 配置](Docker-Config)

## 📖 開發指南
- [編碼規範](Coding-Standards)
- [Git 工作流程](Git-Workflow)
- [測試指南](Testing-Guide)
- [貢獻指南](../CONTRIBUTING.md)

## 🔗 相關資源
- [API 文檔 (Swagger)](http://localhost:8001/api/docs)
- [前端開發伺服器](http://localhost:3000)
- [Adminer 資料庫管理](http://localhost:8080)
