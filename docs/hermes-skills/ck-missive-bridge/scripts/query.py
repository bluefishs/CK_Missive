...
# [2026-05-30] 30→90s：agent_query 走 full RAG+tool agent，重查詢（search_documents）
# 實測達 42s，30s 會 timeout。輕查詢仍秒回，加大上限不影響快路徑。
TIMEOUT_S = 90
...
ACTION_HANDLERS: dict[str, dict[str, Any]] = {
    ...
    "agent_query": {
        "method": "POST",
        # [2026-05-30] 修正：實際端點是 /api/ai/agent/query（query_sync 會 404）。
        # 走 X-Service-Token（M2M，require_scope read:agent 比對 MCP_SERVICE_TOKEN）。
        # 已實測 HTTP 200。Bearer 路徑(require_auth 使用者 JWT)不適用 M2M。
        "path": "/api/ai/agent/query",
        "body_required": True,
        "auth_required": True,
        "auth_style": "service_token",
        "expected_args": ["question"],
    },
    # [2026-06-03 S3 段 B 預寫] Meta 跨平臺統整管道 — 讀坤哥 Memory Wiki 成長摘要。
    # 對應 ADR-CK-003 §5/§7-S3 + s3-meta-federation-briefing-design.md 段 B。
    # ⚠️ 依賴段 A（CK_Missive 開 GET /api/ai/memory/digest）；段 A 未完成前打此 action
    #    預期回 http_error 404（後端尚無此 route）—— 屬「機制就緒、等後端」的預期狀態，非 bug。
    # 由 cron/awakening collector 呼叫（段 C），不寫進 SOUL 強制規則（避 S1 prose 反傷 dispatch）。
    # since/limit 為選配 query param（預設後端取近 7 日）。
    "memory_digest": {
        "method": "GET",
        "path": "/api/ai/memory/digest",
        "body_required": False,
        "auth_required": True,
        "auth_style": "service_token",
        "expected_args": [],          # since / limit 皆選配
        "query_args": ["since", "limit"],  # GET → 附為 query string（若提供）
    },
}
...
def main(argv: list[str]) -> int:
    ...
    # GET query string：若 handler 宣告 query_args，把已提供者附為 ?k=v（選配，缺省略）
    query_keys = handler.get("query_args", [])
    if query_keys:
        qs = {k: args[k] for k in query_keys if k in args}
        if qs:
    ...
    if handler.get("auth_required"):
        ...
        # auth_style: "service_token" → X-Service-Token（M2M，後端 require_scope 比對
        # MCP_SERVICE_TOKEN）；預設 → Authorization: Bearer（require_auth 使用者 JWT）。
        if handler.get("auth_style") == "service_token":
