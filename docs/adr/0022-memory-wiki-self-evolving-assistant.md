...
| 6 | Observability | `memory_wiki_metrics.py` 99L 暴露 7 Gauge + 3 Counter 到 `/metrics`；diary/pattern_extractor/crystal_applier 三處 best-effort hook |
...
3. **SOUL.md 改為 DB 表**：會讓 user 失去「直接編輯人格」的能力；markdown +
   ...
   協作空間。
**2026-04-20 覆盤補記**：上線後深度驗證揭露四層 silent failure 連鎖
（見 CHANGELOG v5.7.1 與 `session_20260420_summary.md`）。核心教訓：
自我進化系統的 silent failure 會傳染——單一 datetime bug 會讓無關的
`save_trace` 報 session 錯亂。因此 `regression test` + 源碼掃描不可省略；
所有與 DB 狀態變化相關的 repository 方法，寫入路徑必須有 column 型別匹配
檢查（本 ADR 已補 11+7+2 regression tests）。

4. ~~**Phase 6 Observability（deferred）**~~：原評估為後續 ADR，但實際發現是
   Memory Wiki 正確運作的必要防線（助理若突然停寫日記 = Agent 閉環斷了，
   pending proposals 積壓 > 10 = user 未審），遂於同一 session 直接交付：
   - Gauges：diary_days / patterns / failures / crystals / proposals_{total,pending} / autobiographies
   - Counters：diary_appends / pattern_extract_runs{status} / crystal_applied
   - Hook 點：diary `append_entry` / pattern_extractor `extract_daily` /
     crystal_applier `apply_proposal` 各 best-effort inc，try/except 不 raise
   - Endpoint `/metrics` 活體暴露，與既有 KG / DB pool / inference 指標並列
