> ⛔ **已作廢（2026-09-08）**：它描述的衝突檔（`simple_main.py`／`optimized_main.py`＝所謂 8003「優化版」）早已不存在；它也正是「版本管理只有宣告沒有機制」的那份宣告。現行機制＝weekly 122（活文件不得描述不存在的基礎設施）＋ `docs/archived/` 為唯一歸檔位置。
