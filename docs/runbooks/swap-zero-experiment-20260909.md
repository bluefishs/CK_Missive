...
- ⚠️ **探針靜默不可判定**：同一小時 12 次段錯誤、5 個容器重啟，AaaP／Website 三支 15–30 分鐘的探針全部回報正常（故障秒級、policy=always 約 1 秒拉回）；看到 502 的是視覺走查那一張截圖。⇒ 本 runbook 任何靠「**站台探針**沒告警」支持的結論一律標不可判定。**容器重啟的共因偵測由 CK_Website 的 `check-restart-resilience` 艦隊訊號負責（15 分鐘取樣 `RestartCount`，累積計數器不會漏掉重啟；判準「≥3 個容器各增加 ≥2 次」，本波正確判 `fleet=5/18`），本 repo 不另建**；查詢方式＝跑該腳本看 `fleet` 欄。它的缺口是摘要行只投影成一個字 `resilience=DEGRADED`（已連續 776 輪因無關的備份退出碼而紅），由 CK_Website 補獨立欄位。
