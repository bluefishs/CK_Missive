...
## 0. ✅ **已解除（2026-08-31 20:42 部署完成）**

> owner 把部署提前到 20:40，已執行完畢：**`rc=0`，runtime = `v6.69 @ fb46bdfd`**。
> 複驗：容器與映像**完全一致**（原本 25 個檔只在容器裡，現在 0 個）
> ⇒ **現在怎麼重啟都安全**，`--force-recreate`／`down` 也不會失去任何修法。
>
> | 部署後複驗 | 結果 |
> |---|---|
> | 資料基線（6 張表） | **零筆增減**（documents 2,040／KG 50,166／pm_cases 253／專案 175／報價單 257／帳款 63） |
> | 四層驗證 | host:8001 200／公網首頁 200／公網 `/api/health` 200／`/api/auth/check` 401（**會走 ORM 與認證鏈**） |
> | 公網抽樣 | 三次皆 200（單次不足以證明埠轉發，L76） |
> | 知識庫向量檢索 | 回 3 筆（修法前**每一次查詢都 500**） |
> | 報價單 `CK2026_GN_02_001`（#541） | 承辦＝**洪慶忠**（owner 回報看不到的那張） |
> | 專案團隊查詢 | 175 個專案中 **160 個查得到成員**（修法前是 **0**） |
> | `?sort_by=metadata` | 安全退回預設（修法前 500） |
> | `/pm/cases` 列管範圍 | 253 → **117**（移交 136 筆給 `/contract-cases`） |
> | KB 向量同步時刻 | **05:15**（在知識地圖重生 04:51 之後；原 04:45 在它之前 6 分鐘） |
>
> 以下內容保留作為**當時的現場紀錄**與症狀對照，不再是待辦。

---

## 0-舊. ⛔ 部署前的狀態：**修法只在容器裡，映像沒有**
