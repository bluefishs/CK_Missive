...
## 5.5 跨系統備份現況（2026-08-11 全 portfolio 盤點）

⚠️ **本 runbook 的 §1–§5 只涵蓋 CK_Missive。** 2026-08-11 把同樣的問題拿去問其他系統，
答案差很多 —— 而每一個都有「看起來在運作」的外觀。

| 系統 | 本機備份 | NAS 異地 | 排程 | 能否復建 |
|---|---|---|---|---|
| **CK_Missive** | ✅ 每日 | ✅ 四類齊全（DB／附件／金鑰／里程碑） | ✅ 03:00 | ✅ 已實測（空白容器從零還原） |
| CK_PileMgmt | ✅ | ✅ 10h 內 | ✅ 02:30 | 🟡 素材齊，**未實測還原** |
| CK_DigitalTunnel | ✅ | ✅ 增量正常（4057 檔） | ✅ 03:30 | 🟡 同上 |
| **CK_lvrland_Webmap** | ✅ 每日 01:00（170 檔 3.9GB） | 🔴 **停在 2026-03-09** | 🔴 **從未註冊** | ❌ **3.9GB 只在 D 槽一顆磁碟** |
| **FT_StorageTank** | 🔴 停在 06-30 | 🔴 停在 06-30 | 🔴 無 | ❌ 連本機都停了 |
| **CK_Website（KV）** | — | 🔴 **最後有效 07-18** | ✅ PM2 04:30 但失敗 | ❌ 見下 |

### lvrland：腳本寫好了，但沒有人跑它

`CK_lvrland_Webmap/scripts/sync_backups_to_nas.ps1`（2026-05-20 建立）存在且看起來完整，
但 `Setup-AutoBackup.ps1` 要註冊的 `LandValuation_Daily_Backup` **在工作排程器裡不存在**。
NAS 上那 145 個檔案全是 03-09 之前的 —— 也就是**腳本建立後從未成功同步過任何一次**。

兩個放大它的因素：

1. 該腳本的 `$RemotePath` 用 **`Z:\` 對映磁碟機**。Missive 在 07-02 已經學過這一課
   （排程／服務情境下對映磁碟機不一定掛得上，要用 UNC `\\CKNAS\...`）——
   **有正確範例卻沒擴散**，同型第 N 次。
2. 排程存活稽核（`windows_task_liveness_audit`，08-05 建）只掃 **`CK*` 前綴**的任務名，
   而這支叫 `LandValuation_Daily_Backup` → **即使註冊了也不在雷達上**。

而 lvrland 是**公網上線的主系統**（土地查估）。

### CK_Website KV：22 份備份每天產生，內容是空的

| 日期 | 檔案大小 | `stats` |
|---|---|---|
| ~07-18 | 101 KB | `total_keys: 154, errors: 0` ← 真備份 |
| 07-19 起 | 4.3 KB | `total_keys: 0, errors: 5` ← **空殼** |

根因是 PM2 非互動環境沒有 `CLOUDFLARE_API_TOKEN`，wrangler 五個 namespace 全讀不到。
**資訊一直都寫在檔案裡**（`errors: 5`），只是沒有任何人在讀那個欄位；
檔名日期照跑、輪替照跑，於是「每天都有新備份」這個外觀完全成立。

KV 存的是 `CK_EMPLOYEES`（**四個子系統 SSO 的身分來源**）／`CK_LOGIN_AUDIT`／`CK_DASHBOARDS` 等。
它在 Cloudflare 雲端，所以**不受本機損毀影響**——它防的是誤刪與竄改，而那道防線目前退回到 07-18。

⚠️ 輪替是有時限的風險：07-10 之前的已被空殼擠掉，僅存的有效備份也會被擠掉。
（08-10 起腳本已加「失敗不 rotate」的保護，但只在**完全失敗**時觸發。）
...
| NAS 本身的冗餘 | ✅ **已查證（2026-08-10 owner 回覆）：RAID5**。單碟容錯成立 —— 但見 §8.1，RAID ≠ 備份 |
...
### 8.1 NAS 是 RAID5 —— 它解決了什麼、沒解決什麼

2026-08-10 owner 回覆 NAS 採 RAID5 建構。這把 §6 那條「NAS 冗餘未查證」解決了，
情境 A／B 的信心也明顯提高。但**必須講清楚它沒有動到哪一塊**，
因為「有 RAID 就等於有備份」是最常見也最貴的誤解：

| 風險 | RAID5 |
|---|---|
| 單顆磁碟故障 | ✅ 可重建 |
| **勒索軟體加密** | ❌ **完全沒有保護** —— 檔案被加密後，RAID5 忠實地把加密後的內容寫進所有磁碟與 parity。RAID 是磁碟層冗餘，它不知道檔案內容發生了什麼 |
| 誤刪／誤覆蓋 | ❌ 同上，刪除會同步到 parity |
| 火災／竊盜／水災 | ❌ 整台 NAS 在同一地點（192.168.50.250，同一棟建築） |
| **雙碟同時故障** | ❌ 只容許 1 顆。而且**重建期間正是第二顆最容易掛的時候** —— 重建要數小時到數天，期間所有磁碟高負載全盤讀取，正是 URE（無法修正的讀取錯誤）最容易發生的時刻。磁碟容量越大風險越高 |

**所以 §8 的 3-2-1 評估不變：仍然是 2 份副本、0 份離線。**
RAID5 不增加副本數 —— 它是「同一份副本」的磁碟層保護。

**RAID5 的維運要點**（因為它現在是還原鏈的一環）：

- 定期看 SMART 與 NAS 的磁碟健康通知，**壞第一顆時要立刻換**，
  拖著不換等於失去全部容錯
- 若 NAS 支援熱備碟（hot spare），建議配置 —— 它把「人發現並更換」
  的反應時間從數天縮短到零
- 磁碟建議**不同批次／不同廠**混用，同批磁碟有同時老化失效的傾向

### 8.2 若為 Synology（埠 5000/5001 + nginx 高度指向）

DSM 的快照能力**取決於 volume 的檔案系統**：

| 檔案系統 | 可用方案 |
|---|---|
| **Btrfs** | ✅ `Snapshot Replication` 套件 —— 快照唯讀，勒索軟體改不到；DSM 7.2+ 另有**不可變快照（immutable）**，連管理員都刪不掉，是防勒索最強的一層 |
| ext4 | ❌ 無快照。只能用 `Hyper Backup` 備到另一個目的地（第二台 NAS／外接碟／雲端） |

**確認方式**：DSM → 儲存空間管理員 → 看儲存空間的檔案系統欄位。

若是 Btrfs：安裝 `Snapshot Replication` → 對
`CK_Project/#Project_data` 建立排程快照（建議每日、保留 30 天）→
**DSM 7.2+ 務必開啟不可變快照**。設定一次即永久有效，
是本清單裡唯一「零成本且不依賴任何人記得做」的防勒索手段。
...
| 3 | **DSM → 儲存空間管理員 → 看檔案系統是不是 Btrfs；是就開 Snapshot Replication（7.2+ 加開不可變快照）** | 10min | **情境 C 勒索軟體**。NAS 已是 RAID5，但 RAID 擋不了加密，見 §8.1 |
