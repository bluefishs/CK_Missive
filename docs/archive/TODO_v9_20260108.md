...
- [x] **Phase 3 權限管控機制** (2026-01-08)
  - [x] 事件列表依使用者權限過濾（建立者、指派者、專案成員、管理員）
  - [x] 事件查看權限驗證 (403 Forbidden)
  - [x] 事件更新權限：僅建立者或指派者可修改
  - [x] 事件刪除權限：僅建立者或管理員可刪除
  - [x] Google 同步權限驗證
...
- [x] 匯入時自動關聯 agency_id
- [x] 機關代碼對照表管理介面 (`/agencies` 頁面)
  - 完整 CRUD 功能（新增/編輯/刪除）
  - 統計卡片（機關總數、政府機關、民間企業、其他單位）
  - 搜尋與篩選功能
  - 分頁與排序功能
- [x] 機關資料批次更新功能 (批次智慧關聯 API)
...
**已完成項目**:
- [x] 業務同仁選擇（多選模式）- 連結專案成員
- [x] 專案切換時自動填入業務同仁
- [x] 承辦人篩選功能 (document_service)
- [x] StaffPage 承辦同仁管理介面 (`/staff`)
- [x] ContractCaseDetailPage 承辦同仁 Tab

**實作方式**:
- 使用 `project_user_assignment` 關聯表
- `assignee` 欄位儲存為逗號分隔字串
