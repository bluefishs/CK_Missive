...
## 七、身分範圍也是同一個問題（owner 追加：「同步考量整合配合角色與帳號」）

「這個人能看到哪些案」在收斂前有**兩份判準**：

| 位置 | 怎麼判 |
|---|---|
| `core/case_scope.has_company_wide_scope` | 超級使用者 ＋ 角色在 `{admin, exec, finance, superuser}` |
| `erp/quotations._quotation_scope` | 超級使用者／管理員 ＋ 持有跨案查詢權限 |

⚠️ **實測兩者結果完全相同**（三位主管／財務兩邊都是「不限縮」，一般同仁兩邊都是 170 個案）
—— 因為那三位**剛好同時符合兩份判準**。
與「已收款三份實作算出同一個數字」是同一個形狀：**分歧存在，只是還沒發作。**

收斂後 `has_company_wide_scope` 是唯一判準，三條進入路徑都保留：

1. 超級使用者／管理員（走既有 SSOT，併看旗標與角色 —— 本 repo 有兩位 `role=admin`
   而 `is_admin` 旗標是 false，只看旗標會把他們當成一般同仁）
2. 持有 `CROSS_CASE_PERMISSION`（可授權的擴充點，不必改角色就能開）
3. 角色在 `_COMPANY_WIDE_ROLES`

`_quotation_scope` 改成直接呼叫 `scope_filter`，不留第二份。
**收斂前後所有使用者的可見範圍完全相同**（12 位逐一比對）。

⚠️ **一個我在收斂途中製造又修掉的缺陷**：第一版把權限碼寫成
`erp:quotations:cross_case` —— **那個碼在資料庫裡不存在**，等於把路徑 ② 靜靜關掉。
當時不會有人發現，因為那幾位還有路徑 ③ 撐著。
既有的碼是 `reports:erp:view`，已改回並在常數旁註明「不要在這裡發明新的權限碼」。

守門 weekly 132 因此擴大：除了金額算式，也盯「全公司視角判定」與
「角色集合」出現在 `core/case_scope` 之外。正向控制：注入一份 → rc=2 並指名。
