...
### 10. 舊案號尾碼：附檔比對「忽略尾碼」vs 建案「保留尾碼」

同一個識別碼（舊案號 `B115-C017a-0`）在兩條路徑上**刻意用不同的正規化政策**：

| 用途 | 實作 | 政策 |
|---|---|---|
| 回簽 PDF 掛到報價單 | `signed_quotation_import.normalize_legacy_no` | **忽略尾碼**（`B115-C017a-0` → `115|C|17|A`）|
| 彙整表建立 PM 案件 | `quotation_legacy_import._derive_case_code` | **保留完整案號**，不去尾碼 |

**為何刻意 —— 兩邊錯的代價相反**：

* 附檔比對若嚴格 ⇒ 檔名與彙整表兩份紀錄各自演化的寫法（`C017a-0` vs `C017-a`）
  對不上，**5 個回簽檔有 3 個掛不上**（2026-08-19 實測）。掛錯的代價低、掛不上的代價高。
* 建案若寬鬆 ⇒ **把不同的案子併成一件**。實測 `B114-B026`（平鎮區土地協議市價查估）
  與 `B114-B026-0`（永翠76透地雷達測量）是兩個完全不同的案子；
  去尾碼會讓 4 組被硬掛在一起。合併錯的代價高、分開的代價低。

⇒ **同一個欄位，兩種讀法，都是對的。** 差別在「錯了會怎樣」。

⚠️ **不要統一它們。** 若有人以「同一個概念兩份實作」為由要收斂，
先問「這一側如果用另一側的政策，錯的時候會發生什麼」。

⚠️ 但**寬鬆那一側曾經造成沉默的重複**：`_ensure_pm_cases` 只比對完整 case_code，
2026-08-20 匯入因此建出 **26 件分身**（兩側案名完全相同）而不出聲。
修法不是改政策，是**讓它出聲**（2026-08-27，commit `6a53c3af`）——
匯入前預覽會列出「同 base 且案名完全相同、只差版次」的候選，仍會建立但說出來。
存量的偵測見 `scripts/checks/case_award_pipeline_audit.py`（weekly 77）。
