...
## 業務模組審計 + 收斂（2026-07-20 /loop，Explore 全掃 + 人工核實）

owner「各側頁選單確保無異質重工/非標準化」→ 全業務模組掃描。**整體良好**（ERP/機關/廠商/wiki 後端 DDD 落實高、前端 DetailPageLayout 覆蓋廣）。熱點 4 處，逐一核實後分級處置：

| # | 發現 | 核實結論 | 處置 |
|---|---|---|---|
| **掛載去重** | WikiGraphTab 同掛 KnowledgeBasePage+WikiPage | 重複呈現 | ✅ **移除 KnowledgeBasePage 那份**（`c3bca0d5`，統一 WikiPage） |
| **平臺資訊** | API對應表/統一表單示例 dev/demo 混正式導覽 | 冗餘/demo | ✅ **is_visible=false 隱藏**（`76a33491`，可逆） |
| **HH-3** | expenses.py `/financial-overview`·`/case-finance` 端點內跨表聚合 | 含 ExpenseInvoice 範疇＝與 FinancialSummaryService **不同範疇非重複** | ✅ **搬遷入 ExpenseInvoiceService**（`6746f69b`，行為保真、標準化 layer） |
| **HH-1** | 桃園派工 link 端點繞 TaoyuanLinkService | 🔴 **真行為分歧非純重複**：端點與 service 對 `agency_doc_number` 用**不同衍生**（link_type vs order.agency_doc_id FK），**實測 8 筆有 2 筆文號不同** | ⚠️ **不盲收斂**（會改 ~25% 派工單顯示文號）→ 潛在一致性 bug，**需 owner domain 決策哪算法正確** |

**元教訓（本輪 3 次生效）**：「先核實再宣稱/動手」擋下 3 次自造不穩定——KB 自癒錯接目標、HH-1 衍生分歧、agency_doc 實測不一致。**agent 判『異質同工』是線索，人工核實常揭發是『分歧』（更該報 bug 非盲合併）**。

**待辦（低風險/需決策）**：
- 低風險標準化：tender subscriptions 直SQL+函式內 BaseModel、taoyuan statistics BaseModel→schemas、PM 匯出入 raw fetch→apiClient（純移動/無行為變更，可擇期批次）
- **HH-1 domain 決策**：派工單 agency/company 文號正解＝link_type 衍生 or FK 衍生？決定後才收斂
- HH-2 ERP 應收/應付 rollup 共用 builder（中價值）；圖譜 2 套渲染元件統一（低）

### 續辦（2026-07-20 續，逐項核實後處置）
- ✅ **低風險標準化批次**（`0449b596`）：tender subscriptions `delete_subscription` 反模式修（原 `req: BaseModel`+函式內 IdReq＝FastAPI 無法正確解析裸 body 真缺陷 → schemas `IdRequest`）；taoyuan `ProgressReportRequest` 端點內 → `schemas/taoyuan/statistics.py` SSOT。
- ⚠️ **HH-2 應收/應付 rollup：核實後判「勿收斂」**：client_receivable（兩層 billing_agg 子查詢 + PMCase→PartnerVendor 主 join、分未成案/成案）vs vendor_payable（單層直 join + keyword filter）＝**查詢結構實質不同**，概念鏡像非結構重複；強抽共用 builder＝過度抽象。**設計合理保留**。
- 🟢 **低價值/半正當（不辦）**：PM 匯出入 raw fetch＝xlsx blob 下載上傳（apiClient 走 JSON，raw fetch 對 blob 半正當非純反模式）；圖譜 2 套渲染元件（KnowledgeGraph vs ForceGraphLazy）統一（低價值）；MorningStatusRequest→schemas（同 ProgressReportRequest 型，cosmetic）。

**🎯 業務模組審計元結論**：agent 兩大「異質同工」候選 HH-1/HH-2 經**人工核實皆非純重複**（HH-1 行為分歧＝潛在 bug；HH-2 結構相異＝合理）；唯 HH-3 為真乾淨搬遷（已辦）。**程式庫實際比表面掃描更標準化**——ERP/機關/廠商/wiki DDD 高、前端 DetailPageLayout 廣。**驗證優先於收斂**再次證值。

### HH-1 深度核實 → 治本（2026-07-20，`f0050080`）
續查 HH-1 揭發**端點本身有非確定性 bug**（非單純分歧）：
- **語意鐵證**（`dispatch_import_service.py:261` `if i==0: agency_doc_id=doc_id`）：`agency_doc_id`/`company_doc_id`＝派工單**主要/首份**機關/乾坤公文（確定）；`agency_incoming`/`company_outgoing` **links＝所有往來函累積**（117 筆派工單有多筆，order 2 有 7 封 Jan→Jul）。
- **端點 bug**：`get_document_dispatch_links` 用 link `LIMIT 1 無 ORDER BY` 取任意→117 筆多 link 派工單每次顯示**隨機文號**（實測 order 1 連查得 …1581，FK 實為 …0279）。
- **治本+收斂**：`TaoyuanLinkService._primary_or_earliest_doc_number` SSOT 衍生（FK 主要公文優先、null 則最早 created_at link＝確定性 fallback 補 32 筆 null-FK）；端點委派 service（消端點內直 SQL + N+1）。**實測 order 1 → 確定性 canonical …0279、連 3 次一致**。
- ⚠️ 顯示變更：多 link 派工單改顯示「主要公文」（語意正確）而非隨機 → owner 可於函文紀錄 Tab 複覽。
- 殘留：同檔 link/unlink/batch + 其他 link 端點（project/dispatch 側）仍直 SQL 但**無此 bug**，屬低優先 backlog。

**業務模組審計三輪總結（2026-07-20）**：WikiGraphTab 去重、平臺資訊清理、HH-3 財務聚合搬遷、HH-1 非確定性 bug 治本、低風險批次（subscriptions/schemas）全辦；HH-2 核實判合理保留。**agent「異質同工」候選經人工核實：HH-3 真收斂、HH-1 是 bug（已治）、HH-2 合理**——驗證優先於收斂全程證值。
