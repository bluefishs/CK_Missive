...
| **A38** | 🔴🔴 **公文附件的異地備份實質停在 2026 年 2 月 —— 1,432 檔（92%）從未備份，而它每天回報「全部已同步」** | 2026-08-28 起因是 CK_Website 回報備份掃描有檔案複製失敗。**追查過程中兩個診斷先後被推翻，最後的結論與起點無關。**<br>**① 分布是決定性的**：<br>　　`manifest_20260828` 涵蓋 **2026/01（44）+ 2026/02（76）= 120**<br>　　漏掉的是 **2026/03 起全部**（874+127+112+106+88+85）＋ `pm_attachments`／`asset_photos`／`receipts`<br>⇒ **不是隨機漏，是某個時間點之後就沒再納入**。`attachments_latest` 最後修改停在 **5 月 18 日**，7 個目錄快照停在 **2026-03-09**，三個訊號一致。<br>**② 而它每天回報正常**：08-22~08-28 七份 manifest 數字**完全相同** —— `total=120 copied=0 skipped=120 removed=0 size=0.0MB`。「copied=0」被讀成「沒有新東西要備份」，實際是「**它只看得到那 120 個**」。<br>**③ ⚠️ 長檔名那條線是岔路，只解釋 2 個檔**：CK_Website 最初診斷「路徑超過 260」，我驗證後推翻（`LongPathsEnabled` **已是 1**；.NET 長路徑 API 實測最長路徑 213 字元、超過 260 的 **0 個**）。他們再查出真限制是 **檔名的 UTF-8 位元組數 > NAME_MAX 255**（Windows 用 UTF-16 **字元**數算＝122 合法，Linux/NAS 用 UTF-8 **位元組**數算＝268 非法，中文一字 3 位元組）—— **這個診斷是對的，我用自己的資料驗證了**（uploads 最長檔名 122 字元 / 268 位元組）。<br>**但它只影響 2 個檔，而且機制早就處理了** —— `remote_backup.json` 的 `last_sync_message` 白紙黑字寫著「**附件 2 檔為長檔名，已打包 1 個目錄**」。<br>⇒ **1,432 個漏掉的裡面只有 2 個是長檔名，其餘 1,430 個是完全不同的原因。** | **⚠️ 我沒有在重啟前動備份機制** —— 改備份是高風險操作，重啟前不是做它的時機。<br>**下一步（查清楚，不是先修）**：<br>① 附件同步的**掃描起點／過濾條件**是什麼 —— 為什麼只看得到 2026/01~02，而 `pm_attachments`／`asset_photos`／`receipts` 三個目錄整個不在範圍<br>② `attachments_latest`（5/18）與 7 個 2026-03 目錄快照是舊機制殘留，**它們可能是那個時期的唯一副本，我不會自行刪**<br>③ `remote_backup.json` 的 `sync_enabled: False` 實際語意（DB dump 確實有跑：`ck_missive_backup_20260828_015959.sql` 526MB）<br>**修法方向（CK_Website 建議，值得評估）**：附件快照改走**壓縮封裝**（tar/zip）—— 封裝檔內部不受目的地 FS 的 NAME_MAX 限制，順帶解決「副本的副本每次完整複製」。你們的 docker volume 備份是 tar.gz，所以從來沒撞到這個。<br>**判準（08-24 記過，這次是第二個實例）：備份成功 ≠ 備份到我們需要的東西。** |
