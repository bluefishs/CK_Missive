...
| 09-06 排程退出碡 triage（AaaP 知會） | AaaP drift §40 R4 報三支 host 排程 rc≠0、其中一支從未 triage。判讀：①`SelfAudit-CapabilityUsage` rc=1＝腳本自訂「判定時點（08-31）已到，需人工下結論」——資料足夠（26.6 天）、69 個零流量 API 候選＋2 豁免；依 `zero_traffic_is_not_dead`（11 個核實後 0 個該刪）只能當人工複核線索 ⇒ **A110 待 owner 決定要不要逐一複核或把判定時點往後推**；②`SelfAudit-Flow` rc 2→1＝19 過 1 敗，敗的是回歸鎖 `visual-walk-lock-contract-case-mobile` 鎖「得標金額」標籤，而 09-05 依 owner 裁示改名「議價金額」——鎖過期不是頁面壞，已改鎖；③`Fitness-Weekly` rc=1＝本輪有 RED（16 支，22 YELLOW），rc 語意是「RED → exit 1」，紅燈名冊由 weekly 94 管。三支的 rc 都是設計中的黃／紅語意，不是執行失敗 | ✅ |
