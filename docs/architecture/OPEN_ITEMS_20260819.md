...
| **A103 RWD 整體調整規劃（執行中）** | owner 09-05「優先處理 RWD，多數透過手機檢視」⇒ 依 `RWD_REVIEW_20260905.md` 開做。**第一批已上線**：①表格包裝 `mobileCard`——手機（< 768）不畫表格改畫卡片（`MobileCardList`），分頁與點列進詳情保留；套到專案帳款／承攬案／PM 案／委託單位帳款／廠商帳款五個列表 ②欄位隱藏門檻 992 → 768（平板與筆電全欄顯示）③金額與序號欄拿掉隱藏（廠商帳款已付總額／付款率、桃園序號）④手機 Modal 撐滿、Drawer 不超寬。**下一批**：P4 量測閘門（四寬度溢出探針）、詳情頁 Descriptions 欄數、表單列 | 🟡 進行中 |
