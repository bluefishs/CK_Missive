...
| 項 | 內容 | 狀態 |
|---|---|---|
| **A90 同名承攬案已存在的 9 筆 PM 已承攬案** | 09-03 匯入建的 PM 案標 `contracted`，同名承攬案在別的案號下（`CK2025_PM_02_058`↔`_057`、`_074`↔`_188`、`_077`↔`_076`、`CK2026_PM_02_030`↔`_029`、`_004`↔`_003`、`CK2024_PM_02_002`／`CK2025_PM_02_006`↔通霄 1355 兩案、`CK2026_PM_02_028`／`_064`↔「建物第一次測量」3 件）。**是同一件工作建兩次，還是同名不同案？** 判「同案」→ 把 PM 的成案編號指向既有承攬案並刪重複報價單；判「不同案」→ 改名後成案。weekly 105 基線登記 10（含 #243） | 🔴 待 owner |
| **A91 #503 `B114-C033b-0` 三方不一致** | 總表總價 106,124；PM 合約額＝承攬合約額＝請款額 132,342；報價單 119,738。哪個對？ | 🔴 待 owner |
| **A92 報價單總價更正 229 張（＋4,827,188）** | 03-17 批 `含稅−2×稅`、08-20 批 `未稅×0.85`；229 張 PM／承攬／請款三方都＝總表總價。更正清單 `scratchpad/total_price_fix.json`（id／舊值／新值／依據）。**批次 SQL 兩次被 Claude Code 權限分類器擋下**（協同界線③：不轉包）。執行方式：備份表 → 逐張 `UPDATE ... WHERE total_price=舊值` → `audit_logs` 每張一筆；**另把 #390 的自動第一期 billing #304 由 215,152 改 265,776**（成案時總價還是錯的）。執行後 weekly 104 ①191／⑨216 應歸零 | 🔴 待 owner 授權 |
