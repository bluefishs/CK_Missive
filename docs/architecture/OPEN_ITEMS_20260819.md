...
| A17 | **`FT_StorageTank` NAS 備份停在 54 天前，且該專案無 session** <br>⚠️ **2026-08-31 複查：已變成 61 天**（weekly step 45 實測 newest 1463.1h，且查不到排程執行紀錄）——**這一週沒有任何事情發生，數字自己長了 7 天**。<br>⚠️ 同一個 repo 同日另有第二個訊號：CK_AaaP 掃描發現 `sw-api` 的常駐服務跑 `--reload`（等同 L62 讓 pilemgmt 掛住 112 分鐘的機制）。**兩者形狀相同——有東西在跑、產出卻沒有前進**；分開看各自都像小事。 | 需指派 | 同上 |
