...
| **A111 前端 vitest 腐爛** | 09-06 首次完整跑：254 失敗／2,664 過（41 檔），主因 `vi.mock` 沒跟上新 export（ClickableStatCard／useCaseCodeMap／useTaoyuanDispatchOrders／ROLE_FILTER_OPTIONS…）與 jsdom 缺 getComputedStyle／ResizeObserver。**沒有任何排程或閘門在跑它**。補法：mock 改 `importOriginal` 展開再覆蓋、jsdom polyfill；跑得起來後接進 weekly 24 基線比對 | 待做 |
| **A112 後端 pytest 基線失真** | host 跑 65 失敗而 `known_failures.json` 只 2 筆 ⇒ weekly 24 每週紅 63、沒人收。失敗集中在本週改過的服務的 mock 測試（quotation／items／case_code／auth／digital_twin／financial_dashboard）＋測試庫 schema 漂移（weekly 87）。補法：先修本週造成的、測試庫 upgrade、其餘登基線；再談 pre-push 跑 `tests/unit -x` | 待做 |
