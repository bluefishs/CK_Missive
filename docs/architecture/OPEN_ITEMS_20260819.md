...
### A115 — 2026-09-07 待 owner 裁示的四項（其餘皆已完成並複驗）

當日所有修法的完整性複查結果見下；**除這四項外，逐條都已驗到綠**。

| # | 事項 | 現況 | 為什麼要 owner |
|---|---|---|---|
| ① | **賴秀玲的權限雙向漂移** | 缺 `operational:write`／`operational:approve`，另有 **28 項角色以外的權限**（從 admin 轉 finance 時使用者層沒清） | 按「同步該 role 所有 active user」會**移除**那 28 項，可能有意保留 |
| ② | **業務同仁現有委託／協力帳款權限** | 角色層 staff 兩個新碼都是 true | 與 09-07 早先「業務同仁僅需看到專案帳款」不一致；是我拆碼**之後**被勾上的，未自行更動 |
| ③ | **總表 2 筆「未標成立、系統已成案」** | `B114-B026-0`（120,000，請款1／發票1）、`B114-C029-1`（18,900，請款1） | 一筆已開發票 ⇒ **比較可能是總表漏填 v**，不是系統誤成案。撤哪一邊是業務判斷 |
| ④ | **`CK2026_PM_02_075` 的 q640** | 總價 1,271、無編號、仍活著；同案的 `B115-B019-0`（8,400）已依總表復原 | 刪除會影響承攬案，不自行決定 |
