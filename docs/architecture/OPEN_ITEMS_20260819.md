...
## 🗂 2026-09-01 統整入口（A57–A65 一覽）

> 08-31～09-01 兩天新增 9 項。**這張表只是索引**，內容在各自的段落。
> 排序依「誰在等」：owner 裁示 > 有期程 > 觀察中。

### 等 owner 裁示（我不會自己做）

| # | 事項 | 為什麼要你決定 | 量到的事實 |
|---|---|---|---|
| **A61** | 40 筆「同一件工作建了兩次案」 | 同名同年同客戶**可能真的是兩件工作**；4 筆對到多筆；既有專案已綁別的案號 | 逐組對照＝`docs/runbooks/quotation_revision_dups_20260901.md`｜互動版 artifact |
| **A57-舊** | 已成案的 136 筆 `pm_cases` 要不要刪 | 刪除不可逆，且 136 張報價單掛在這些 `case_code` 上 | 里程碑 0 筆、`contract_projects` 對得上 136/136 |
| **A63 P2** | `config/` 目錄移除（09-15） | 「grep 找不到讀者」≠「執行時沒有人讀」 | 已標記 `_remove_after`，P1 完成 |
| **A65** | 下拉接伺服器端搜尋的時機 | 接線動 4 個消費端，而目前狀態可用；我今天在這條路上弄壞過兩次線上 | hook 已就緒、守門會先出聲 |
| **A56** | 區網＝超級管理員 | 會影響「快速進入」流程 | 未帶憑證 POST 回真實公文 |
| **A17** | FT_StorageTank 三個訊號 | 別的 repo | 備份 61 天無新檔等 |
| **推送** | 190+ commit 未推送 | `! git push origin main` | — |

### 有期程

| # | 事項 | 期程 | 狀態 |
|---|---|---|---|
| **A64** | SSO 到期靜靜變「訪客」 | **09-08** | `X-Reauth-Required` 全庫**只在規範文字裡、程式碼一個都沒有** |
| **A63** | 設定目錄收斂 P2 | **09-15** | P1 已標記完成；weekly 96 守著不再長出第三個 |

### 已辦（09-01 收束，列此以免重複提案）

| # | 事項 | 結果 |
|---|---|---|
| A57 | 91 筆已承攬未成案 | ✅ 成案 51 筆（91→40），逐筆走正式流程、快照留存 |
| A58 | 3 張報價單無承辦 | ✅ 查明**兩條綁法都是 0 筆指派**，非程式缺陷 ⇒ 需人補指派 |
| A59 | 伺服器分頁頁面失去排序 | ✅ `/pm/cases` 與 `/erp/quotations` 已接後端排序；`SelfAudit-Flow` 由 FAIL 轉 **GREEN 20/20** |
| A60 | 兩個「王駿穠」帳號 | ✅ 查明 alias 群組有生效（兩帳號都得到 6 件）；併不併由 owner 決定 |
| A62 | 12 個承攬案無成案編號 | 既有 GN 舊案，與本次無關（仍待決定是否補號） |

### 新增的守門（防這一輪的問題復發）

| weekly | 做什麼 | 負向對照 |
|---|---|---|
| **95** | 下拉取數上限 vs 資料筆數 —— **每個下拉還能長幾筆才會靜默截斷** | 4/4 通過 |
| **96** | 設定目錄 SSOT —— 專案根不得長出第三個 | 2/2 通過 |
| 匯入端 | `revision_dups`：同一份估價單的不同版次（`B114-B048` vs `B114-B048-1`） | 正負向皆通過 |
