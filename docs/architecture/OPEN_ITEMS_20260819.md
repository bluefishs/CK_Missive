...
| 項 | 是什麼 | 為什麼要你決定 | 建議 |
|---|---|---|---|
| **A127 主機層記憶體故障** | swap=0 實驗前提被 AaaP 推翻（十筆故障六筆發生在零 swap I/O 窗口）；`runbooks/swap-zero-experiment-20260909.md` §執行前必讀 | 三個選項都要停機或都可能無結論 | (a) 跳過 swap 直接 memtest86+ (b) 兩個都排 (c) 都不排、繼續累積證據——**在「兩條路都可能過關」的前提下 (c) 不是拖延** |
