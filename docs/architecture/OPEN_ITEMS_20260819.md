...
| 項 | 是什麼 | 為什麼要你決定 | 建議 |
|---|---|---|---|
| **A127 主機層記憶體故障** | swap=0 實驗前提被 AaaP 推翻（十筆故障六筆發生在零 swap I/O 窗口）；`runbooks/swap-zero-experiment-20260909.md` §執行前必讀 | 三個選項都要停機或都可能無結論 | (a) 跳過 swap 直接 memtest86+ (b) 兩個都排 (c) 都不排、繼續累積證據——**在「兩條路都可能過關」的前提下 (c) 不是拖延**。⚠️ 10:xx 再降一級：DT 容器層 `refault_anon` 整場 +1 ⇒ swap=0 可能只是「移除一個沒在用的東西」；觸發率修正為 4 次 1 次（N≥6 是 n=3 時算的）；AaaP 建議先查 08-11 三筆 Windows 更新 |
| **A129 長期紅燈 9 支登記中** | weekly 有 10 支從來沒綠過，9 支已在 `.chronic_red_registry.json` 登記處置（34／35／50／54／55／56／61／69／71）；`WEEKLY_ROLLUP_20260909.md` §三 | 「繼續掛著」與「撤掉檢核、承認它不該紅」是產品判斷 | 逐支看一次，撤掉的從 runner 移除，留下的寫到期日 |
| **A130 誤植成案怎麼下架** | `VOID_VS_REASSIGN_20260909.md`：註銷狀態會擴散到 92 處口徑；建議改做「金流轉掛」D1 預覽→D2 轉掛→D3 解 4 筆孤兒 | D2 是不可逆資料操作 | 先做 D1（唯讀預覽，零風險），你看過再決定 D2 |
| **A131 promote 端點缺身分範圍檢查** | 成案端點只驗登入不驗「這個案在不在你的範圍」 | 收緊會讓業務同仁無法替別人的案成案，那是不是現行作法要你說 | 補 `assert_case_scope`，超管／管理員不受限 |
| **A132 mdsched 記憶體診斷** | 本機從未做過（A66-P3）；與 A127 同一件事 | 整夜停機＝你排 | 與 A127 一起決定 |
...
四支新守門（128 表頭篩選鍵接線／129 承辦身分合併展開／130 請求物件的猜／131 列表與統計身分口徑／132 經費指標唯一實作）＋ weekly 統整彙總器；六支本週新紅全清（59／76／102／63／32／111）；
財務儀表板年度口徑差 1,693 萬已修＋類別加「全部」；協力帳款「顯示 (3) 卻 0 家」（alias 沒展開）已修；PM 統計卡身分範圍已修；卡片標籤固定版位；裝置偵測多證據；13 處 getattr 猜欄位已清；
統計中心服務上線（`services/stats/`）並驗證三個畫面同值 108,108,873、12 位使用者範圍一致。

---

## 🧭 09-06 統整入口（v6.74；本表往下是歷次逐條紀錄，先看這張）

### A. 等 owner 決定的（我不自行動手）

| 項 | 是什麼 | 為什麼要你決定 | 建議 |
|---|---|---|---|
| **A92 報價單總價更正 229 張** | 03-17 批＝`含稅−2×稅`、08-20 批＝`未稅×0.85`，少記 482 萬 | 批次 UPDATE 兩次被權限分類器擋下；**跨 session 代跑＝權限轉包，不做** | 你授權後我在本 session 執行（已備份、附審計） |
| **A99 名稱快照漂移 5 筆／A93 客戶名不同 2 筆** | 「張啟良建築師」vs「張啟良建築師事務所」這類 | 要判「同一家還是兩家」，那是業務事實不是程式問題 | 逐筆看一次，決定以主檔為準或改主檔 |
| **A105 委託單位雙主檔** | 同一單位在兩張主檔各有一筆 | 合併會動到既有金流的外鍵 | 我先出對照表，你點頭再合 |
| **A113 正式庫 12 張遷移備份表** | `*_bak_a80`／`*_bak_v2` 等，佔空間也會混淆稽核 | DROP 不可逆 | 建議 2026-10-04（滿一個月）後 DROP，先由我出清單 |
| **A103 RWD 整體調整** | 手機版仍有逐頁差異 | 要你先定「哪幾頁最常用手機看」 | 給我 3–5 頁，我逐頁做到閘門全綠 |
| **平臺 colo（跨 repo）** | Cloudflare 邊緣落在 SJC，每支 API 多 0.4–0.55 秒 | 屬 CK_AaaP 的平臺決策 | 由 AaaP session 提案，你拍板 |

### B. 我接下來會做的（不需你決定）

| 項 | 內容 | 依據 |
|---|---|---|
| 兩份基線守著 | weekly 24／114 任何一筆失敗都是新問題；pre-push 已擋在推送前 | 09-06 兩份基線歸零 |
| 逐頁清 RWD 手機品質基線 | 字級與點擊目標基線 0／59，往下降 | weekly 111 |
| 慢查詢追蹤 | `pg_stat_statements` 每週看前 10 名 | 09-06 起常駐 |
| 結果契約統一 | 各層留痕鍵名不一（`checked_at`／`captured_at`／`ts`），寫一支 writer 讓 weekly 113 只讀契約 | 機制圖缺口 6 |

### C. 建議（有代價、值不值得由你判斷）

| 建議 | 好處 | 代價 |
|---|---|---|
| 視覺走查改週排程（僅報告） | 版面回歸不再靠人想起來 | 每週多一份圖要人看；不看就沒有意義 |
| 效能探針入庫看趨勢 | 頁面變慢時看得出是哪一版開始 | 多一份留痕檔要維護 |
| 真實負載壓測（locust 已在 requirements） | 併發行為目前全是推的 | 需要一個不影響正式庫的環境 |
| 前端 RUM（web-vitals，lvrland 已有） | 使用者體感有數字 | 要送一份匿名資料到自家端點 |

### I. 09-07 稅務呈現／一票多案／互抵／每日財務健檢（已上線）

| 項 | 狀態 |
|---|---|
| 兩張發票金額更正 | ✅ 已依授權改完，weekly 104 的 RED 清空 |
| 稅務三欄呈現（未稅／稅額／含稅＋免稅標示） | ✅ |
| 一票多案分攤表 `erp_invoice_allocations` | ✅ 表與守衛就緒；**UI 尚未提供分攤輸入**（目前需由 API 帶 `allocations`）—— 等第一筆真實需求出現再做畫面，避免做出沒人用的表單 |
| 互抵／約定不開票 | ✅ 2 筆已正名；新填必須寫依據 |
| 每日 07:00 財務健檢摘要 | ✅ 只推新增、落點站內通知 |
| 互抵雙向對帳（`offset_payable_id`） | ⏸ 目前只有 2 筆，為 2 筆建雙向對帳不划算；**等到第 5 筆再說**，那時也會知道實際形狀 |

### H. 09-07 填報當下的檢核與自動關聯（已上線）

| 項 | 內容 |
|---|---|
| 已辦 | 名稱→鍵解析器＋併寫偵測（應付建立）、發票三道守衛＋自動綁請款、收款上限；weekly 117 以執行時驗證守衛本身 |
| 端到端實測 | 併寫 400／未知廠商 400／正常 200；發票超額 400／稅額不相稱 400／同額自動綁 200 |
| **新發現：主檔本身有併寫列** | `partner_vendors` 裡有 `楊長燁加李雅倫`、`祐鴻+昱緯+建倫` 兩筆 —— 那是主檔資料錯誤（同 #51 家族）。從下拉選它們仍可用（有 vendor_id 就不進解析器），但**掛在它們名下的錢一樣分不出是誰的** |
| **待你決定** | 這兩筆要不要拆成各自的主檔（拆完舊資料同樣需要金額依據）；另 `顏新元` 在主檔有三筆同人不同寫法（57／109／288），屬 A105 家族 |

### G. 09-07 owner「拆帳、修正」授權後

| 項 | 做了什麼 | 狀態 |
|---|---|---|
| **請款 > 合約額（4 件）** | 逐案調證後分兩型。**型一（已修）**：#127／#155 的請款正好是含稅合約額再 ×1.05（把已含稅的金額又課一次稅），未收款、發票是系統自動補建 ⇒ 用系統 API 改回 434,000／323,200 | ✅ weekly 100 由 4 件降為 2 件 |
| ├ **型二（待你決定）** | #144 已收 26,250、#108 已收 49,000，而報價只有 21,000／45,000 —— **錢真的進來了**，少記的是營收。系統自己的守衛擋住直接改總價（已有請款時要走新版次），**那道守衛是對的，我不繞過** | ⏸ 請在畫面上以 revision+1 補到實收，或告訴我是追加還是原本漏記 |
| ├ **發票同步（被權限擋）** | 兩張自動補建發票（CK-202602-0090／CK-202603-0118）仍是舊金額 455,700／339,360。直接 SQL 與系統 API **兩條路都被權限分類器擋下** | ⏸ 請在 `/erp/invoices` 改成 434,000（稅 20,667）與 323,200（稅 15,390），或授權我這類寫入 |
| **拆帳（應付 #51）** | 調證結果：**系統裡沒有任何憑證能決定 414,750 怎麼分**（沒有分列工項、沒有核銷發票、沒有指派金額、匯入來源檔不在 repo）。猜一個比例會產生看起來像事實的數字，比不拆更糟 ⇒ 交付工具 `scripts/tools/split_vendor_payable.py`：你給兩家金額，它負責拆得對（總額不符即拒絕、帳本同步拆、備份可回溯）；試算已驗 | ⏸ 請給銢欣（80321095）與司乃耳（82349892）各多少 |
| **慢性紅燈判準三個錯** | ①daily 對「此環境無法判定」寫的是字串 `"skip"`，而判準 `!= 0` 把**沒檢查算成一直紅**；②視窗混了 daily 與 weekly，weekly 跑得少所以它的步驟在視窗裡看不到；③「登記過期」用「不在慢性名單」判，於是視窗最舊那輪剛好是綠的步驟被誤判成已轉綠（44／45 最近 7 輪都紅） | ✅ 三個都修，weekly 94 GREEN（12 支長期紅燈皆已登記） |
| **複驗** | 每日 runner 12/17 判定、0 紅（唯一 YELLOW 是今天部署七次造成的重啟，已登記）；後端單元 4,005 全過；公網 200 | ✅ |

### F. 09-06 深夜 owner 五頁走查（已修，已上線）

| 頁面 | 症狀 | 真因 |
|---|---|---|
| `/erp/financial-dashboard` | 專案財務一覽與 Top 15 空白 | API 型別少一層包裝，讀 `.items` 永遠 undefined（**空表不報錯**） |
| `/taoyuan/dispatch` | 手機上表格每格 1–2 字 | 前兩次修法只讓「整頁溢出」歸零，擠壓量不到 ⇒ 改卡片 |
| 統計卡 | 只有一張帶小數、卡片高度不齊 | 顯示層未取整 ⇒ `fmtMoney` 加 `Math.round` |
| 委託單位應收表 | 最右欄被切 | 金額欄寬不足 8 位數＋千分位 |

**複驗**：weekly 111 GREEN（39 頁，點擊目標 57 優於基線 59）；教訓 L144 入冊。
**資料面待你看**（不是顯示問題）：財務儀表板「收入／支出／淨額」多為 0，因為帳本尚未登錄實收；未收總額 63,668,220、205 筆。

### E. 09-06 授權後逐項辦理（owner「授權 依前述規劃與建議事項逐一辦理」）

| 項 | 做了什麼 | 結果 |
|---|---|---|
| **A92 報價單總價** | **先驗證再動手** —— 實查稅比異常 0 筆、weekly 104 ①⑨ 已歸零 ⇒ **09-04 就已執行完畢**，不需要也不應該再更正一次 | 已結案（重複更正會造成第二次錯誤） |
| **A99／A93 名稱漂移 12 筆** | 逐筆看證據後分三類：①**簡稱**（張啟良建築師／顏新元 代書／竣吉不動產估價師）5 筆 ⇒ 快照對齊主檔；②**公文上的字是事實記錄**（受文者寫「張坤樹代表」）4 筆 ⇒ 登記為合理漂移，不改寫；③**真的可疑** 2 筆 ⇒ 留給你 | 12 → 2 |
| ├ 公文 #361 | 快照「380000000A (桃園市政府)」而鍵指向工務局 —— **代碼可證是錯鍵**（380000000A 屬桃園市政府 id 9） | 已改鍵並正規化快照 |
| ├ 應付 #39 | 快照「林晉廷」，主檔是「林宥廷測量技師事務所」（統編 26771256），金額 6,000 | **待你判**：是不是打錯字 |
| └ 應付 #51 | 快照「銢欣有限公司乃耳企業社」＝**兩家併寫**（銢欣 80321095／司乃耳 82349892），金額 414,750 只掛在銢欣 | **待你判**：要不要拆帳 |
| **A105 雙主檔** | 對照表產出（15 家名稱完全相同）＋三個方案與代價 → `docs/runbooks/dual_master_client_vendors_20260906.md` | **待你選 A／B／C**（建議 A：補一條鍵，不合併） |
| **A113 備份表** | 依原建議**不動**，2026-10-04 滿一個月再議 | 維持 |
| 結果契約統一 | `lib/result_contract.py`：每層寫同一種形狀到 `integration-health/<layer>.json`；weekly 113 改讀契約 | 三個新層已接 |
| 效能探針入庫 | weekly 115：每路由 API 支數與 wall 入庫並與上次比對（>50% 或多 3 支才提）；**不判紅** | 首筆基準已寫 |
| 視覺走查週排程 | weekly 116：每週拍五個代表頁存 `docs/health/visual/<日期>/`；**不判紅**（要人看） | 已掛 |
| 每日 runner 三紅 | ①探針在容器內 `cwd` 不是 /app ⇒ `ModuleNotFoundError`（我加的步驟每天紅而它其實沒毛病）⇒ 自補應用根與 cwd；②容器可見的索引缺 13 支 ⇒ README 補齊；③今天搬 docs 造成 9 檔未入庫 ⇒ 跑增量同步 | 3 紅 → 0 紅 |

### D. 09-06 已結案

| 項 | 結果 |
|---|---|
| **A46 pre-push** | 以「只跑相關測試」的快速版接上（全套 467 秒那版不接） |
| **A110 零流量 API** | 69 個候選：65 有前端呼叫、3 個 Hermes、真死 1 個（已刪）；判準改「零流量 ∧ 無呼叫者」 |
| **A111 前端 vitest** | 254 → 0，接 weekly 114 基線比對 |
| **A112 後端 pytest 基線** | 65 → 0，基線歸零 |
| **A107／A108** | 棄用的發文字號 router 與頁面刪除，schema 併回 `schemas/document.py` |
...
| **A103 RWD 整體調整規劃（執行中）** | owner 09-05「優先處理 RWD，多數透過手機檢視」⇒ 依 `RWD_REVIEW_20260905.md` 開做。**第一批已上線**：①表格包裝 `mobileCard`——手機（< 768）不畫表格改畫卡片（`MobileCardList`），分頁與點列進詳情保留；套到專案帳款／承攬案／PM 案／委託單位帳款／廠商帳款五個列表 ②欄位隱藏門檻 992 → 768（平板與筆電全欄顯示）③金額與序號欄拿掉隱藏（廠商帳款已付總額／付款率、桃園序號）④手機 Modal 撐滿、Drawer 不超寬。**第二批**（owner 09-05：「/contract-cases 卡片風格優於 /documents」「篩選列收合、/erp/quotations 查詢佔手機 1/3」）：⑤`FilterBar`——手機只常駐搜尋框，篩選與操作收進「篩選」鈕（顯示生效篩選數），展開後每個控制項一整行；套到 `/erp/quotations`、`/contract-cases` ⑥`/documents` 手機改同一套 `MobileCard`（收發／發文形式／附件數／案件）⑦公文字號頁四張卡接篩選、weekly 108 假互動偵測（66 張卡全接）⑧行動探針門檻 400→24px、加 1024 寬度。**第三批**（owner 09-05「資訊卡片呈現與風格為 RWD 檢視目標，同步修正列表欄寬與文字比例」）：⑨窄螢幕表格**保留欄寬改橫向捲動**（不再 13 欄擠 38px）⑩文字比例三級（桌面 14／平板 13／手機 12.5）⑪MobileCard 樣式集中 class ⑫案件管理篩選列收合 ⑬weekly 109 整頁溢出閘門（≥24px 即 RED；表格內橫向捲動不計）——首跑 RED 2：`/pm/cases`@390 84px（按鈕列不換行，已修）、`/staff`@768 32px（查中）。⑭統計卡網格統一 `xs={12}`（公文字號／委託單位帳款／廠商帳款／應收應付分頁／承辦同仁此前 `xs={24}` 手機一張一列；weekly 108 加網格判準，負向對照修法前 13 處）⑮案件顯示以成案案號為主（報價單詳情／PM 詳情／卡片）⑯帳本、承辦同仁列表手機卡片。**09-05 09:38 探針對照**：390／768 整頁溢出 11 列 → **6 列**（最大 84px → 32px；閘門 RED 2 → 1，`/staff` 寫死 `scroll.x` 已拿掉）；剩 5 列 3–17px（分頁鈕／Select／icon）已改換行與精簡分頁，等下次探針。⑰**登入裝置偵測＋版面模式**（owner「能偵測登入平臺是否為行動裝置自動轉換」）：UA-CH／UA／觸控偵測，行動裝置橫放 768–991 仍走手機版面；使用者選單可強制手機版／桌面版（localStorage）。**下一批**：其餘列表頁 FilterBar、詳情頁 Descriptions 固定欄數 18 處、表單列 | 🟡 進行中 |
| 09-05 頁面成本量測（lvrland 跨 session 知會） | ①`cdn-cgi/trace` colo=SJC：公網 `/api/health` 0.56s、本機 0.009s ⇒ **每支 API 固定多 ~0.55s 往返**（平台級，AaaP 登記）。②冷載入每頁 8–15 支（其中 7 支是 layout 級：nav／csrf／auth-check／auth-me／unread／filing-gaps／calendar-events）；**SPA 暖切換只剩 0–7 支**，React Query 快取有效。③後端真實耗時多在 15–75ms，唯一慢的是 `/kunge/ops` 的 `ai/digital-twin/dashboard` 冷 4.2 秒（六源 gather、有一源撞 3s timeout）⇒ 加 60 秒程序內快取（4069→7ms）。④費用頁不在帳本分頁也打 `erp/ledger/list` ⇒ `enabled` 條件化。⑤未做：layout 級七支合併（結構性）、`/document-numbers` 為何打 `calendar-events`（待查）。探針已入庫 `scripts/perf/route_cost_probe.cjs`。**第二層追查**（逐源量）：孿生快照 4.2 秒的真因有三——①聯邦註冊表預設指向**已廢止（ADR-0015）的 `nemoclaw_tower`**，每 60 秒 TTL 同步 DNS 失敗 4 秒且在事件迴圈上（同時進來的 navigation/action 也被拖到 4.5 秒）⇒ 沒設環境變數就不打；②每日鏡像 `generate_mirror_report` 少傳 `ai_connector` ⇒ TypeError 被吞、**daily 永遠 None 而不報錯**；③鏡像要跑 LLM 6.6 秒 ⇒ 改背景回填快取。冷載入 4069→1204ms、迴圈延遲 4079→15ms | ✅ |
| **A110 能力使用判定時點已到** | **09-06 已辦**：69 個 7 日零流量 API 逐一對照程式碼——65 個前端程式碼有呼叫（管理頁、圖譜、匯出入、成案等低頻動作，只是視窗內沒人用）、3 個是 Hermes 排程／skill 打的、**真正無人引用 1 個**（`/api/knowledge-base/stats`，已刪）。人工複核 69 個是白工 ⇒ 快照判準改為「零流量 ∧ 無任何呼叫者」才列 `dead_candidates` 短名單，黃燈只在短名單非空或到期時亮；下一檢視點 2026-12-01。同族教訓 `zero_traffic_is_not_dead` | ✅ |
| **A111 前端 vitest 腐爛** | 09-06 首次完整跑：254 失敗／2,664 過（41 檔），主因 `vi.mock` 沒跟上新 export（ClickableStatCard／useCaseCodeMap／useTaoyuanDispatchOrders／ROLE_FILTER_OPTIONS…）與 jsdom 缺 getComputedStyle／ResizeObserver。**沒有任何排程或閘門在跑它**。補法：mock 改 `importOriginal` 展開再覆蓋、jsdom polyfill；跑得起來後接進 weekly 24 基線比對 | 待做 | **→ 09-06 晚：三個共同根因修掉（barrel 部分 mock 改 `importOriginal` 展開 ×62 檔／vitest 補 `dedupe`＋`server.deps.inline` 收掉 sso-js 的第二份 React／`useResponsive` mock 補 `responsiveValue`）254→85；接 weekly 114 基線比對（86 項＝畫面改了測試沒跟，逐檔清）。commit 閘門仍待 A46**；另有 3–4 支（AdminDashboardPage／PageRender／ERPQuotationFormPage 的「renders page title」）**多檔並行才紅、單檔跑綠**＝測試隔離問題，pre-push 以關並行複跑排除、不擋，待逐檔查。**09-06 深夜逐檔清存量：86→7**（LoginPage 12 支全是 ADR-0033 前的帳密表單 → 重寫成 redirect 3 支；其餘為標題更名／統計卡改 financeTerms／分頁併入／操作欄改點列／狀態列舉改值／下拉上限 100→1000／CSRF 改每請求一張／導覽 fallback 不含業務項／圖譜節點 27→32 等契約漂移；首支測試冷載入 >5s 的檔把 waitFor 拉到 9s）。剩 7 項：SystemHealthDashboard ×2、useDeleteDocument、usePermissions 未登入、DocumentPage 匯入、EntryPage ×2（session store 宣告式導向）|
| **A112 後端 pytest 基線失真** | host 跑 65 失敗而 `known_failures.json` 只 2 筆 ⇒ weekly 24 每週紅 63、沒人收。失敗集中在本週改過的服務的 mock 測試（quotation／items／case_code／auth／digital_twin／financial_dashboard）＋測試庫 schema 漂移（weekly 87）。補法：先修本週造成的、測試庫 upgrade、其餘登基線；再談 pre-push 跑 `tests/unit -x` | 待做 | **→ 09-06 晚：全套重跑 38 失敗（65→38），本週改動造成的 mock 失敗修 7 支（報價單 5／PM 建案 2），基線重錄 31 項；weekly 24 下次只對新增失敗紅。auth 3 支全套紅、單跑綠＝順序相依，留基線待查** |
| **A113 正式庫殘留 12 張遷移備份表** | `*_bak_a80`／`*_bak_v2`／`*_bak_20260902`／`erp_quotations_total_price_backup_20260904`／`amount_align_bak_a85` 等 12 張（217 欄），是 08-29～09-04 幾次資料修正前的安全複本。它們讓 weekly 87 誤報「測試庫缺 217 欄」（已改為排除）。留著佔空間也擾亂稽核；要不要在確認修正無誤後 DROP 是 owner 決定（建議保留到 2026-10-04 再刪） | 待 owner |
| 09-06 業務鏈自主測試（owner「再次複查一鍵建案、新增報價、創案、報價單填列、成案、財務管理」） | 部署探針從一條鏈（一鍵建案 15 斷言）擴為兩條鏈 **32 斷言**、每次部署必跑、跑完硬刪（帳本／應付／關聯／工項／發票全清）。鏈 B（手動全程）首跑抓到**三個真斷點**：①工項填列把**未稅小計**寫進 `total_price`，而定義是含稅 ⇒ 填了工項的報價單少 5%；改含稅（小計×1.05）、稅＝小計×5%。②工項總價只回寫報價單，PM 合約金額不動 ⇒ 按「成案」被擋「尚未填寫合約金額」，得手抄同一個數（L140 同型）；改同步 PM／承攬案 `contract_amount`，並掛成案即應收。③協力廠商指派自動建的應付沒掛 `billing_id`，一建就是 weekly 99 RED；報價單唯一請款時自動對上。另：工項端點把總價鎖的 ValueError 回成 404 ⇒ 改 400；已有請款者不得經工項改總價（與 update 同一把鎖）。**設計確認**：評估中不得直接成案（要先已承攬）是規則不是 bug | ✅ |
| 09-06 評估建議辦理① | /loop「依前述規劃與建議逐一辦理、整理資料夾」：#3 **`pg_stat_statements` 已載入**（compose command 加 preload、postgres 重啟 1 秒、CREATE EXTENSION，健康全綠）；#4 `files/storage-info` 掃描結果快取 120 秒；#5 新 weekly 112 `async_sync_io_audit`（AST）首跑 11 處 RED——Google 登入的同步 `verify_oauth2_token`、備份 pg_dump／psql、自診斷 urlopen、五處 git 快照——**全部改 `to_thread`**，基線 0；A63 三份設定檔收斂完成。資料夾：根目錄雜檔 `'2026-05-18'`（1.27MB）與兩個 `;C` 空目錄刪除；`backup/`（90MB 五月事故快照，在版控）移出 repo 至 `D:/CKProject/_archive/CK_Missive/backup_incidents_202605`；README 從「# .」改為真的索引；兩份 OpenClaw 文件封存至 `docs/archived/2026-09-openclaw/`。docs 根層 56→42：9 份時點報告（04-15 架構審視、02 月最佳化報告／行動計畫、部署缺口／教訓、Hermes 遷移計畫）移 `docs/reports/`，3 份 CI 文件（Actions 03-09 停用）封存 `docs/archived/2026-09-ci-disabled/`，37 個引用檔連結重寫、殘留 0；`docs/archived/`（5 檔）併入 `docs/archived/`。其餘 42 份多為仍有效的指南，90 天未動不等於過期，未動 | ✅ |
| 09-05 持續優化④已修 | ①日誌降噪第二版與根路徵靜音：10 分鐘 1,243 → 119 行（−90%），CK_AaaP 以 Loki 複驗 219 對 219 一致、移除 KNOWN_DOMINANT 登記；②費用分組表每頁 20 組；③手機點擊目標地板（<768px 小尺寸分頁鈕／按鈕／小輸入框墊到 32px）：weekly 111 smallTap 262 → **59**，閘門 GREEN；基線同步。剩餘 59 多為表格展開圖示與 44×24 小輸入框 | ✅ |
| 09-05 持續優化③已修 | ①視覺走查桌面圖抓到 09-05 引入 FilterBar 後的**桌面回歸**：Row 放進 Space 的 inline-flex item 寬度塌成內容寬，承攬案／PM／專案帳款三頁的年度／類別／狀態下拉只剩 40px、選中值看不到，而 weekly 109／110 都不會紅——改 flex 版面；weekly 111 探針加 `narrowSelect`（<80px）並在 390 與 1440 兩寬度都跑（負向對照：部署前 3＋3，部署後 0；行事曆表頭「9月」選擇器排除）。②費用列表手機卡片（134 個小點擊目標全在表格列內；點卡片展開明細）、分組表每頁 20 組（此前不分頁、手機整頁 23,000px）、對不到主檔的舊案號費用改標「未對應案件＋案號」不再與「未歸屬」混（A104 兩筆）。③手機字級地板後基線 tinyFont 327→0、smallTap 392→262。④L142／L143 入冊（與 CK_AaaP#L51／#L37 互引） | ✅ |
| 09-05 持續優化②已修（跨 session） | CK_AaaP 量到本容器每請求記 3 行（START＋END＋uvicorn access，全 INFO 帶 Details），使用中 14,000–20,000 行/h、一度佔全平台日誌 53.8%，drift §54 擋下他們的 push（暫登記 `B-MISSIVE-LOG-PER-REQUEST-FANOUT`，到期 2026-10-05）。本地複量 10 分鐘 1,243 行、START/END 694。修：START 降 DEBUG；health／metrics／unread-count／auth-me／csrf-token 的 2xx 不記 END 與 access log（非 2xx 照記）；連線池建立降 DEBUG。REQUEST_END 一行含 method／path／status／耗時，可觀測性不減 | ✅ 複量：10 分鐘 1,243 → 204 行（−84%），每請求 3 行 → 2 行，health／metrics 0；第一版「降 DEBUG」在 stdout 上沒降（logger 設 DEBUG 且傳播到 root），第二版改只在 LOG_LEVEL=DEBUG 才記；再把 Blackbox 每 15 秒探的 `GET /` 納入靜音 |
| 09-05 持續優化①已修 | /loop「請持續優化」：①手機字級地板（<768px 把 inline 10px 文字升 12px，/dashboard 299 處、/calendar 28 處）；②`/pm/cases` 表頭補年度／類別／承攬狀態漏斗與工具列下拉共用狀態（§2.6 ④，A98 少一頁）；③A103 的「Descriptions 固定欄數 18 處」實查全是 `column={1}`、多欄的 40 處早就是斷點物件 ⇒ 該項不成立、關閉 | ✅ |
| 09-05 第三十五輪已修 | owner「統整更新文件與設定確認版次一致；加強視覺檢核機制確保 RWD 正確性」。版本 v6.73（CLAUDE.md SSOT、CHANGELOG 對照表）。新 weekly 111 `rwd_mobile_quality_gate`：host 以 adapter 簽憑證、Playwright 390px 登入量五指標（截字／字級<11px／點擊目標<28px／fixed 遮蔽 ≥40%／統計卡獨列）。首跑 39 頁：遮蔽 1（`/erp/operational` 浮動鈕壓住分頁鈕 75% ⇒ 手機內容區底部留白 88px）、統計卡獨列 4 頁（財務儀表板帳齡、報價單詳情、報表兩分頁 ⇒ `xs={12}`）、截字 0、字級<11px 327（/dashboard 10px Tag）、點擊目標<28px 399（AntD small 控制項）走基線。⚠️ 首版把外層容器 col 算成統計卡，行事曆被誤報。視覺走查 `--click=` 先點再拍。⚠️ `.shared-selfaudit/ui_visual_walk.cjs` 是 vendored，`--click` 尚未回流 shared-modules ＝ A106 | ✅ |
| 09-05 第三十四輪已修 | owner「感覺非常多案例皆有此問題，如何複查與排除」。全庫普查名稱／鍵成對欄位（總表入 `FIELD_SEMANTICS.md`「名稱／鍵成對欄位總表與排除 SOP」）：公文 sender／receiver 無鍵 92＋94 全可精確對上機關主檔 ⇒ 回填；帳本應付分錄廠商鍵 5 筆可由來源推導 ⇒ 回填；PM #207 用地科第二筆補鍵 78；承攬案兩把鍵（舊 `client_agency_id` 34 筆 vs 新 `client_vendor_id`）實查 0 筆不一致。weekly 107 擴到公文兩欄（漂移判準只認「原文與主檔名毫無交集」，否則 519 筆原文附註天天黃）＋兩條推導鍵；現況 RED 0、YELLOW 12 要人判（張啟良／顏新元／竣吉／林晉廷／銢欣 ×7、公文「張坤樹代表」×4、#361 桃園市政府→工務局）。排除三層＝寫入端補鍵／讀取端以鍵為準／存量只回填精確相等者 | ✅ |
| 09-05 第三十三輪已修 | owner「`/contract-cases` 手機登入無法進行篩選，功能遮蔽無法顯示」「桃園 1 案 64,800 另列一列，前端已改工務局仍顯示用地科」。①390px 實測（臨時 Vite 免認證＋Playwright）：篩選列可展開、下拉可開，機制沒壞——但切換鈕只有兩個小圖示、看不出是篩選入口，且手機是卡片模式**沒有表頭可點排序**。改：鈕加「篩選」文字、有篩選時主色；手機篩選列加排序下拉（承攬金額／年度／成案案號／狀態，走後端 `sort_by`）。②CK2025_PM_02_133 三表只有名稱快照「桃園市政府工務局用地科」、`client_vendor_id` 全空 ⇒ 帳款腿 2 用名稱分組就另起一列；owner 改名沒落到任何一表。資料：PM 193／承攬案 169 補鍵 78、名稱改主檔名；程式：腿 2 改**先用主檔鍵分組**（鍵是關聯、名稱是快照），明細承攬案用鍵或名稱都算這家。修後桃園一列：2026 二案、2025 三案（含 64,800）。全庫承攬案無鍵剩 0、PM 剩 1 | ✅ |
| 09-05 第三十二輪已修 | owner「`/erp/client-accounts` 並無依年度篩選統計：桃園 2026 應僅 2 件委辦案件」「搜尋提示仍寫代碼＝統一編號」。真因兩層：①年度只掛在報價單金額子查詢，案件數與案件清單沒跟著篩（桃園任何年度都 7 案）；②口徑用 `erp_quotations.year`，而它是**補建那年**（14/277 張成案報價單與案號年不同，桃園 CK2023 案的報價單 year=2026）。裁示：案件類統計年度一律＝案號年（`repositories/erp/case_year.py` 唯一實作），委託單位帳款／廠商帳款／帳齡／財務摘要一覽全改；同時抓到「承攬金額（含稅）」卡片加的是報價總價（差 377 萬）＋含了未成案報價單（差 1,271）＋明細沒排除已刪報價單。修後 2026 委託單位帳款合計 83,099,209.5 ＝ `/erp/quotations` 承攬金額，桃園 2026＝2 案 33,235,000＝依類別統計。搜尋改可用統一編號（前後端）。⚠️ 桃園在列表出現兩列（另一列 1 案 64,800）＝承攬案的委託單位名稱快照與主檔不同，屬 A99 | ✅ |
| 09-05 第三十一輪已修 | owner「專案財務統計：依計畫類別 01 委辦招標／02 承攬報價，統計各機關與協力廠商應收付帳款」。新端點 `financial-summary/by-category`（口徑同專案帳款：成案報價單、案號年度、類別＝案號段、承攬金額＝議價→契約→報價總價、委託單位以主檔鍵為準），儀表板新區塊：年度／類別切換＋四張合計卡＋兩張表（機關應收：案數／承攬金額／已請款／已收／未收；協力廠商應付：應付／已付／未付），手機改卡片，點列進專案帳款（帶年度＋類別＋委託單位）或廠商明細。2026 實測 01 類 6 機關承攬 62,134,210、未收 18,650,750；應付 4 家 5,159,000 全未付。⚠️ 寫 SQL 又踩 `(?:` 冒號被 text() 當參數（同 `login_history.py:178`），改 `\:` | ✅ |
| 09-05 第三十輪已修 | owner「①是否仍有相同架構與問題 ②全面檢視 `/erp/financial-dashboard` 數據來源並重新設計，預算使用率排行對財務主管無意義」。①同族複查：四張雙鍵表（pm_cases／erp_quotations／contract_projects 的 `case_code`⇔`project_code`、指派表）單邊 **0**；附件孤兒 0；`expense_invoices` 舊案號孤兒 2（A104）。②來源盤點：帳本總覽＝`finance_ledgers`（現金口徑：入帳收款／已付應付＋核銷）、專案一覽＝帳本＋報價單四欄、月趨勢＝帳本、**預算使用率＝帳本支出÷收入**（名不副實）、帳齡＝請款日／到期日分桶。重設計：頂列「財務主管指標」六張可點卡（承攬金額／已請款／已收／應收未收／逾期>30 天／應付未付，與專案帳款同口徑同年度）；帳本列標明現金口徑；預算使用率換「委託單位未收集中度 Top 10」（點長條進明細）；利潤排名改名「案件實際損益（帳本實收−實付）」；`/erp/quotations` 讀 URL 參數承接 KPI 點擊 | ✅ |
...
| **A107 `/api/document-numbers` 後端 router 廢止候選** | 前端 `/document-numbers` 頁走 `documentsApi.getNextSendNumber`，該 router（sequences／generate／next-number）前端零呼叫、72 小時 access log 零流量、Hermes skill 無引用。依 `zero_traffic_is_not_dead`（11 個零流量端點核實後沒有一個該刪）不自行刪，要 owner 確認有沒有外部呼叫者（LINE／排程）。**09-05 owner 裁示刪除**：兩支 router（607 行）與 routes.py 掛載一併移除，schema `document_number.py` 因 stats.py 仍用而保留 | ✅ 已刪 |
| **A108 `pages/DocumentCreatePage.tsx` 孤兒** | Send／Receive 兩個建文頁已取代，只剩 `__tests__` 在 import；送出只印 log 不呼叫 API。**09-05 owner 裁示刪除**：頁面 221 行＋測試 189 行一併移除 | ✅ 已刪 |
| **A109 整頁載入 layout 級 7 支 API 合併（跨站共同題）** | csrf-token／auth-check／auth-me／navigation-action／unread-count／filing-gaps-mine／calendar-events 每次整頁載入固定打，colo=SJC 下每支 ~0.55s；lvrland 6 支同型。lvrland 提議由 AaaP 收集五站清單一起送裁，**不各站各做一套**；本 repo 清單已送 AaaP，等平台決策（CF Pro/Argo／split-horizon）與合併方案 | 待平台 |
...
| 09-04 第二十六輪已修 | owner「01 委辦招標一定有議價程序；02 承攬報價議價金額應為「—」，非零」。承攬案分頁：01 類才給填議價金額、未填標「尚未議價」；02 類顯示「—」不給填。服務層：非 01 類的議價金額一律清空（改類別時也清）。weekly 104 加 ⑫ 01 執行中未議價（YELLOW，首跑 3）／⑬ 非 01 類有議價（RED，0）。存量：#200 清空、#36 議價→契約 | ✅ |
| **A102 01 委辦招標執行中尚未填議價金額 3 案** | CK2026_01_01_003（契約 空）、CK2026_01_01_002（契約 7650000）、CK2026_01_01_001（契約 0）——決標後請在承攬案頁填實際承攬金額；沒填之前承攬金額＝契約金額 | 🟡 待 owner 填 |
| 09-04 第二十五輪已修 | owner「`/contract-cases/194` 實際費用為議價而非契約金額」。三個名詞定死：**契約金額**（`contract_amount`＝成案時報價）／**議價金額**（`winning_amount`＝決標後實際）／**承攬金額**＝議價無則契約，應收面一律用承攬金額：報價單列表欄顯示承攬金額並標「議價」與原契約、承攬案卡改「承攬金額合計」、應收總額（未稅）依承攬金額換算、成案即應收第一期＝承攬金額、weekly 104 ① 對承攬金額。存量：12/285 有議價、3 筆與契約不同（194：625,000→596,000；65：1,300,000→1,260,000；200 填 0）；194／65 系統自建的第一期請款已依議價更正 | ✅ |
| A101 #200 議價金額填 0 | owner 09-04 晚裁示「02 承攬報價議價金額應為「—」非零」⇒ #200 清空；#36（02 類、只有議價 2,186,100 沒契約）議價→契約金額 | ✅ 依裁示辦畢 |
| 09-04 第二十四輪已修 | owner「各類經費名詞或定義請增列註記補充」。建 UI 端 SSOT `financeTerms.tsx`（21 個名詞：顯示文字＋定義，含稅／未稅、來源欄位、與其他名詞關係），統計卡與欄位標題改 `termTitle(key)`＝文字＋ⓘ；套到專案帳款、承攬案、委託單位帳款（列表＋明細）、廠商帳款（列表＋明細）、報價單詳情共 7 頁 41 處。順手抓到報價單詳情「應收總額」其實是含稅總價、與列表頁「應收總額（未稅）」同名不同數 ⇒ 改「報價總價（含稅）」。文件鏡像＝FIELD_SEMANTICS「經費名詞字典」（由 TSX 產生） | ✅ |
| 09-04 第二十三輪已修 | owner「`/contract-cases` 合約總額與 `/erp/quotations` 應收總額為何不一致；合約總額是議價金額還是契約金額；文字是否統一為應收總額」。量了：2026 契約金額合計 81,710,000（116 案）vs 應收總額 63,785,409（115 案）＝稅 989,591＋1 案 `year` 欄≠案號年（#21 開口契約 16,935,000）。合約總額＝議價金額＝**契約金額（含稅）**，三名一數；文字統一「契約金額」，卡片標「契約金額合計（含稅）」；不改成「應收總額」（那是未稅口徑）。對帳表入 FIELD_SEMANTICS | ✅ |
| A100 承攬案 `year` 欄 vs 案號年 | #21 `CK2025_01_03_001`：owner 09-04 23:10 已在承攬案頁把 `year` 改為 2025（audit_logs），兩頁年度歸屬一致；2026 契約金額合計因此由 81,710,000 降為 64,775,000（承攬金額合計 65,306,000＝再計入 194／65 議價與 191 只有議價金額） | ✅ owner 已辦 |
| 09-04 第二十二輪已修 | owner「已有匯入匯出彙整表則刪下載空白範本」「統計卡改 應收總額(未稅)／應收未收／應付款項／成本總額，毛利先隱藏（尚未統一計算方式）」。範本鈕移除；摘要加 `total_payable`（應付合計）；卡片依序重排、毛利卡隱藏（`statFilter` 邏輯保留，口徑統一後放回）。順手發現這四張卡**此前只換底色不篩列表**（同 `/contract-cases` 第十五輪）⇒ 加 `card` 參數：有未收／有應付／有成本拆解，容器對真值 104／3／0 全對、非法值 422 | ✅ |
| 09-04 第二十一輪已修 | owner「`/erp/quotations` 計畫類別、委託單位皆無法正常篩選」「批次匯入回簽 PDF 如何執行，如無法對應所有案件則刪除」。①**真因在 api 層**：`api/erp/quotationsApi.list` 逐一白名單只放行 search／year／status／case_code，頁面送的 `category`／`client_name`／`case_status`／`include_unawarded` 在這一層全被丟掉——後端逐家驗過、頁面 tsc 也過，就是沒驗中間這一層（verified_wrong_layer 第四次：**改了兩端，中間有一層各自的白名單**）。改為全部透傳，新增參數不必再登記。②批次回簽此前只認檔名裡的舊案號（277 張只有 246 張有、QT 號只有 139 張有），改三把鑰匙任一：舊案號／QT 號／案號（含舊制成案編號），每張報價單都有 case_code ⇒ 任何案件都對得到；容器預覽對照 6 檔 5 對 1 拒（無編號）。保留、不刪 | ✅ |
| 09-04 第二十輪已修 | owner「`/erp/quotations` 表格標頭無篩選排序機制（這不是規範要求嗎）」——§2.6 ④ 第十五輪只做了 `/contract-cases`。補：年度（排序＋漏斗）、成案編號（排序＋類別漏斗）、委託單位（漏斗、可搜尋、選項＝案件實際客戶）、專案名稱（排序）、議價金額（排序＝報價總價）；漏斗不帶 `onFilter`，勾選值進 `params`，與工具列下拉共用同一份 | ✅ |
...
> 09-08 21:27 開機後 `ck-pilemgmt-25` 通報（**轉述、未經本 repo 查證**）：`PM2_Autostart` 這次回 **0**，但 pm2.log 顯示 resurrect 實際恢復 **0/13**，13 分鐘後由 Website PostBootGuard 補齊；AaaP 側三支平台服務同樣沒隨開機 resurrect（21:42 人工拉起）。⇒ 只新增一項確定的事：**退出碼（255 或 0）都不能當恢復成功的訊號**。(a)/(b) 仍無證據分辨，「機制未確認」的表述保留。
...
| **A63-P2** | 移除 `config/` 目錄與兩份非權威複本 | **09-15** | ✅ 09-06 已辦（owner /loop「整理專案資料夾並刪除散亂資料」）：`backup/utils.py` 改指 `paths.REMOTE_BACKUP_CONFIG`，`config/` 與 `configs/remote_backup.json` 刪除，weekly 96 GREEN | — |
...
### A115 — 2026-09-07 待 owner 裁示（收斂覆盤見 `CONSOLIDATION_20260907.md`；下一輪四項在其第六節）

當日所有修法的完整性複查結果見下；**除這四項外，逐條都已驗到綠**。

| # | 事項 | 現況 | 為什麼要 owner |
|---|---|---|---|
| ① | ~~賴秀玲的權限雙向漂移~~ | **09-07 已辦**：finance 角色先補齊 5 個基本讀取權，再同步（41→21 項）；`is_admin` 旗標依 owner 裁示清除（李昭德同）。收斂 A 後此類漂移在正規路徑上不再可能 | ✅ |
| ② | **業務同仁現有委託／協力帳款權限** | 角色層 staff 兩個新碼都是 true | 與 09-07 早先「業務同仁僅需看到專案帳款」不一致；是我拆碼**之後**被勾上的，未自行更動 |
| ③ | ~~總表 2 筆「未標成立、系統已成案」~~ | **owner 2026-09-07 裁示：已開發票者為總表漏填 v** ⇒ 系統不動，總表補 v 後 weekly 120 自動歸零 | ✅ 已裁示 |
| ④ | ~~`CK2026_PM_02_075` 的 q640~~ | **owner 2026-09-07 裁示：未成案先保留** ⇒ 不刪除，維持現狀 | ✅ 已裁示 |

另有兩項是**資料本身的缺口**，不需決策但要有人補：

* 總表第 59 列（台86線⋯「二量」2,186,100）仍在檔案裡，尚未存檔刪除 —— 系統無此紀錄
* 6 筆請款留白**且報價單也沒有日期** ⇒ 稽催的時間錨點是空的，催不到
...
| A122 | **9 個公文附件檔名超過 Linux 255 bytes** | 08-29 上傳端已封頂 200 bytes，但 2–4 月的 9 個舊檔（doc_848／885／2366／2445／2446）容器讀取 I/O error ⇒ 下載 404、備份 Errno 5 | ✅ 09-08 改名至 ≤200 bytes 並更新 `file_path`（原始檔名仍在 `file_name`）；容器 find 0 錯誤、`/api/files/149/download` 200。**09-09 補**：`backups/attachments/attachments_backup_20260303–0308`（6 個、共 97 MB）裡還有改名前的 doc_885 舊檔，容器每晚清理都 `Errno 5` 失敗（02:00 備份日誌 6 筆 warning、退出仍 ok）；主機端刪除被權限分類器擋下，**留 owner 用檔案總管刪或授權**。它們早已超過 7 天保留期 | 待 owner（刪 6 個舊備份目錄） |
...
| A127 | **backend 假活 44 分鐘（09-08 22:27–23:11）：行程活著、埠綁著、accept backlog 422、主執行緒 100% CPU、公網 000** | py-spy ×4＋gdb 原生堆疊一致：`/metrics` 第一次抓取在事件迴圈裡做延遲 import（`prometheus_middleware.metrics_handler:236` → `shadow_baseline_metrics.py:28 import sqlite3` → `from _sqlite3 import *`），C 層 `PyImport_ImportModuleLevelObject` → 靜態函式 → `_PyUnicode_EqualToASCIIString` 迴圈空轉；`/proc/1/maps` 無任何 sqlite .so（未進 dlopen）。utime ≈ 整輪存活 ⇒ 啟動完成後第一個 scrape 就卡，**這一輪從未服務過任何請求**。同容器新程序 `import sqlite3` 正常 ⇒ 是該行程狀態壞了（疑 C 層 NULL 終止表被踩，**未判型**），不是映像／ld.so。NER「發現 50 筆」是巧合（前一輪同批 22:22 正常收尾）。docker healthcheck 判 unhealthy 57 連敗但 `restart: always` 不看 health ⇒ 不會自癒；告警投遞目標本身就是 missive（Website 08-29 拓樸預測，依 Website 的計數今天是**第二、三個**空窗，第一個是 9/02；兩邊計數以 Website `RESTART-RECOVERY-SOP.md` 為準，其 P1-4 待 owner 拍板第二條投遞路徑）。**22:26 那次重啟來源已查明＝行程 segfault ＋ `restart: always`，不是人為**：dmesg（整台 WSL2 VM 共用）今晚 72 筆 Python 行程 segfault（uvicorn 23／python 11／celery 3／hermes 1），其中 21:48:53／21:55:43／22:20:37／22:20:46／22:26:04 五筆 uvicorn 段錯誤各自對上本容器 21:50:30／21:57:27／22:20:52／22:21:51／22:27:21 的「應用程式已啟動」（每次差 1–2 分鐘＝崩潰→重啟→啟動）。段錯誤位址多為 `at 0／4／8／10／aa／ab`（空指標小偏移＝Python 物件指標被踩），全部落在 libpython 內、22 個相異 offset（DT 統計）。**libpython 磁碟原檔＝page cache**（O_DIRECT 讀 sha256 與快取讀一致、與四個 repo 的一致）⇒ 壞的不是共享唯讀頁，是各行程的**私有記憶體**（heap／.data COW 頁）。爆發時段 21:31–21:37、22:02–22:26 兩波，22:26 後至今無新筆（陣發）。⇒ 本案「假活」與這 72 筆是同一晚同一台機器的同一件事：**程式碼層找不到兇手，先看 dmesg**。⚠️ **RestartCount=7 是 09-08 23:00 的觀測值**；09-09 00:13 與 00:46 兩次部署 recreate 容器後計數歸零（`docker restart` 不歸零、recreate 歸零，DT 實測），之後拿 `RestartCount` 比對要連 container id／`StartedAt` 一起看。 | ✅ 23:10 `docker restart` 恢復（health／metrics／公網 200）。②④ **09-09 已辦**（owner「依建議事項逐一辦理」）：②`prometheus_middleware.prewarm_lazy_imports()` 在 lifespan 啟動期先載 psutil／shadow_baseline_metrics／tender.metrics，handler 裡的 import 變成 sys.modules 命中；④`app/core/liveness_sentinel.py` 獨立執行緒每 30s 打 `127.0.0.1:{BACKEND_PORT}/health`，**只有無回應算失敗**（503 算活著，DB 掛了不會無限重啟），連續 3 次即 `os._exit(3)` 交給 `restart: always`；6 支單元測試鎖判準。①因此對「迴圈卡住」這型已不需要 autoheal；其他型（行程活著但 health 503）仍不自癒，屬 owner 決。③取證法已入 memory `backend_wedged_pyspy_sidecar.md`。⚠️ **跨 repo 同族訊號（未判型，只登記）**：同一台主機、同一晚非預期關機後，DT 的 `ck-tunnel-celery-exporter-1` 反覆 `SystemError: unknown opcode 119`（DT 待辦 B5，另有測試行程 `isinstance` 回錯答案），與本案 CPython C 層 import 空轉都是「行程記憶體像被踩」的形狀；本機**從未跑過記憶體診斷**（A66-P3 `mdsched`，owner 仍未做）——加上 dmesg 72 筆跨四個 repo 的 segfault，把 A66-P3 提前到**下一次可停機時立刻做**；在那之前任何一個 Python 行程都可能再倒（含寫 DB 的 backend，資料層目前無 segfault 紀錄）。這仍是假說，證據要等 mdsched。**跨來源補證（Website／DT，23:40）**：①Windows 事件日誌崩潰前 55 分鐘有 8 個互不相關的行程接連當機（19:17 ChatGPT／Explorer、19:21 LINE／WebView2、19:22 與 20:00 dwm、20:01 SearchHost／crashpad_handler、20:05 powershell；19 時 13 筆、20 時 8 筆），與重啟後 dmesg 的 Python 風暴是同一形狀、來源獨立——**dwm／Explorer 不跑 CPython，這一條把「CPython use-after-free」的軟體假說排在硬體之後**；②**真實死亡時刻 ≈ 20:12**（事件 6008 的 19:47 是最後一次乾淨旗標寫入，落後 25 分鐘；最後一筆事件 20:12:11）；③**反向證據**：21:27:14 事件 1101/1201「Windows 記憶體診斷未偵測到問題」（開機自動短測）、WHEA 近 7 天 0 筆——它們排除的是嚴重故障，不排除間歇性（非 ECC 不會進 WHEA）⇒ **owner 跑完整 mdsched 若 PASS，仍不能當作問題消失**。**00:10 Website 補證**：`MemDiag.bin` 建立於 21:27:14、WER 參數 `Standard`／`Pass`／4440（若為秒＝74 分鐘，恰好填滿 20:12 崩潰到 21:27 開機的空白）⇒ 那次不是短測、是開機前完整跑過一輪 Standard；**再跑 Standard 幾乎沒有新資訊，建議改跑 mdsched Extended 或 memtest86+ 整夜**。另一個不對稱：開機後 2.5 小時 Windows 側當機 0 筆、WSL 側 72 筆 ⇒ 若是整條 DIMM 壞，兩側都該中；較能解釋的是「壞的實體區域這次被分配給 WSL VM」或「WSL 側另有原因」，未判型。lvrland backend 21:41–22:17 重啟 22→42 落在同一窗口；**01:16 Website 兩個資料點**：①01:10:53–01:15:56 Windows Defender 排程掃描期間 ck-hermes-web／sw-worker 探針同時逾時（TimeoutError，連得上但慢）⇒ **防毒掃描會製造與「不穩定復發」一樣形狀的健康翻轉**，看容器翻轉要先對 Defender 事件 1000/1001；②WSL VM 工作集 24.5 GB 貼著 docker 上限 23.47 GiB（主機 64 GB 可用 19 GB）⇒「balloon 回收」那條在貼著上限的 VM 上更合理，但記憶體不足的簽名是 OOM／137 不是空指標，只支持「WSL 側另有原因」的方向。開機後 4 小時 Windows 側 0 筆、容器重啟自 00:50 未變，48 小時不復發則硬體解釋可信度下降——**02:44 更正：復發了**。dmesg 新增三筆：17641s（02:20:59）`python` general protection fault in libpython3.11、18384s（02:33:22）`hermes` segfault in python3.13、18429s（02:34:07）`uvicorn` segfault `at 4` in libpython3.11（對上 lvrland webmap backend 02:34:59 重啟、RestartCount 42→47）。安靜期 22:48–02:20 共 3.5 小時，第三個爆發跨三個 repo 的三種行程，與前兩波同型（空指標小偏移）；Missive 容器本身未中（02:26 部署後 startup 1 次）。⇒ 「負載下未複現」那條撤回，**mdsched Extended／memtest86+ 的優先級回到最高**；**Website 02:5x 補證**：第三波期間 Windows 側當機 0／WHEA 0／System 錯誤 0，Defender 只有 02:27 的定義更新（1150/1151）不是掃描——**第三波完全在 WSL 側**，與第一波（19:17–20:12 全在 Windows 側）恰好相反，任何解釋要同時容納兩者；其 docker events 與 dmesg 逐筆對上（hermes-web die 02:34:11 ↔ segfault 02:33:22；lvrland die 02:34:58 ↔ segfault 02:34:07，49–51 秒是核心層先記、容器等 PID 1 結束才發 die）。⚠️ `ck-hermes-web` 那次 ExitCode=0 而 dmesg 有 segfault ⇒ 段錯誤在子行程、PID 1 乾淨退出——**以容器退出碼判「有沒有崩潰」會漏掉這一類**（L127 家族；以容器計今晚 1 例、以行程計至少 3 例——hermes-web／lvrland／02:20 GPF 對應的重啟，五個有重啟的容器 ExitCode 全是 0。另：`RestartCount` 只計 restart policy 觸發的重啟、人為 restart 不加一，所以它每增加一次在定義上就是非預期退出，比 ExitCode 可靠）；**AaaP 02:4x 壓力取樣**：第三波實為 7 筆（17557–18429s，前 5 筆在 84 秒內），同時 WSL VM SwapFree 1.41GB→0.16GB、pswpout +671k 頁、allocstall 583→1,001——**故障與記憶體壓力同時動**，且假說先於觀測寫下；lvrland 對新 7 筆做位址分類：NULL 系 75%、5/7 落在已知熱點（`0xb1f5c` 佔 4）。⇒ 新的候選解釋：**不是 RAM 本體，是 WSL2 的 swap 路徑**（換出再換入的頁面內容被改）——它同時容納「Windows 側乾淨、WSL 側中招」與「三波都落在高負載時段（開機全起／22:0x 多 session 開工／02:20 Missive 全量部署建置）」，也解釋為什麼開機 Standard 記憶體測試 PASS。**便宜的判別實驗（owner）**：`%UserProfile%\.wslconfig` 設 `swap=0`（或把 swap 移到另一顆磁碟）後 `wsl --shutdown` 重開，再跑同樣負載（Missive 全量部署＋pytest）看 dmesg 有沒有新筆；若歸零＝swap 路徑（磁碟／vhdx），若照舊＝回到 RAM（memtest86+）。L130 記過 docker_data 磁碟 e2fsck 有殘留 inode，同一顆磁碟的 swap.vhdx 值得先看；**03:17 續**：20592s（03:10:10）`python[19169]` general protection fault **in libc.so.6**（不是 libpython），對上 DT `ck-tunnel-celery-exporter-1` 03:10:59 重啟（restarts 6、ExitCode 0＝子行程崩潰型）。當時 VM 內 MemAvailable 16 GB 但 SwapFree 僅 318 MB／4 GB、pswpout 累計 1.76M 頁——**記憶體早已不緊，但 02:4x 被換出的頁還躺在 swap 裡，之後任何時候換回都可能踩到**，與 swap 路徑假說相容、與「當下壓力」不相關；**AaaP 03:2x 補換入側**：pswpout 02:4x→03:2x **完全沒動**（1,762,577）、pswpin +383k 頁、SwapFree 回升 0.16→0.32GB ⇒ 03:10 那筆發生時**只有換入在動**。因此兩個方案優劣不是成本差：調高 `memory=` 對已在 swap 裡的頁**無效**，`swap=0` 是唯一針對已觀察機制的方案。AaaP 保留三點：未把單次換入對上單次故障／`pgmajfault` 同期 +34,748 含檔案缺頁／這筆在 libc 而非 libpython、樣式變了。dmesg 環形緩衝已輪替掉部分早期筆數（現存 63），AaaP 帳本 264 筆是唯一不掉的；**05:5x 意外對照實驗（Missive 部署）**：一次完整 docker build＋換容器製造 **13,836 頁／3 分鐘＝約 77 頁/秒**（高於爆發期的 57）、SwapFree 由 5 MB 降到 **0**、PSI some avg60 峰值 0.46 ⇒ **新故障 0 筆**（最後一筆仍是 04:28:34，已安靜 82 分鐘）。配合 AaaP 03:2x–05:4x 的觀察（swap 比爆發期更滿而故障停了）⇒ **空間耗盡不觸發、單純的高換頁速率也不觸發**；爆發期真正不同的變因尚未指認。**候選之一（ollama 反覆重載）已被四天資料劃掉**（06:0x）：`docker logs --tail 200000 --timestamps ck-ollama` 抓 `loading model tensors` 得 566 筆／09-05 起，逐小時對照故障數：21時 8載入/0故障｜**22時 6/37**｜23時 3/0｜00時 5/0｜01時 5/0｜02時 6/7｜03時 5/2｜**04時 18/4**｜05時 8/0 ⇒ 故障最密的一小時載入最少、載入最多的一小時只有 4 筆；且 04:28:34 最後一筆之後 13 分鐘內連續 12 次載入、零故障。**這個 churn 已連續四天（每小時 2–31 次），是背景常數不是變因。**（09-08 20 時整小時 0 載入＝崩潰到開機的離線窗，與 Website 量的真實死亡時刻互相佐證。）剩餘候選：叢發持續時間（3 小時 vs 3 分鐘）、同時換頁的行程數。**majflt 基準已建**（`docs/health/majflt_baseline_20260909.json` ＋ README 含採樣指令；`--pid=host` 不可省）。**06:05:32 復發一筆**（`uvicorn[22138]: segfault at 0`，安靜 97 分鐘後；Missive 容器隨之重啟一次、0.6 秒空窗、公網 502 約 30 秒後恢復）。當下 121 秒的 majflt 差分：合計 +5,054（41.8 次/秒），**containerd 一支就佔 +3,516（70%）**、minio +768、其餘皆 <250 ⇒ 主要缺頁由容器執行期主導，不是業務服務。**另修一筆已證實的成本**（Website 從我們的 log 找到作用點）：啟動 warm-up 三個模型，而我們預熱的 `qwen2.5:7b` 與 Hermes 常駐的 `qwen2.5:7b-ctx64k` **是兩個不同的模型**、且啟動後不久就被 TTL 釋放 ⇒ 拆出 `WARMUP_MODELS`，預設只預熱嵌入模型（`OLLAMA_WARMUP_MODELS=all` 可還原）。⚠️ 這不是因果修法，因果未證明。**06:1x Website 自撤**：他原本的「重啟→預熱→記憶體壓力重建」正回饋在最關鍵一環是錯的——`docker stats` 10.36 GiB 裡不可回收的匿名記憶體僅約 1.4 GB、其餘是可回收 mmap，`qwen2.5:7b` 是 100% GPU，且當下 MemAvailable 16,468 MB/24,611 MB＝**沒有壓力**。改動仍成立，但**理由是「效益未證實」不是「省記憶體」**；三處引用「約 5 GB」的文字已訂正。⇒ 兩個各自的錯誤剛好指向同一方向時，說服力會被高估。⚠️ 方法論：**單一時窗的差異不是差異，要看它在沒出事的時候是不是也長這樣**——我把「爆發期在重載、部署時沒重載」順手當成差異，拉四天才發現它是常數。⚠️ 82 分鐘 < 今晚最長安靜期 3.51 小時，不可結案。**06:5x 序列分析（41 筆，22:09:49–06:51:49）**：原始間隔 40 個 中位數 43s／平均 783s／標準差 2,197s／**變異係數 2.81**；以「>120s 分波」得 14 波（每波 [2,14,5,2,1,1,5,2,1,1,4,1,1,1]），波間隔 4/3/2/19/3/**211**/12/36/40/36/97/4/42 分 ⇒ 中位數 19 分、**變異係數 1.42** ⇒ **不是週期性的**（排程或計時器觸發的變異係數會接近 0）⇒ 排除「某個 cron 在敲」這一類，但**不能**分辨硬體與軟體。⚠️ **50 秒常數是條件成立的**：06:51:39.64 那次容器死亡**前面沒有任何核心故障**（最近一筆是 06:51:49 的 `alembic` GPF，比死亡晚 10 秒，且我們啟動不跑 alembic ⇒ 不是我們的、更不是死因）。三次正向 49/51/52 秒只在「核心層先記到 segfault」時有意義，**不可反推每次 die 都有對應 fault**；**DT 03:3x 找到可控的壓力源**：`ck-ollama` 02:58:25 載入 `qwen2.5:7b-ctx64k`（8.7 GB、27% 在 CPU 側）擠進 24 GB 的 VM，12 分鐘後就是 03:10 那筆；671k 頁≈2.6 GB 與該量級相符。DT 另有乾淨對照：兩次 47 分鐘後端全套件（重度 Python）期間 pswpout 完全沒動、零故障 ⇒ **重負載本身不觸發，重負載＋swap 壓力才會**；DT 的 B5（08-15 起、症狀都在跑測試套件時）對 swap 路徑假說是佐證而非反證。**Missive 查證：不是我們呼叫的**——shadow_trace 02:40–03:15 零筆、ai_connector 從未引用 ctx64k 變體；該模型是 Hermes 棧的主模型，ollama 顯示 keep-alive 一直被續（UNTIL 6 分鐘）＝有週期性呼叫者在 Hermes／Open WebUI 側（同網路容器只有 hermes-gateway／hermes-ops／open-webui），Hermes session 離線、交 AaaP 查。**不必重開機的先手**（DT 提議）：縮短 ollama keep-alive、或別在其他工作進行時載入大 context 模型，可在 `swap=0` 實驗之前先降壓。**DT 03:4x 找到呼叫者與機制鏈**：`ck-hermes-ops` 的 DA-6 smoke 每 **5 分 05 秒**打一次 ollama 推論端點，而 keep-alive 是 **5 分鐘** ⇒ 模型剛過期卸載、下一次探測又載回——不是靜態釘住 8.7 GB，是**週期性配置／釋放 8.7 GB**；09-08 當天 16 次載入事件、其中 3 筆 `loaded runners count=2`（兩個模型同時常駐），churn 集中在 02:14–02:27 與 02:58，整段落在 pswpout +671k 的窗口內。**⚠️ AaaP 03:5x 更正兩處**：`OLLAMA_KEEP_ALIVE=30m` 不是 5 分鐘；DA-6 smoke 本來就只打 `/api/tags`。真正重載模型的是 **DA-5 keep-warm**（`runbooks/hermes-stack/ops-sidecar.sh`，每 60 秒 tick 檢查 `/api/ps`，模型不在就 `POST /api/generate` 重載）——模型不在 GPU 上的原因不是到期，較可能是**被驅逐** ⇒ **迴路：記憶體壓力 → ollama 驅逐模型 → DA-5 在 60 秒內重載 8.7 GB（27% 在 CPU 側）→ 更多換出 → 更多壓力**（查證當下容器內 PID 2066 正是 re-warm）。降 churn 與保冷啟動延遲（DA-5 存在的理由：閒置卸載後冷啟動 >120 秒）是取捨不是 bug，AaaP 不代決，三個選項與代價入其台帳待 owner。**三個不必重開機的選項（Hermes／AaaP 手上，依更正後機制重列）**：①DA-5 重載前先看記憶體壓力（swap 使用量高就不重載）②拉長 DA-5 的 tick 或改成只在工作時段 keep-warm ③接受冷啟動延遲、關掉 keep-warm。另：open-webui 日誌的 `GET /.env 200` 不是外洩（SPA catch-all 回 text/html 首頁、0 行 KEY=VALUE）。**DT 04:0x 更正：那批探測是 AaaP 自己的 `scripts/check-vcs-exposure.py`**（四條路徑逐字相符、UA `Python-urllib/3.13`＝host Python、打 127.0.0.1:{port} 故容器看到 bridge gateway），且該腳本第 66 行早就知道 200 陷阱並比對內容——**沒有任何資安事件**；Missive 查證非本 repo 所為這條仍對。DT 記了兩個錯：n=1 抽到自己的 curl（168 筆 urllib 對 3 筆自己的）、把自家監控讀成威脅（行為特徵能認「是什麼動作」認不了「是誰的動作」）。頻率（AaaP 04:1x）：它不是常駐排程，唯一呼叫點是 check-doc-drift.sh §49（每日一次＋每次 git push 的 pre-push Stage 1）；今晚 42 輪是 AaaP 反覆推送與手動複驗造成，停下即回到一天一次。「頻率配合事實變化速度」這句 AaaP 已記為將來擋 5 分鐘排程的理由。AaaP 已登記 B-DOCKER-LOGS-FORWARD-READ-STALE 與 B-OLLAMA-REWARM-CHURN（7c963a4）。**Website 04:0x 把時間軸往前拉到 08-15**：DT 的 `ck-tunnel-celery-exporter-1`（04-19 建立、從未重建、映像 2025-07-15 建置的固定二進位）log 有 95 次 `Fatal Python error: _PyEval_EvalFrameDefault: Executing a cache`（位元組碼快取被當指令執行＝記憶體被改的同族）——04-19～08-14 **四個月 0 次**，08-15／16／17 突然 12／25／20，之後每日 1–7，今晚 4。**同一份二進位、同一個容器，軟體沒變而開始出錯。**唯一找到的環境變動：**08-11 裝了三個 Windows 更新**（KB5123304／KB5121003／KB5120708，2025-10-20 以來第一次），標為候選（4 天空窗未解釋、五個月只對上一件）；08-18 16:13 另有一次 Kernel-Power 41 非預期關機。反面約束：同機同期建立的 celery-beat 與 sw-worker 致命錯誤 0 ⇒ 不是全機一起壞，受害程度隨行程記憶體活動量。**對 Missive 的影響**：①**備份可信度待確認的範圍是 08-15 起**，NAS 30 份輪替裡 08-11～08-14 的 dump 是爆發前最後的乾淨參考（08-11 那份 09-10 就會被輪掉）——**09-09 04:1x 已把 08-14 那份（538,880,081 bytes）複製到 `missive_databsae/known_good_pre_20260815/`，SHA256 相符**，輪替不會碰它；要用它做 09-10 還原演練的話正好；②完整記憶體診斷若 PASS，下一個查 08-11 那三個更新，不是「問題不存在」；③DT 的 B5 起點 08-15 與此吻合。**04:30 第四波，這次 Missive 中了**：dmesg 23005s（03:50:23）python GPF libc、25174s（04:26:32）celery GPF、25220s（04:27:18）uvicorn segfault at a9、25264s（04:28:02）uvicorn segfault at 0、25296s（04:28:34）uvicorn segfault at a9；`ck_missive_backend` 04:28:09／04:29:17／04:29:46 三次重啟（RestartCount 1→4，各次停機 <1 秒、ExitCode 0、容器 log 無 faulthandler、哨兵未觸發＝核心層 segfault），04:29:25 起穩定、公網 200。當時 **SwapFree 只剩 22.8 MB／4 GB**（03:49 還有 330 MB）、pswpout 1.91M（+128k）、PSI full avg60=1.24；MemAvailable 仍 16 GB。觸發時段＝主機 SelfAudit-Flow 04:15（Playwright）＋ 04:30 Sweep 將起＋ctx64k 模型常駐。**今晚我自己的 weekly 全套／全量部署／pytest 也是負載來源之一，04:30 起本 session 不再啟動任何重活。**AaaP 04:3x 判斷（我同意）：**不停 keep-warm、不叫醒 owner 做這個決定**（已推播寫成「醒來後決定」）——急性事件已過（穩定 2 分 43 秒、PSI 退到 full avg10=0）；swap 耗盡≠OOM 迫近（MemAvailable 15.9 GB，pswpout 停、pswpin 在走）；**現在動 keep-warm 會汙染 swap=0 的判別實驗**（整夜的價值在於沒有一次同時動兩個變數）；取捨條件沒變。應急判準先寫下：同一容器 10 分鐘內重啟 ≥3 次且未回 healthy、或任一站點連續 2 次非 200，成立即動。AaaP 同樣停跑全套 pytest 與非必要 drift。全平台重啟 65 次裡 lvrland backend 佔 47，Missive 4 次是小的。⚠️ DT 順帶：hermes gateway 日誌印出被拒的 Telegram token 前段未遮罩，屬 AaaP／Hermes，owner 該知道；**Hermes bridge 已回 8/8**（Website 01:22 實測；中斷期間的 4/5 隨 Missive 恢復結案；weekly 28 看到的 CK-Hermes-Health-Smoke／DT-PostRestart 21:30 result=1 是開機 +3 分鐘的假紅、boot 觸發不自動重跑）；④DT 挖出 dmesg 前三名 offset 的機器碼：`mov rcx,[rax]`（obmalloc free-list pop）、`test [rax+0xa9],8`／`test [rax+0xab],0x10`（`tp_flags` 的 HAVE_VECTORCALL／UNICODE_SUBCLASS 位元），fault 位址 0x0／0xa9／0xaa／0xab 全是 **rax=NULL**——被踩的是型別指標，程式碼本身完整；40 筆裡 33 筆 fault 位址 <0x10000。DT 留的疑點：隨機位元翻轉不該一再打出乾淨的 NULL，較像釋放後讀取——但①的跨行程風暴無法用 CPython bug 解釋，兩者並存的解釋是「同一段壞掉的實體頁被不同行程輪流拿到」，仍待 mdsched。watchdog 已排除為 22:26 重啟來源（無 docker.sock 掛載、程式碼無 restart 路徑）。**且它是啞的**：`main.py:557` 的 Self-health watchdog 是跑在**同一個事件迴圈上的 coroutine**，迴圈被佔住時它連 `sleep(120)` 都醒不來 ⇒ 44 分鐘一行 lag 警告都沒有——**偵測「迴圈卡住」的哨兵不能住在那個迴圈裡**（同 L109「假的事件流」形狀）。修法＝獨立執行緒定時打 `127.0.0.1:8001/health`，逾時即 `os._exit(1)` 讓 `restart: always` 接手（零費用、不需 docker.sock）＝A127-④，一併等 owner | 等 owner |
| A128 | **四項流程複查（owner 09-09）：成案刪除無金流防呆／統計卡四處錯／表頭有排序沒篩選／RWD 三個盲區** | 全文＝`docs/architecture/FLOW_REVIEW_20260909.md`。**已修並推送**：成案刪除補金流足跡防呆（e01ee1f8，281/292 會被擋）＋weekly 100 反向孤兒偵測（首跑 4 筆）；`/agencies` 補 `serverPaged`（此前靜默給錯答案）、`/staff` 統計卡改後端全量、`/erp/expenses`「待審核」卡值與動作對齊、發票彙總分母不再自我歸零；weekly 116 從註冊起沒執行過（用錯直譯器）已修、weekly 82 退出碼顛倒＋誤報已修（9f67db95） | **待 owner 決**：①成案下架機制 —— **09-09 已出整體建議＝`docs/architecture/VOID_VS_REASSIGN_20260909.md`：建議不做註銷狀態，改做「金流轉掛」**（`ContractProject.status` 後端 92 處引用，加第三值＝92 處都要重判「要不要排除已註銷」，漏一處＝已註銷的案算進營收；而轉掛只改「這筆金流屬於哪個案」，92 處一個字都不用動，且能解掉既有 4 筆孤兒）。分三階段：D1 唯讀預覽（零風險、現在就有用）／D2 實際轉掛（單一交易＋審計，**不可逆、要 owner 授權**）／D3 解孤兒②表頭篩選（**09-09 已做第一、二批**：資產／營運帳目／委託單位帳款／協力廠商帳款／統一帳本，並收斂出共用工具 `utils/tableFilters.ts` 讓「哪一種要帶 onFilter」不再靠人記；剩 `/documents`、`/staff`、`/users`、`/vendors`、`/erp/invoices/summary-view`）②表頭篩選三批次的優先序 ③RWD 五項（手機橫向捲動表格 10 頁無卡片、768–991 無人看、統計卡網格 6 處、weekly 111 兩指標量了不判、smallTap 被雜訊主導）④既有 4 筆金流孤兒的處置（3 筆舊制案號回填、1 筆主檔不見要查）⑤promote 與報價單刪除沒有身分範圍檢查 | 等 owner |
| A124 | **報價單「總價是否含稅」線上無從指定**（owner 09-08：「前端也須同步建立管控填報機制，以利前後端經費統計」） | `tax_included` 此前只有匯入器寫得到（總表 K 欄）⇒ 線上建的每一張都被當未稅、工項小計一律 ×1.05；對單價已含稅的案那是**多算一次稅**，而小計／稅額／總計三個數彼此自洽、看不出錯 | ✅ 已辦：明細分頁加開關（`/erp/quotations/:id?tab=items` 與 `/pm/cases/:id?tab=quotations` 同一元件），切換後由後端**同一份算式**重算（`_apply_totals` 抽成唯一定義，`recompute_totals` 給切換用）；已有請款者仍被既有的鎖擋下。稅額列改為含稅時也顯示並標「稅內含」——原本 `tax > 0` 才印，含稅時整列消失，而那正是要看到的事 | 已結 |
| A125 | **異常案件標註與篩選**（owner 09-08：那 5 筆「已收 17,850 已付清、發票多開」） | 異常此前只有 weekly 104 每週看得到，使用者當下沒有入口，也沒有「判讀完就別再提醒我」的辦法 | ✅ 已辦：`finance_anomaly.py` 四條**推導式**判準（發票>請款／已收>請款／請款>合約／已收無發票），`/erp/quotations` 加「金流異常」欄與篩選（待判讀／全部），標籤可點開記錄判讀原因（必填）或退回待處理。**判讀解除的是待辦不是事實**——標籤仍在、轉灰標「已判讀」。實測：12 案有異常（那 5 筆在內），ack→11、unack→12。⚠️ 刻意不與 weekly 104 合併（逐案 vs 逐筆兩種粒度），改由 weekly 127 盯方向一致性。**09-09 owner「問題錯誤如何標注與顯示」**：首版只有列表欄的標籤＋滑鼠提示，手機卡片沒帶、詳情頁沒有、說明藏在 hover 裡。補：列表標籤下直接印差額、手機卡片帶標籤與差額列、詳情頁分頁上方 `AnomalyPanel`（每條異常：類型／差額／為什麼／判讀理由／記錄判讀・退回按鈕）、詳情端點同一份判準回 `anomalies`。現況 12 案 0 判讀 | 已結 |
| A126 | **發票買受人自由輸入**（owner 09-08：「發票抬頭及買受人統編是否對應 /clients 提供下拉選單，若無對應則可新增」） | 同協力廠商 08-18 那次的形狀：名字打不一樣就對不回主檔，而兩邊都不會報錯 | ✅ 已辦：共用元件 `InvoiceBuyerFields`（Modal 與獨立填報頁**同一份**），委託單位主檔下拉＋`dropdownRender` 就地新增（帶統編一起建）、挑主檔自動帶入統編；**不強制**只能從清單挑——買受人可能是個人（蔡蕙宇）或第三方，既有自由值標成「不在委託單位主檔」而不是顯示成壞掉 | 已結 |
