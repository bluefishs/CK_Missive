...
```
  governance_fitness_report_freshness_hours                   6.2
  ...
  scheduler_job_last_run_age_seconds{job_id="tender_dashboard_warm"}        235.9
  scheduler_job_success_created{job_id="tender_dashboard_warm"} 1780189923.8
...
- `0ab8e66f feat(scheduler): cron events jsonl log + dashboard §9.6 歷史摘要 (紀錄變文件化)`
- `20b39e27 fix(dashboard): §9.5 cron 真活全表自動抓 (對齊 owner 真活大於規劃)`
...
**統計**：1 真活 cron / 1 GREEN / 0 YELLOW / 0 RED
...
| 時間 | Job | 狀態 | 耗時 |
|---|---|---|---|
| 09:12:03 | `tender_dashboard_warm` | ✅ success | 353ms |

**統計** (最近 1 個事件): 1 成功 / 0 失敗 / 失敗率 0.0%
