...
### 承攬金額合計 vs 應收總額（2026-09-05 owner「是否皆以含稅呈現」⇒ 兩頁同一口徑）

| 頁 | 卡片 | 算法 | 稅 |
|---|---|---|---|
| `/contract-cases` | **承攬金額合計（含稅）** | Σ COALESCE(NULLIF(`winning_amount`,0), `contract_amount`)，年度＝`contract_projects.year` | 含稅 |
| `/erp/quotations` | **應收總額（含稅）** | Σ 成案報價單的承攬金額（議價→契約→報價總價），年度＝案號 `CK{年}` | 含稅 |
...
| `contract_amount_sum` | 承攬金額合計（含稅） | 目前篩選範圍內所有承攬案的承攬金額（議價金額，無則契約金額）加總，含稅。與報價單頁「應收總額（含稅）」同一個數。 |
| `quotation_total` | 報價總價（含稅） | erp_quotations.total_price，含稅；請款、發票都以它為上限。未稅＝總價 − 稅額。 |
| `receivable_total` | 應收總額（含稅） | 篩選範圍內成案的承攬金額（議價金額→契約金額→報價總價）加總，含稅。與承攬案頁「承攬金額合計（含稅）」是同一個算法、同一個數；未稅＝÷1.05。 |
...
**改名紀錄**：合約總額 → **契約金額**（原報價）；**議價金額**＝決標後實際金額（只有 01 類）；**承攬金額**＝議價金額，無則契約金額——應收面用它；應收總額（未稅）→ **應收總額（含稅）**（09-05，與承攬案頁同數）。報價單詳情頁「應收總額」（含稅總價）→ **報價總價（含稅）**。
