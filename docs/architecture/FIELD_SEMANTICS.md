...
| 頁 | 卡片 | 算法 | 稅 |
|---|---|---|---|
| `/contract-cases` | **承攬金額（含稅）** | `stats.finance.awarded_amount()`＝議價→契約→報價總價（三層），年度＝`year` 欄優先 | 含稅 |
| `/erp/quotations`、財務儀表板類別分解 | **承攬金額（含稅）** | **同一支** `awarded_amount()`，年度＝`stats.finance.case_year_condition()` | 含稅 |

> ⭐ **2026-09-09 owner「異值同工」收斂**：上表兩列此前是**兩個算法**（承攬案頁只有兩層、年度用 `year` 欄；
> 報價單頁三層、年度用案號年），而這份「權威文件」自己就把兩個口徑並列著。
> 現在算法只在 `backend/app/services/stats/finance.py` 一份，`test_finance_metrics.py` 15 條鎖住；
> 在它以外自己寫金額算式會被 weekly 132 點名。**要算錢，從那裡拿片段。**
...
| 委託單位帳款、廠商帳款、帳齡、依類別統計、財務摘要一覽 | **案件年度＝`year` 欄優先、案號年後備**（`stats.finance.case_year_condition`；09-09 類別分解仍用案號年，與上方 KPI 差 1,693 萬，已收斂）——**09-05 owner 裁示**：「桃園 2026 應僅 2 件委辦案件」。09-04 曾改成報價單年，但 `erp_quotations.year` 是**補建那年**（14/277 張成案報價單與案號年不同；桃園 CK2023_01_01_001 的報價單 year=2026），用它篩 2026 會列進 2023、2025 的案——09-04 說的「少 563 萬」正是那張 2023 案的報價單。而且當時年度只掛在金額子查詢，案件數根本沒篩 | 案件管理與財務同一個年度，數字才對得上 |
...
| `billed` | 已請款 | **有請款日期**的請款單金額加總，含稅（`stats/finance.BILLED_CONDITION`）。成案時自動建的第一期是**應收佔位**（請款日期留白，09-07），**不算已請款**——09-09 owner「案件皆請款？」：舊定義把佔位算進去，每個案成案那一刻就已請款 100%（全庫 31,180,060／85 筆）。上限與超支判斷（110%、請款超支）另算，要含佔位。 |
| `unbilled` | 未請款 | 承攬金額 − 已請款，含稅（成案佔位屬於這裡）。 |
| `received` | 已收款 | 已收款的請款實收金額（erp_billings.payment_amount）加總，含稅。 |
| `outstanding` | 應收未收 | **承攬金額 − 已收款**，含稅（09-09 起；此前寫已請款−已收款，兩者在佔位算進已請款時同值）。列表頁點此卡列「承攬金額 > 已收款」的案。 |
...
owner：「5 筆的『已收 17,850』已付清、發票多開 —— 是否異常案件標註機制並增列
篩選查詢，以利解除或處理異常費用之案件機制」。

**異常是推導的，不是報價單上的一個旗標。** 判準的唯一定義在
`backend/app/services/erp/finance_anomaly.py`；把它存成欄位，那一刻起就有第二份
宣告（發票補開、請款追加時旗標不會跟著動 ⇒ 列表說異常而數字已經正常，或反過來）。

| 代碼 | 標籤 | 級別 | 判準（逐案彙總，含稅） |
|---|---|---|---|
| `invoice_over_billing` | 發票額大於請款額 | RED | 未作廢發票合計 > 請款合計 |
| `paid_over_billing` | 已收款大於請款額 | RED | 實收合計 > 請款合計 |
| `billing_over_contract` | 請款額大於合約總價 | RED | 請款合計 > 報價總價 × 1.001（千分之一容差＝四捨五入） |
| `paid_without_invoice` | 已收款但未開發票 | YELLOW | 實收 > 0 且未作廢發票合計 ＝ 0 |

**判讀（ack）解除的是待辦，不是事實**：`erp_finance_anomaly_acks` 記的是
「有人看過了，原因是 X」（原因必填）。判讀後標籤仍在、轉灰標「已判讀」——
數字沒變。若判讀會讓標籤消失，這個機制就成了「一鍵讓問題從畫面上消失」。

⚠️ **與 weekly 104 不是同一份，這是刻意的**：104 逐筆比對（這張發票 vs 它綁的
那次請款），這裡逐案彙總。兩種粒度會給出不同答案 —— 一案分兩次開票互補時逐筆會紅
而逐案不紅；發票沒綁 `billing_id` 時逐筆看不到而逐案看得到。要防的是
「一邊改了另一邊不動」，由 **weekly 127**（`finance_anomaly_parity_audit.py`）
盯方向一致性（一邊 0 一邊非 0 才紅，數量不等不紅）。

入口：`/erp/quotations` 的「金流異常」欄與篩選（待判讀／全部）、
API `/api/erp/anomalies/{types,list,ack,unack}`。

## 報價單「總價是否含稅」（`erp_quotations.tax_included`，2026-09-08）

來源＝總表 K 欄「稅內含」。

| 值 | 意思 | 工項小計 → 總價 |
|---|---|---|
| `true` | 工項單價**已含稅** | 總價 ＝ 小計，`tax_amount` ＝ 0 |
| `false`（預設） | 工項單價**未稅** | 總價 ＝ 小計 × 1.05，`tax_amount` ＝ 小計 × 5% |

此前這個旗標只有匯入器寫得到，線上填報無從指定 ⇒ 線上建的每一張都被當成未稅、
小計一律 ×1.05；對「單價本身已含稅」的案那是**多算一次稅**，而小計／稅額／總計
三個數字彼此自洽、畫面上看不出錯。09-08 起可在報價單明細分頁切換
（`/erp/quotations/:id?tab=items`、`/pm/cases/:id?tab=quotations` 同一個元件），
切換後由後端用同一份算式重算（`QuotationItemService._apply_totals` 是唯一定義）；
已有請款者會被既有的那把鎖擋下（請款額與發票額都對著總價）。守門＝weekly 123 家族的
`erp_amount_semantics_audit` ⑮（有工項但小計×1.05 ≠ 總價）。
