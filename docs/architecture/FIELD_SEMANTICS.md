...
### 承攬金額合計（含稅）——兩頁同名同數（2026-09-05 owner「文字與統計統一為承攬金額合計（含稅）」）

| 頁 | 卡片 | 算法 | 稅 |
|---|---|---|---|
| `/contract-cases` | **承攬金額合計（含稅）** | Σ COALESCE(NULLIF(`winning_amount`,0), `contract_amount`)，年度＝`contract_projects.year` | 含稅 |
| `/erp/quotations` | **承攬金額合計（含稅）**（owner 09-05：文字與統計統一） | Σ 成案報價單的承攬金額（議價→契約→報價總價），年度＝案號 `CK{年}` | 含稅 |
...
| `contract_amount_sum` | 承攬金額合計（含稅） | 篩選範圍內所有承攬案的承攬金額（議價金額→契約金額→報價總價）加總，含稅。承攬案頁與專案帳款頁同一個名字、同一個算法、同一個數（owner 09-05 統一）；未稅＝÷1.05。 |
...
**改名紀錄**：合約總額／應收總額（未稅）／應收總額（含稅）→ **承攬金額合計（含稅）**（09-05 定案，承攬案頁與專案帳款頁同名同數）；**契約金額**＝成案時報價；**議價金額**＝決標後實際（只有 01 類）；**承攬金額**＝議價無則契約。報價單詳情頁「應收總額」（含稅總價）→ **報價總價（含稅）**。
