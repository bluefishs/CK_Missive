...
<!--enforced-by: scripts/checks/container_image_freshness_check.py（weekly 60：容器內容 vs 映像／host 同檔漂移）-->
...
<!--not-enforceable: 修法是結構性的（EnhancedTable／ResponsiveTable 共用 stripClientOnlyColumnFeatures 剝除器）；新表格外殼要用同一支剝除器靠 review-->
...
<!--enforced-by: scripts/checks/container_image_freshness_check.py（container_vs_image_drift 整棵樹比對，不再手抄清單）-->
...
<!--enforced-by: scripts/checks/run_fitness_daily.sh（第 16 步業務鏈探針＝端點層固定樣本；本次改動涉及的端點仍靠 review 在端點層驗）-->
...
<!--not-enforceable: 「量測方法先驗」是慣例——新檢核附 self_test／正向控制，靠 review 不靠檢核-->
...
<!--not-enforceable: 診斷方法論（基底率 vs 管線缺陷），靠 review 不靠檢核-->
...
<!--not-enforceable: 診斷方法論（顯形時刻 vs 成因時刻），靠 review 不靠檢核-->
...
<!--not-enforceable: 應急診斷程序（docker 引擎卡死的兩種成因），落在 runbook 與 memory，不是可強制的規則-->
...
<!--not-enforceable: runbook 撰寫慣例（裝置以 UUID 定位、步驟用 $DEV），靠 review-->
...
<!--not-enforceable: 行為準則——轉述他人結論必標「未經本 repo 查證」，靠 review 不靠檢核-->
...
<!--not-enforceable: 操作紀律——對不可逆函式試算前先 grep 鏈路上有沒有 commit，或在測試庫跑；事後由承攬案基線抓到但不算防範-->

| 欄位 | 內容 |
|---|---|
| **Trigger** | A81 匯入 41 筆的腳本用外層 `db.rollback()` 當 dry-run，第一次跑到第 41 筆撞防呆崩潰。第二次 dry-run 說前 40 筆「legacy 已存在」——它們**已經在資料庫裡**。 |
| **真因** | `promote_to_project` 第 533 行自己 `await self.db.commit()`。08-29 的記憶檔 `rollback_did_not_roll_back` 第一句就是「對不可逆函式試算前先 grep 它有沒有 commit」，我當天讀過那份記憶、沒做。 |
| **結果** | 寫入的 40 PM／40 報價單／39 承攬案／38 指派逐筆核對與預期一致，**沒有損失——那是運氣不是設計**。 |
| **修法** | 改 `begin_nested()` savepoint 也擋不住內部 commit（它會結束外層交易）。可靠的 dry-run 只有兩種：在測試庫跑，或 **grep 確認鏈路上沒有 commit** 再談 rollback。 |
| **一般形式** | 讀過教訓 ≠ 會在需要時想起來。把「grep commit」寫成腳本檔頭的固定檢查（`lib_adoption_audit` 那類），比靠記憶可靠。 |

## L140 — 每張表單獨看都正常，鏈才是斷的；而 50% 門檻看不見 20% 的系統性偏差（2026-09-04）

| 欄位 | 內容 |
|---|---|
| **Trigger** | /loop 金流複查。委託單位帳款 2026 比損益摘要多 150,024，追下去是 5 張「未成案卻有請款」的報價單，再追是 **16 筆 PM 案標 `contracted` 而沒有承攬案**——匯入服務對總表「已成立」的列只寫狀態、不建承攬案。 |
| **為什麼沒人看到** | PM 列表：狀態已承攬 ✓。報價單：confirmed ✓。請款：pending ✓。承攬列表：那 16 件**不在**——而「不在」不是一個會紅的訊號。weekly 97–104 各查一個環節，沒有一支問「已承攬 ⇔ 承攬案存在」。 |
| **第二層** | 同一輪用**比值分布**掃 `pm.contract_amount / q.total_price`：1.105×119、1.235×103、1.000 只有 17。**230 張報價單總價錯、少記 482 萬**，而 weekly 100 的門檻是「差 >50%」——每一張都在門檻內。錯法有固定簽名（`含稅−2×稅`／`未稅×0.85`），簽名比門檻精確得多。 |
| **修法** | weekly 105 案件狀態一致性（已承攬⇔承攬案⇔project_code⇔結案同步）；weekly 104 加 ⑨ 簽名 RED／⑩ 其他不等 YELLOW；匯入「成立即走正式 promote，被擋的列出來」。 |
| **一般形式** | ①**狀態是一個宣稱，記錄才是事實**——「已承攬」的事實是那筆承攬案，檢核要問事實在不在，不是問狀態對不對。②**跨表金額先看比值分布再定門檻**：系統性偏差集中在一個比值上，隨機誤差才是散的；門檻只擋得住後者。③**權限被擋的批次寫入不轉包、不換路徑**（協同界線③）——列成待授權項目，把更正清單與執行步驟留給 owner。 |

## L141 — 部署把服務放進空窗，而三次修法才對；vite 不做型別檢查，缺 import 的 bundle 隨 dirty build 上線（2026-09-04）
<!--enforced-by: scripts/deploy/deploy-public.sh（build 前 tsc --noEmit 閘門，失敗即停不動 dist）-->

| 欄位 | 內容 |
|---|---|
| **Trigger** | owner 一天內三次回報「頁面載入失敗：Failed to fetch dynamically imported module」。第一次我判「舊 index.html 快取」——實測 index 本來就是 no-cache；第二次「保留上一版 assets」——build 完才補回，而 **vite 是先清空 dist 再寫**，清到寫完的幾十秒內所有 chunk 404；第三次才把 build 移到 `dist_next` 覆蓋式更新、24 小時內舊 assets 保留、前端攔 `vite:preloadError` 重載。 |
| **第二件** | 我把 `useQuery` 的 import 拿掉（以為沒人用），eslint 擋住了提交，但我的指令鏈用 `;` 接部署 ⇒ **提交失敗、部署照跑**，vite 不做型別檢查，缺 import 的 bundle 上線幾分鐘。 |
| **修法** | 部署腳本：build 前先 `tsc --noEmit`（閘門）；build 失敗即停、不動 dist。指令鏈：提交與部署之間用 `&&`。 |
| **一般形式** | ①**「檔案不存在」有兩種時間形狀**：永久（快取／世代）與瞬時（寫入中的空窗）；用 curl 事後量到 200 只排除前者。②**工具鏈的每一段都要問「它擋什麼」**：eslint 擋提交不擋部署、vite 擋語法不擋型別；沒被任何一段擋住的東西就是會上線的東西。③ 第一個能解釋症狀的假說在「旁邊剛有大事」時最危險（同 L132）——這次旁邊的大事是「今天部署了 24 次」。 |

## L142 — 「檢查通過」≠「到達生效路徑」：把 START 降 DEBUG，stdout 上一行沒少（2026-09-05）
<!--not-enforceable: 驗收準則——改等級／轉義／設定的修法要量生效路徑的輸出，靠 review-->

| 欄位 | 內容 |
|---|---|
| **Trigger** | CK_AaaP 量到本容器每請求 3 行 INFO、一度佔全平台日誌 53.8%。第一版修法把 `REQUEST_START` 從 INFO 降 DEBUG，py_compile 過、部署過、探針全綠；部署後複量 10 分鐘仍 301 行 START/END。原因：`ck_missive.*` logger 自己 `setLevel(DEBUG)` 且**傳播到 root 的 stdout handler**，「降級」在那條路徑上等於沒降。第二版改成 `LOG_LEVEL=DEBUG` 才產生，複量 1,243 → 204 行，與 AaaP 的 Loki 數字 219 對 219 一致。 |
| **同日同型** | 09-05 上午的 `(?\:` 轉義（第一版寫成 `\:` 是 Python 語法錯，會吵；若寫成 `\:` 而 SQL 端拿到 `\:` 就是安靜地錯）；AaaP 同日回報他們的 `${VAR}` 進雙引號段標題、`-replace '\'` 被吃成 `''`——語法檢查都過。 |
| **修法** | 凡是「改了等級／改了轉義／改了設定」這類**不改行為路徑本身**的修法，驗收一律量**生效路徵的輸出**（stdout 行數、SQL 實際送出的字串、日誌實際寫到的檔），不是量「檔案改了」「編譯過了」「部署過了」。 |
| **跨 repo 對照** | `CK_AaaP#L51`「一個在東西壞了時仍會通過的探測，不是弱探測，是『不是探測』」——同日 AaaP 也踩三次（bash -n 不展開變數／PowerShell 不驗正則字串／自己的迴圈無條件印 ✅）。可操作的問法：**「我剛跑的檢查，如果這個改動完全沒生效，會不會照樣通過？」會 ⇒ 換一個能證偽的觀察（執行一次／比對前後／造反例）**。與 `CK_AaaP#L37`「守門錨在非生效物件」的分野：L37 是看錯對象（改了不生效的那份），本條是看對對象但檢查維度不涵蓋失敗模式；本 case 改對了檔、驗錯了維度。本 repo 保留自己的編號（registry 分裂之慮以 FQID 互引解決，不複製條文）。 |
| **一般形式** | 三個訊號都會綠：語法檢查、部署探針、以及「我確信這樣改是對的」。它們驗的都是**改動本身**，沒有一個驗**改動有沒有走到那條路**。與 L141（vite 不做型別檢查）、L100（旗標在、呼叫時少了旗標）同族：**驗證訊號的粒度比被驗證的性質粗**。 |

## L143 — 視覺走查抓到閘門看不見的桌面回歸；而我先花一小時證明「不能模擬手機登入」（2026-09-05）

| 欄位 | 內容 |
|---|---|
| **Trigger** | owner 問「為何無法模擬行動裝置登入畫面檢測」。我先走 Chrome 擴充（沒登入態、不能代登）再走免認證 Vite（有版面沒資料），一小時後才想起 `run.sh --visual` 早就會在容器內簽臨時憑證、以 390px 登入拍圖——每日探針靠的就是它。拍出來的桌面圖立刻顯示 09-05 引入 FilterBar 後三個列表頁的下拉被壓成 40px（Row 放進 Space 的 inline-flex item 寬度塌成內容寬），而 109／110 兩支 RWD 閘門都不會紅，因為它們只量溢出與手機。 |
| **修法** | ①記憶 `feedback_mobile_login_screenshot_use_visual_walk`：手機／登入後畫面的問題第一步就是 `run.sh --visual`，routes 不帶前導斜線（Git Bash 會轉成本機路徑拍到 404）。②視覺走查加 `--click=` 先點再拍。③weekly 111 `rwd_mobile_quality_gate` 量截字／字級／點擊目標／遮蔽／統計卡獨列／**下拉塌陷**，390 與 1440 兩個寬度都跑；負向對照：部署前 pm/cases 3、contract-cases 3 個塌陷，部署後 0。 |
| **一般形式** | **工具存在但沒有成為預設路徑**，與 lib 採用率 3.3%（L93 家族）同一件事——使用者因此以為系統沒有這個能力。另：**手機閘門量不到桌面回歸**（手機走收合區、Select 100% 寬），RWD 是連續的不是二元的，同一個判準要在兩個寬度都跑（08-15 mobile_probe 加 768 的理由一樣）。 |

## L144 — 「不溢出」與「讀得到」不是同一件事；而少讀一層的資料只會變成空表，不會報錯

| 欄位 | 內容 |
|---|---|
| **Trigger** | owner 09-06 丟五個頁面網址。逐頁走查抓到三件事，三件的共同點是**沒有任何錯誤訊息**。 |
| **① 擠壓不是溢出** | `/taoyuan/dispatch` 在 390px 下 10 欄擠在一起、每格剩 1–2 個字垂直堆疊。08-03 與 08-29 修過兩次，都是「拿掉 scroll.x／拿掉欄寬／藏 3 欄」—— 那讓**整頁溢出量測歸零**，於是 weekly 109 一直是綠的。修法：窄螢幕改用卡片（與其他列表頁同一個形狀），而不是繼續調欄寬。 |
| **② 少讀一層** | `/erp/financial-dashboard` 的「專案財務一覽」與「案件損益 Top 15」都是空的。端點回 `{success, data:{items}}`，而 API 層的型別寫成沒有包裝的 `{items}` ⇒ 頁面讀 `.items` 永遠 `undefined`。**TypeScript 也擋不住**：型別是人手寫的斷言，不是實際回應。 |
| **③ 一排卡片只有一張帶小數** | 四張統計卡只有「應收未收」是 39,431,749.5 —— 含稅換算生出 .5，其他卡剛好整除。多出的字元讓那張卡在 390px 換行、高度不齊。`fmtMoney` 一律 `Math.round`（只改顯示，資料庫仍存原值）。 |
| **一般形式** | **量測的維度沒有涵蓋使用者實際在意的那個維度**：溢出量得到「有沒有超出畫面」，量不到「一格裡剩幾個字」；型別量得到「我宣稱回什麼」，量不到「它實際回什麼」。⇒ 新增判準前先問「使用者抱怨的是什麼，我這個數字會不會在那件事發生時仍然是 0」。 |
| **守門** | 卡片化後 weekly 111 五指標全綠（截字 0／字級 0／點擊目標 57＜基線 59／遮蔽 0／統計卡獨列 0）。②的形狀已在 `docs/architecture/AUTONOMOUS_TESTING_MAP.md` 記為缺口：前端沒有「型別 vs 實際回應」的比對層。 |

## L145 — 一份宣告改了，另一份沒改：權限碼、稽催錨點、名稱快照，同一天三次

**2026-09-07。** 三件事各自獨立發生，形狀完全相同：**同一個事實在系統裡有兩份宣告，
改了其中一份，另一份不會跟著動，而且沒有任何一方會報錯。**

| # | 兩份宣告 | 症狀 |
|---|---|---|
| ① | 角色層 `role_permissions` ↔ 使用者層 `users.permissions` | 拆出新權限碼只改角色層 ⇒ 管理員打開委託帳款是空的，畫面上看不出原因 |
| ② | 三份各自實作的「逾期幾天」 | 請款日改留白，漏改一處那 86 筆佔位就從該消費端整批消失（198→118），畫面只會顯示「逾期變少了」 |
| ③ | 委託單位主檔 ↔ 案件的名稱快照 | 改主檔名稱，列表與篩選下拉還是舊名；選了新名就是空表 |

**三者的修法也是同一個形狀**：先把定義收成一份，再補一支盯「有沒有第二份出現」的檢核。
① weekly 119 加角色↔使用者比對；② 定義收進 `billing_dunning.py` ＋ weekly 121；
③ 顯示改成主檔優先 ＋ 改名時傳播快照。

### 兩個關於判準的自我更正（同日）

* weekly 121 首版判「檔案裡有沒有出現模組名」，而三個消費端的**註解**都寫著
  「見 billing_dunning.py」⇒ 負向控制把 import 拿掉也不會紅，**判準等於沒有作用**。
  改成只認真正的 import 才紅。
* weekly 120 首版逐字比對報 8 筆漏匯入，其中 2 筆實際是**同一案的不同版次**
  （`B114-C031-0` 對上 DB 的 `-1`）。**把誤報當事實交出去，會讓人做出錯的清理動作。**

### 而「型別對不上」只有實跑看得到

②的定義寫成 `COALESCE(billing_date, quoted_at)` 之後才發現前者是 `date`、後者是 `datetime`：
Python 端直接 TypeError，SQL 端回 interval（畫面印「逾期 557 days 天」）。
靜態讀程式碼看不出這個差別 —— **收斂定義之後一定要把每個消費端各跑一次**。

---

## L146 — 匯入器讀到了「彙總區塊」的欄位，而那一欄剛好也叫報價金額
<!--enforced-by: scripts/checks/quotation_master_table_diff.py（weekly 120：總表 vs 資料庫含金額欄對帳）-->

**2026-09-07。** 總表右側（第 38–43 欄）另有一個各承辦年度小計的區塊，於是
「報價金額」這個表頭在同一張工作表**出現兩次**。欄位對應是
`{HEADER_MAP[h]: i for i, h in enumerate(header)}` —— **後面的覆蓋前面的**
⇒ 讀到第 41 欄的小計。實測 `B110-020-0A` 被寫成 351,200（賴柏霖 2026 年度小計），
正確值是 39,000；其餘各列該欄是空的 ⇒ 總價 None ⇒ 成案被擋。

⇒ **同名表頭取最左邊那一個**（主表在左，右側是後來貼上去的輔助區塊）。

### 同日的另外兩個「不會報錯的略過」

* 沒有報價單編號的列，匯入器的 `if not legacy: continue` **連略過都沒有記錄** ——
  而缺案名、不是案號都會進 `skipped` 並顯示在預覽裡。4 列、約 246 萬從未進系統。
* 唯一索引**不排除軟刪除的列** ⇒ 重建被刪過的編號會 IntegrityError 整批回滾。
  那筆是「存在但被刪」，而所有查詢都帶 `deleted_at IS NULL` ⇒ 看起來像「沒匯入」。

**共同教訓：略過與成功在畫面上長得一樣時，資料就會安靜地少。**

## L147 — 檢核是在看兩份宣告不一致；收斂是讓不一致不可能發生

**2026-09-07。** owner：「感覺整個系統四分五裂，重覆優化架構鬆散。」他是對的。

同一天同一個形狀出現十次（見 L145 的表加上 ERP 六頁、兩張圖譜、坤哥的來源契約）：
**同一件事有兩份宣告，改了一份另一份不會動，而且沒有任何一方會報錯。**
而我每次的反應是**加一支檢核** —— 本月 89 支、當天 6 支、總數 214。

檢核的本質是「看著」兩份宣告有沒有分家，不是「讓它們不可能分家」。
它讓問題**可見**，但架構沒有變緊；所以每次事故照樣發生，只是發現得比較早。
這就是「重覆優化」的體感來源：檢核數量上升，而同一類事故不停重演。

### 真正有效的那幾次，都是刪掉第二份宣告

* `case_staff.py`：承辦查詢從八份收成一份 ⇒ 那一族問題不再發生
* `billing_dunning.py`：稽催錨點從三份收成一份
* **收斂 A**：角色成為權限唯一來源，使用者清單與旗標改為推導 ⇒ weekly 119 ③④ 降為僅資訊
* **收斂 B**：選單表成為選單／路由守衛／API 三邊唯一來源 ⇒ 改一次三邊生效，零部署

**判準：一項收斂做完，對應的檢核應該可以降級或刪除。檢核數量下降才是架構變緊的訊號。**

### 兩個過程中的錯

* 依 exec 角色的**設計註解**（「全域唯讀」）移除其寫入權 —— 實測三位 exec 全是承辦（一位 81 案）。
  **角色的定義要看那個角色的人實際在做什麼，不是看當初寫下的意圖。**
* 前端 `USER_ROLES.default_permissions` 差點被當成權威套回 DB —— 那是**建立角色時的範本**，
  照套會讓管理員失去 ERP 財務、業務同仁失去 owner 指定保留的專案帳款。
  **收斂之前先問「這份宣告是誰在維護、給誰用的」，不是看它叫什麼名字。**
...
**部署後第二層（同日）**：新容器上線後公網抓同一個 PDF **仍 200** —— `cf-cache-status: HIT`、`Age: 1907`。
是我 30 分鐘前那次未登入實測把它放進了 Cloudflare 邊緣快取；沒抓過的第二個檔已 401。
而這不只是殘留：回應沒帶 `Cache-Control` 時 CF 依副檔名快取 PDF，**登入者抓一次、之後任何人未登入都拿得到**，
認證等於只擋第一次。補 `Cache-Control: private, no-store`（CF 對 private／no-store 回 BYPASS）。
⇒ **有 CDN 在前面時，「端點要認證」還要加一句「回應不得可快取」**；驗收要用**沒抓過的路徑**，
不然你量到的是自己上一次的快取。已在 CF 快取裡的附件只有 purge 能清＝owner 動作。

## L149 — 分歧存在、結果相同、只是還沒發作：數字對得起來不能證明只有一份實作

**2026-09-09。** owner 定調「異值同工」、要求把統計整合成中心服務。盤點得到：承攬金額 4 份實作、已請款／已收款／應付各 3 份、「誰是全公司視角」2 份、案件統計範圍兩份平行程式碼。

同一天有**三處**是同一個形狀：

* 「已收款」三份的狀態條件是 `IN ('paid','partial')`／`= 'paid'`／完全不看狀態——**三個不同的東西**，而三者算出同一個 23,072,409，因為 59 筆有金額的請款狀態剛好全是 `paid`
* 承攬金額：統計卡兩層（議價→契約）、財務指標三層（＋報價總價）——2026 年度無差，全年度有 25 件會不同
* 全公司視角：`has_company_wide_scope`（超管＋角色集合）與 `_quotation_scope`（超管＋跨案權限）——三位主管／財務兩邊都不限縮，因為他們**剛好同時符合兩份判準**

三處都「數字相同」，三處都是**等第一筆不符合的資料進來就會分家，而不會有任何一方報錯**。
過去一個月反覆修「某頁某卡數字不對」，修的都是**已經發作**的；還沒發作的比發作的多。

修法＝中心服務（`services/stats/finance`＋`case_scope`＋`core/case_scope`）＋15 條口徑鎖＋weekly 132（新增第二份即紅、存量 7 處走基線）。統一到最嚴謹的那份**實測不改變任何現有數字**——那正是「還沒發作」的證據。
...
## L151 — 檢核守得住「一致」，守不住「定義」：一個錯的定義被一致地套用，每一支檢核都是綠的

**2026-09-09 晚。** owner 看 `/erp/client-accounts`：承攬金額 5,057,835、已請款 5,057,835 ——「案件皆請款？這也是大問題」。
實查：09-03「成案即應收」讓系統在成案當下自動建一筆請款單（金額＝承攬金額）當應收佔位，與真的請款單同表同欄；
09-04 寫進名詞字典的「已請款」定義就是「所有請款單加總，含成案自動建的第一期」⇒ **每個案成案那一刻就已請款 100%**。
全庫 96,872,983 的「已請款」裡 31,180,060 是這種佔位（85 筆、全無請款日期）。09-07 把佔位的請款日期留白，只改了畫面，沒改加總。

**為什麼一個月立的檢核都沒抓到**：weekly 132 守「只有一份實作」、131 守「列表與卡片同身分」、108 守「卡片有接篩選」——
全部在驗**一致**，沒有一支在驗**意義**。定義錯了而被一致地套用，每一支都是綠的；能判斷「這個數字對不對得上現實」的只有看數字的人。
⇒ 這一類只能靠**語意鎖**：把定義寫成可執行的斷言（「有請款日期才算已請款」進 `stats/finance.BILLED_CONDITION`，`test_finance_metrics` 鎖片段含這個條件），
之後改定義的人至少要改到測試。

**同日第二個形狀（承辦篩選）**：`/erp/quotations` 選了承辦，列表 85 張、卡片仍是全公司 112 張——列表 schema 有 `staff_user_id`、損益摘要 schema 沒有。
兩份請求宣告在不同時間各自長，沒有一支檢核比對「統計端點收得到列表的每個篩選欄位嗎」。⇒ weekly 133（從 runtime `app.routes` 讀兩邊的 model，缺即紅；豁免寫在列表 schema 上）。
**首版假綠**：FastAPI 0.135 的 `ModelField` 沒有 `type_`，13 組全讀成 `None → None` 而印 GREEN——同一天第六次「判準自我更正」，加了「全部讀不到就不下結論」。

**第三個形狀（字樣判準的盲區）**：weekly 132 的判準只認 SQL 文字 `SUM(…billing_amount`，`func.sum(ERPBilling.billing_amount)` 這種 SQLAlchemy Core 寫法它看不見——七處第二份實作躲了一天，
其中財務總覽的「已收」拿的是 paid 請款單的**請款金額**不是收款金額。⇒ 判準補 Core 寫法；上限／超支兩處（要含佔位）永久豁免並寫明理由。

判準句：**檢核只能證明各處算得一樣，不能證明算的是對的東西；定義要用測試鎖成斷言，而「這個數字合不合理」永遠要有人看。**

---

## L152 — 秒級的基礎設施故障，被應用層的懶初始化放大成「直到有人重啟」的全面 500

**2026-09-09 16:40–16:55。** 主機段錯誤把 backend 打掉，Docker 約 1 秒拉回。拉回後的第一波併發請求同時觸發 SQLAlchemy 的
**mapper 懶配置**而競態，registry 被毒化（`'_EventKey' object has no attribute 'dispatch'`）——之後**每一個**走 ORM 的請求都 500
（refresh／Google 登入／通知），15 分鐘 54 次，直到我手動重啟。owner 從登入頁看到 500 回報。

兩層要分開看：段錯誤是主機層的事（A127，程式擋不住）；**把半分鐘變成一刻鐘的是我們自己的懶初始化**。
修法＝`lifespan` 在 `yield` 前 `configure_mappers()`，服務開門前把只該做一次的事做完；回歸鎖兩條。

同族：任何「第一次用到才初始化、而初始化不是原子的」東西（mapper、快取預熱、單例），在高頻重啟的環境裡都會變成這個形狀。
判準句：**只該做一次的初始化，要在開門前做完；靠第一個請求去做的，會被第一波併發做壞。**
