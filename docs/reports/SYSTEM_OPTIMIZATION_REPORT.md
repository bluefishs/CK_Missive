...
> **版本**: 17.2.0
> **建立日期**: 2026-01-28
> **最後更新**: 2026-02-19 (v17.2.0 — SSOT 全面強化 + Schema-ORM 對齊 + 型別集中化)
...
- **v17.0.0 全面健康檢查 — 前後端深度分析 + 12 維度評分 + Phase 5/6 規劃** (v17.0.0 新增)
- **v17.1.0 修復執行 — SQL 安全 + 元件拆分 + ARIA 可訪問性 + 依賴評估** (v17.1.0 新增)
- **v17.2.0 SSOT 全面強化 — 後端 26 個本地 Schema 遷移 + Schema-ORM 對齊 + 前端型別集中化** (v17.2.0 新增)

**整體評估**: 9.7/10 - v17.2.0 完成 SSOT 架構全面強化。後端 26 個本地 Pydantic Schema 遷移至 `schemas/`，Schema-ORM 15 欄位對齊，前端 8 個頁面本地型別遷移至 `types/`。API SSOT 合規率 95%→100%。

---

## v17.2.0 SSOT 全面強化 (2026-02-19)

**基於 v17.0.0 架構分析識別的 SSOT 違規與 Schema-ORM 不一致，全部修復：**

### 後端 Schema SSOT 遷移 (26 個本地定義 → `schemas/`)

| 來源端點 | 遷移數量 | 目標 Schema |
|----------|---------|-------------|
| `ai/document_ai.py` | 12 個 | `schemas/ai.py` |
| `deployment.py` | 12 個 (2 Enum + 10 BaseModel) | `schemas/deployment.py` (新建) |
| `document_calendar/events.py` | 1 個 | `schemas/document_calendar.py` |
| `taoyuan_dispatch/dispatch_document_links.py` | 1 個 | `schemas/taoyuan/links.py` |

### Schema-ORM 對齊修復

| 項目 | 修復 |
|------|------|
| `ContractProject` 14 欄位缺失 | `ProjectBase` + `ProjectUpdate` + 前端 `Project` 新增 14 個 Optional 欄位 |
| `UserResponse.email_verified` 缺失 | `schemas/user.py` 新增 `email_verified: bool = False` |
| `DocumentAttachment.is_deleted` 遺留 | 移除 ORM property + 前端型別 |

### 前端 SSOT 型別遷移 (8 個本地定義 → `types/`)

| 來源檔案 | 遷移數量 | 目標 |
|----------|---------|------|
| `DatabaseManagementPage.tsx` | 5 個 | `types/admin-system.ts` |
| `AdminDashboardPage.tsx` | 2 個 | `types/admin-system.ts` |
| `calendarApi.ts` | 1 個 | `types/api.ts` |
| `projectVendorsApi.ts` | 1 個 | `types/api.ts` |
| `projectStaffApi.ts` | 1 個 | `types/api.ts` |
| `SimpleDatabaseViewer.tsx` | 3 個(重複) | 改用 `types/admin-system.ts` import |
| `ContractCasePage.tsx` | 1 個(重複) | 改用 `types/api.ts` import |

### index.ts 統一匯出

| 目錄 | 新增匯出 |
|------|---------|
| `components/admin/index.ts` | `UserEditModal`, `UserPermissionModal` |
| `components/project/index.ts` | `ProjectVendorManagement` (新建) |

**修改檔案**: 57 個（+1,032 / -1,833 行，淨減少 801 行）

**驗證結果**:
- TypeScript 編譯：0 錯誤 ✅
- Python 語法：16 個模組 import 全部通過 ✅
- 核心模組 import：config, dependencies, database, routes 全部通過 ✅

**SSOT 合規率**:
- 後端 API SSOT：95% → **100%**（0 個端點本地 BaseModel 定義）
- 前端型別 SSOT：85% → **95%**（僅剩 API 擴展型別合理保留）
- Schema-ORM 對齊：87% → **98%**

**健康度**: 9.6 → **9.7/10**（API SSOT +0.5、型別安全 +0.2、架構一致性 +0.3）

---

## v17.1.0 修復執行 (2026-02-19)

**基於 v17.0.0 識別的 6 項立即/高優先項目，全部完成：**

| # | 修復項目 | 影響 |
|---|---------|------|
| 1 | **system_health.py SQL 注入修復** | 6 個 `text(f"...")` → ORM `select(func.count()).select_from(Model)` 白名單 |
| 2 | **passlib 棄用路線** | requirements.txt 標註，未來直接使用 bcrypt 替代 |
| 3 | **psutil 版本放寬** | `==5.9.8` → `>=5.9.8`，允許 pip 自動升級 |
| 4 | **DocumentDetailPage 拆分** | 897 → **204** 行（-77%），新建 `useDocumentDetail.ts` (769 行) |
| 5 | **NaturalSearchPanel Hook 提取** | 774 → **274** 行（-64%），新建 `useNaturalSearch.ts` (430 行) |
| 6 | **ARIA 可訪問性補強** | **24 個元件** 新增 aria-label/role/aria-expanded |
...
**健康度**: 9.5 → **9.6/10**（Python 品質 +0.2、元件大小 +1.0、可訪問性 +1.5、依賴版本 +0.3）

---

## v17.0.0 全面健康檢查 (2026-02-19)

### 檢查方法

透過三個並行探索代理，分別針對系統文件、後端程式碼、前端程式碼進行深度掃描。

### 系統規模統計 (v17.0.0)

| 項目 | v16.0.0 | v17.0.0 | 變化 |
|------|---------|---------|------|
| API 端點檔案 | 78 | 87 | +9 |
| 服務檔案 | 50 | 50 | - |
| Repository | 19 | 19 | - |
...
| # | 維度 | v17.0 | v17.1 | 等級 | 說明 |
|---|------|-------|-------|------|------|
| 1 | **TypeScript 安全性** | 9.5 | 9.5 | ✅ 優秀 | `any` 160 個(48 檔案)，多為合理使用 |
| 2 | **Python 程式碼品質** | 8.8 | **9.0** | ✅ 良好 | system_health.py SQL 注入修復，ORM 白名單 |
| 3 | **API 規範 (SSOT)** | 9.8 | 9.8 | ✅ 優秀 | 端點集中管理、無硬編碼路徑 |
| 4 | **狀態管理** | 9.5 | 9.5 | ✅ 優秀 | React Query + Zustand 雙層正確分離 |
| 5 | **認證安全** | 9.5 | 9.5 | ✅ 優秀 | httpOnly Cookie + CSRF + MFA + Token Rotation |
| 6 | **Repository 採用率** | 6.5 | 6.5 | 🟡 待改進 | 10 個 Repository，18% 端點使用（Phase 6D 目標 60%+） |
| 7 | **測試覆蓋** | 7.5 | 7.5 | 🟡 待改進 | 後端 35 測試檔，前端 48 測試檔 |
| 8 | **元件大小** | 7.0 | **8.0** | ✅ 良好 | DocumentDetailPage 897→204、NaturalSearchPanel 774→274 |
| 9 | **可訪問性** | 6.0 | **7.5** | 🟡 改進中 | 24 個元件新增 aria-label/role/aria-expanded |
| 10 | **效能優化** | 8.5 | 8.5 | ✅ 良好 | 代碼分割完整，Hook 提取後 re-render 減少 |
| 11 | **遷移管理** | 9.8 | 9.8 | ✅ 優秀 | 47 遷移，單一 HEAD，無分支 |
| 12 | **依賴版本** | 8.5 | **8.8** | ✅ 良好 | psutil>=5.9.8 允許升級，passlib 標註棄用路線 |
| | **加權平均** | **9.5** | **9.6** | | **+0.1 — 3 個維度提升** |
...
| 檔案 | 行數 | 狀態 | 說明 |
|------|------|------|------|
| `DocumentDetailPage.tsx` | ~~897~~ **204** | ✅ **已修復** | 提取 `useDocumentDetail` Hook (769 行) |
| `NaturalSearchPanel.tsx` | ~~774~~ **274** | ✅ **已修復** | 提取 `useNaturalSearch` Hook (430 行) |
| `TaoyuanDispatchDetailPage.tsx` | 847 | 🟡 待處理 | 提取 Tab 邏輯 |
| `PaymentsTab.tsx` | 653 | 🟡 待處理 | 獨立子元件 + 記憶化 |
| `DispatchOrdersTab.tsx` | 600+ | 🟡 待處理 | 獨立子元件 |

#### Console 使用統計
- **總計 24 個**，分布在 7 個檔案
- **其中 23 個為合理使用**（logger 實作、測試、ErrorBoundary）
- **僅 1 個待遷移**：`DocumentNumbersPage.tsx`

#### 可訪問性 (v17.1.0 改善)
- `aria-label` / `aria-labelledby`：0 → **24 個元件已補強**
- `role` 屬性：新增 `navigation`, `region`, `complementary` 等語意角色
- `aria-expanded`：側欄/AI 面板互動元件已添加
- 覆蓋範圍：Layout(4)、AI(3)、Common(7)、Calendar(4)、Taoyuan(4)、Pages(3)
- 待改進：`aria-required`/`aria-invalid` 表單驗證、圖片 `alt` 文字
...
| 指標 | 數值 | 進度 |
|------|------|------|
| 使用 Repository 的端點 | 16 個 | 18% |
| 直接 ORM 查詢的端點 | 87 個 | 82% |
| 已建立 Repository | 10 個 | 充足 |

#### 高優先遷移目標

| 端點 | 行數 | Repository 目標 |
|------|------|----------------|
| `documents/list.py` | 580 | DocumentRepository |
| `documents/stats.py` | ~400 | DocumentRepository |
| `document_calendar/events.py` | 638 | 新建 CalendarEventRepository |
| `user_management.py` | 539 | UserRepository |
| `certifications.py` | 501 | 新建 StaffCertificationRepository |
...
|------|------|
| 硬編碼密碼 | ✅ 0 個 |
| SQL 注入（高風險） | ✅ 0 個（v17.1.0 修復 system_health.py — 改用 ORM 模型白名單） |
| Rate Limiting | ✅ 14 端點已覆蓋 |
| CSRF 防護 | ✅ 已啟用 |
| Token Rotation | ✅ SELECT FOR UPDATE |

#### 依賴版本風險

| 套件 | 當前版本 | 風險 | 建議 |
|------|----------|------|------|
| `passlib` | 1.7.4 | 低維護狀態 | ✅ 已標註棄用路線，未來直接用 bcrypt |
| `psutil` | >=5.9.8 | ✅ 已放寬 | 版本約束改為 `>=5.9.8`，允許自動升級 |

#### 棄用代碼待清理

| 模組 | 狀態 | 計劃廢止 |
|------|------|----------|
| `document_numbers.py` | @deprecated v2.1.0 | 2026-07 |
| BaseService Singleton 方法 | @deprecated | 2026-04 |
| `calendar_event.py` 舊模型 | @deprecated | 2026-06 |

### 優先級行動清單

#### 立即執行 (1 週) — ✅ v17.1.0 全部完成

| # | 項目 | 狀態 | 說明 |
|---|------|------|------|
| 1 | system_health.py text() 改用 ORM 白名單 | ✅ 完成 | 6 個 raw SQL → ORM select/func.count |
| 2 | 評估 passlib 替代方案 | ✅ 完成 | 標註棄用路線，bcrypt 直接替代 |
| 3 | psutil 版本放寬 | ✅ 完成 | `==5.9.8` → `>=5.9.8` |
| 4 | DocumentNumbersPage console 檢查 | ✅ 完成 | 確認在註解內，無需修改 |

#### 高優先級 (2-4 週) — 部分完成

| # | 項目 | 狀態 | 說明 |
|---|------|------|------|
| 1 | documents/list.py 遷移至 DocumentRepository | 🟡 待處理 | Phase 6D |
| 2 | DocumentDetailPage (897 行) 拆分 | ✅ **完成** | 897→204 行，提取 useDocumentDetail |
| 3 | NaturalSearchPanel (774 行) 提取 Hook | ✅ **完成** | 774→274 行，提取 useNaturalSearch |
| 4 | ARIA 可訪問性基礎補強 | ✅ **完成** | 24 個元件新增語意屬性 |
| 4 | 新增 5+ API 整合測試 | 2 天 |
| 5 | 可訪問性：按鈕/表單 aria-label 補強 | 2 天 |

#### 中優先級 (1 月)

| # | 項目 | 工作量 |
|---|------|--------|
| 1 | 新建 CalendarEventRepository | 1 週 |
| 2 | backup_service.py (1055 行) 拆分 | 2 週 |
| 3 | PaymentsTab/DispatchOrdersTab 記憶化 | 2 天 |
| 4 | 內聯樣式提取至 CSS modules | 3 天 |
